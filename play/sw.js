  ...
  e.waitUntil(
    caches.keys().then(function (keys) {
      return Promise.all(keys.map(function (k) {
        // only OUR caches: caches.keys() is origin-wide, so an unfiltered
        // delete here wiped the portal and every satellite.
        return (k.indexOf("sws-play-") === 0 && k !== CACHE) ? caches["delete"](k) : null;
      }));
