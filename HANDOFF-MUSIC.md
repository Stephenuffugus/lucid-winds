...
sw.js                            ONE `||` clause on the line-259 rule + ONE 3-line guard above the line-298 static rule
...
**LAW 10. No pre-existing file may shrink.** `test/music/no_shrink.mjs` runs `git diff --numstat <P0 commit>..HEAD` and fails if any file that existed at P0 shows deletions. This handoff is purely additive to existing files. If you believe a deletion is required, stop and ask.
...
a. `sw.js`: add `|| url.pathname === '/music-unlocks.js' || url.pathname === '/music-catalog.js'` to the `:259` condition; add, ABOVE the `:298` static rule: `if (url.pathname.indexOf('/music/') === 0) return; // audio streams, never cached (HANDOFF-MUSIC LAW 5)`. Gate `test/music/sw.mjs`: load `sw.js` in `vm` with a fake `caches`/`fetch`, dispatch a fetch for `/music/v1/x/y.mp3`, assert no `cache.put`; dispatch `/music-unlocks.js`, assert it took the s-w-r path.
