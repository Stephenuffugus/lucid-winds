...
(function(){
  ...
  function _scheduleCheck(){
    ...
    _checkPending = setTimeout(function(){
      ...
      try {
        var r = checkAll();
        if (!r) return;
        // Stephen 2026-05-03: toast is now interactive — CLAIM button opens
        // BoS straight onto the QUESTS tab so players don't have to hunt.
        // Plain CSS (no class refs) keeps this self-contained inside the
        // toast's HTML payload. _toClaimToast(...) builds the markup.
        function _toClaimToast(emoji, name) {
          var btn = '<button onclick="if(window._toast)window._toast(\'\');if(window._openCompendiumUI)window._openCompendiumUI();setTimeout(function(){if(window._bosSetTab)window._bosSetTab(\'quests\');},220);" '+
            'style="margin-top:0.5rem;padding:8px 18px;min-height:36px;border:1.5px solid rgba(200,168,75,0.7);border-radius:8px;background:linear-gradient(180deg,rgba(200,168,75,0.32),rgba(120,100,40,0.45));color:#0d100c;font-family:Bebas Neue,sans-serif;font-size:0.7rem;letter-spacing:0.14em;font-weight:700;cursor:pointer;text-shadow:0 1px 1px rgba(255,240,200,0.4);">CLAIM REWARD</button>';
          return '<div>'+emoji+' '+name+'</div>'+btn;
        }
        var _toastOpts = { html:true, duration:7000, interactive:true };
        // Daily completions — interactive toast with CLAIM button.
        if (r.newDailyDone && r.newDailyDone.length) {
          for (var di = 0; di < r.newDailyDone.length; di++) {
            ...
            if (de && window._toast) window._toast(_toClaimToast('☀️ Daily complete:', '<b>'+de.name+'</b>'), _toastOpts);
          }
        }
        ...
        if (r.newWeeklyDone && r.newWeeklyDone.length) {
          for (var wi = 0; wi < r.newWeeklyDone.length; wi++) {
            ...
            if (we && window._toast) window._toast(_toClaimToast('🌙 Weekly complete:', '<b>'+we.name+'</b>'), _toastOpts);
          }
        }
        ...
        if (r.newAch && r.newAch.length) {
          for (var ai = 0; ai < r.newAch.length; ai++) {
            ...
            if (ae && window._toast) {
              ...
              window._toast(_toClaimToast(tierEmoji, '<b>'+ae.name+'</b>'), _toastOpts);
            }
  ...
  var ACH = [
    ...
    // ── Friendship gifting (LW_GIFTS, 2026-05-01) ──
    {id:'soc_gift_first',     tier:'bronze',   category:'SOCIAL',     name:'A First Quiet Gift',         desc:'Send your first gift to a friend.',                                    counter:'gifts_sent',       target:1,    reward:{type:'dew', amt:15}},
    {id:'soc_gift_10',        tier:'bronze',   category:'SOCIAL',     name:'Ten Gifts in the Mail',      desc:'Send 10 gifts to friends.',                                            counter:'gifts_sent',       target:10,   reward:{type:'dew', amt:40}},
    {id:'soc_gift_50',        tier:'silver',   category:'SOCIAL',     name:'Fifty Gifts Out',            desc:'Send 50 gifts to friends.',                                            counter:'gifts_sent',       target:50,   reward:{type:'dew', amt:120}},
    {id:'soc_gift_250',       tier:'gold',     category:'SOCIAL',     name:'A Postal Keeper',            desc:'Send 250 gifts to friends.',                                           counter:'gifts_sent',       target:250,  reward:{type:'dew', amt:300}},
    {id:'soc_gift_1k',        tier:'mythic',   category:'SOCIAL',     name:'A Thousand Quiet Gifts',     desc:'Send 1,000 gifts to friends.',                                         counter:'gifts_sent',       target:1000, reward:{type:'trophy', amt:1, name:'A Thousand Quiet Gifts'}},
    {id:'soc_received_first', tier:'bronze',   category:'SOCIAL',     name:'A First Gift Received',      desc:'Claim your first gift from a friend.',                                 counter:'gifts_received',   target:1,    reward:{type:'dew', amt:15}},
    {id:'soc_received_25',    tier:'silver',   category:'SOCIAL',     name:'Twenty Five Gifts Welcomed', desc:'Claim 25 gifts from friends.',                                         counter:'gifts_received',   target:25,   reward:{type:'dew', amt:80}},
    {id:'soc_received_100',   tier:'gold',     category:'SOCIAL',     name:'A Hundred Welcomes',         desc:'Claim 100 gifts from friends.',                                        counter:'gifts_received',   target:100,  reward:{type:'dew', amt:200}},
    // ── Twin Bloom (Tier 3 cross-distance breeding) ──
    {id:'soc_twin_proposed',  tier:'silver',   category:'SOCIAL',     name:'A First Twin Bloom Proposed',desc:'Propose your first Twin Bloom to a Tier-3 friend.',                    counter:'twin_proposed',    target:1,    reward:{type:'dew', amt:30}, hidden:true},
    {id:'soc_twin_first',     tier:'gold',     category:'SOCIAL',     name:'Twins, Raised Apart',        desc:'Complete your first Twin Bloom — same DNA, two keepers.',              counter:'twin_completed',   target:1,    reward:{type:'dew', amt:80}, hidden:true},
    {id:'soc_twin_10',        tier:'platinum', category:'SOCIAL',     name:'Ten Twins in the World',     desc:'Complete 10 Twin Blooms.',                                             counter:'twin_completed',   target:10,   reward:{type:'dew', amt:300}, hidden:true},
    {id:'soc_twin_50',        tier:'mythic',   category:'SOCIAL',     name:'A Lineage of Twins',         desc:'Complete 50 Twin Blooms.',                                             counter:'twin_completed',   target:50,   reward:{type:'trophy', amt:1, name:'A Lineage of Twins'}, hidden:true},
