/* AIRWORTHY service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete airworthy-* caches, or
...
const SHELL_VERSION = "airworthy-shell-20260906c";
const SFX_CACHE     = "airworthy-unused-v1"; // no recorded audio: every sound is synthesised
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("airworthy-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#070A1A;color:#E8E2D2;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#E8C97A'>AIRWORTHY</h1>" +
    "<p>Cannot reach the orchestra right now.<br>Check your connection and reload.</p></div></body>",
