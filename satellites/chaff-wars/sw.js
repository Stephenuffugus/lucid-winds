/* Pop N Lock service worker — installability + offline art. NETWORK-FIRST (house rule),
...
var CACHE = "chaff-wars-v12";   /* 2026-08-01: match outro + difficulty reflatten */
    ...
    return new Promise(function (r) { setTimeout(r, 3000); }).then(function () {
      return caches.keys().then(function (keys) {
        return Promise.all(keys.map(function (k) {
          return (k.indexOf("chaff-wars-") === 0 && k !== CACHE) ? caches["delete"](k) : null;
        }));
      });
    });
  }));
...
function fromCache(req) {
  return caches.match(req, { ignoreSearch: true }).then(function (hit) {
    ...
    if (req.mode === "navigate") {
      return caches.match("/satellites/chaff-wars/", { ignoreSearch: true }).then(function (shell) {
