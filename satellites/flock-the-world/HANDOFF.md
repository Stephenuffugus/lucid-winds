...
- **Skill trees** (45 nodes, 4 distinct shapes): DEPLOYMENT = broad pyramid; WATCHLIST = twin spine (Watch col ⟷ Enforce col + fusion rungs); NARRATIVE = web; CRISIS = escalation ladder with world-state `gate` functions (`threat` needs Crime Panic; `proxy` needs avgSus>=10 (live meter in its gate text; retuned from an unreachable >18 on 2026-08-25); `pretext` needs avgMil>0.12 and costs +6 oversight on buy). fx keys: `dep dth vis inc sup cmp spr dec mil fear route`.
