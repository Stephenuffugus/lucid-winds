/* PARALLEL service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete parallel-* caches, or
...
const SHELL_VERSION = "parallel-shell-v3";
const SFX_CACHE     = "parallel-sfx-v1";
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("parallel-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#0a0b0f;color:#e9e8f2;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#8b7cf6'>PARALLEL</h1>" +
    "<p>Cannot reach the game right now.<br>Check your connection and reload.</p></div></body>",
...
self.addEventListener("fetch", event => {
  ...
  // Recorded foley in ./sfx/ — immutable once shipped, so cache forever.
  if (url.origin === self.location.origin && url.pathname.indexOf("/sfx/") >= 0) {
    event.respondWith(
      caches.open(SFX_CACHE).then(cache =>
