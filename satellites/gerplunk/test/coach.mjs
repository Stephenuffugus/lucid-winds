 ...
 *      with the thumb still down, in the sky band CLEAR of that thumb and
 *      under the tally post, once, and a second slide past it shows nothing
...
say(facesOn.line === LINES.faces, 'and the faces beat is on the water with the thumb still down: ' + JSON.stringify(facesOn.line));
/* ⛔ AND NOT UNDER THAT THUMB. The review's composite (p8-coach-faces-thumb, the
   first cut) had the disc over "the left is glass" and the spin ring through
   the rest, because the line sat in the band the thumb slides in. The law is
   the line's box against a 45 px pad round the hold point, and the box under
   the tally post so it is not in the chip's top band either. */
const tx = x0 + 240, ty = y0;
const facesBox = await dev(() => { const r = document.getElementById('line').getBoundingClientRect(), p = document.getElementById('post').getBoundingClientRect(); return { top: r.top, bottom: r.bottom, postBottom: p.bottom }; });
say(facesBox.bottom < ty - 45 || facesBox.top > ty + 45, 'and the line is clear of the thumb that turned the lake: the box runs ' + facesBox.top.toFixed(0) + ' to ' + facesBox.bottom.toFixed(0) + ' px, the thumb pad ' + (ty - 45) + ' to ' + (ty + 45));
say(facesBox.top > facesBox.postBottom, 'and under the tally post, not in the chip\'s top band (post ends ' + facesBox.postBottom.toFixed(0) + ', the line starts ' + facesBox.top.toFixed(0) + ')');
