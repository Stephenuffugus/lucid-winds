...
let parsed = true, why = '';
try { createScript(JS, { filename: 'gerplunk-inline.js' }); }
...
const shellM = SW.match(/const SHELL_VERSION = "gerplunk-shell-([^"]+)"/);
...
say(!!shellM && !!stampM && shellM[1] === stampM[1],
  'and sw.js carries the same one' + (shellM ? ': gerplunk-shell-' + shellM[1] : ': sw.js has no gerplunk-shell version'));
const swOwn = SW.indexOf('gerplunk-') >= 0 && SW.indexOf('deepwell') < 0;
