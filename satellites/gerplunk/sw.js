/* GERPLUNK service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete gerplunk-* caches, or
...
const SHELL_VERSION = "gerplunk-shell-20260906a";
const SFX_CACHE     = "gerplunk-unused-v1"; // no recorded audio: every sound is synthesised
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("gerplunk-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#000000;color:#dfeef4;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#9FE8FF'>GERPLUNK</h1>" +
    "<p>Cannot reach the cave right now.<br>Check your connection and reload.</p></div></body>",
