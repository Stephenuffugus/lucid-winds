/* Bandit's Box service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete banditsbox-* caches, or
...
const SHELL_VERSION = "banditsbox-shell-v5";
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("banditsbox-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#1B1822;color:#EDE6DB;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#9FD6C0'>Bandit's Box</h1>" +
    "<p>Can't reach the box right now.<br>Check your connection and reload.</p></div></body>",
