#!/usr/bin/env node
/* GAUGE's laws, checked against the files a browser loads from satellites/gauge/ (everything but test/, tools/, docs/ and
 * package.json). Shape from satellites/tint/tools/lint.mjs.
 *
 *   node tools/lint.mjs
 *
 * What it asserts (plans/gauge/HANDOFF-GAUGE.md P0), each watched to fail on a planted line:
 *   1. every runtime module parses as an ES module
 *   2. nothing a browser loads is a .mjs
 *   3. every relative import and local asset carries ?v= plus GAUGE's stamp (STAMP.js), CORE's included
 ...
 *   9. no sound is played from inside a loop, a forEach or a map (GAUGE's GL2 kept: a pose is one event, one sound)
 *  10. GA9: no parseFloat, Number( or toFixed( anywhere a browser loads (decimals are strings and BigInt); GA5: no money in any
 *      string (dollar, cent, price, coin, change, currency signs); GA6: no string makes the point the hero; no sensors
 */
...
const files = walk(GAUGE, []);
const rel = p => relative(GAUGE, p);
...
/* 3 */
const stampFile = join(GAUGE, 'STAMP.js');
const STAMP = existsSync(stampFile) ? (read(stampFile).match(/export const STAMP = '([0-9]{8}[a-z])'/) || [])[1] : null;
say(!!STAMP, 'STAMP.js names GAUGE\'s one stamp' + (STAMP ? ': ' + STAMP : ''));
{
  const specs = [];
...
/* 4 */
for (const name of ['engine.js', 'decimal.js']) {
  const p = join(GAUGE, name);
...
/* 5 */
{
  const words = assertForbiddenStrings(GAUGE);
  say(words.ok, 'none of the catalog\'s forbidden words anywhere (' + words.detail + ')');
  const gum = assertNoGetUserMedia(GAUGE);
...
{
  const copy = [];
  const content = join(GAUGE, 'content.js');
   ...
   body of every for, while, forEach, map, reduce and some around it is read; none may hold one. */
{
  ...
  say(bad.length === 0, 'no sound is played from inside a loop, so one detent is one sound' + (bad.length ? ': ' + bad.join('; ') : ''));
}

/* 8: the sprite table, once there is one */
{
  const p = join(GAUGE, 'sprites.js');
  if (existsSync(p)) {
    let table = null, why = '';
...
/* 10: GA9, GA5, GA6 and no sensors */
{
  ...
  const floats = [];
  for (const p of JS.concat(HTML)) { const code = stripComments(read(p)); for (const w of ['parseFloat', 'Number(', 'toFixed(', 'toPrecision(']) if (code.indexOf(w) >= 0) floats.push(rel(p) + ' names ' + w); }
  say(floats.length === 0, 'GA9: no parseFloat, Number(, toFixed( or toPrecision( anywhere a browser loads' + (floats.length ? ': ' + floats.join(', ') : ''));
  const strings = [];
  for (const p of JS) for (const m of stripComments(read(p)).matchAll(/(['"`])((?:\\.|(?!\1)[^\\])*)\1/g)) strings.push([rel(p), m[2]]);
  for (const p of HTML) {
