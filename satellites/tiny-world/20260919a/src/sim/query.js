...
export function nearestHuman(w, e, r, f) {
  ...
  for (let k = 0; k < w.nHumans; k++) {
    const o = H[k];
    if (o === e || E.dead[o] || E.inside[o] || E.perch[o] || !f(w, e, o)) continue;
...
export function findGrass(w, e) {
  const T = w.T, cx = Math.floor(w.E.x[e] / T), cy = Math.floor(w.E.y[e] / T), n = w.R.graze.searchTiles, grass = w.C.tid.grass;
  let best = -1, bd = 1e9;
  for (let y = cy - n; y <= cy + n; y++) for (let x = cx - n; x <= cx + n; x++) {
    if (!inB(w, x, y)) continue;
    const i = y * w.cols + x;
    if (w.terr[i] !== grass || w.eaten[i] > 0) continue;
    const s = w.grid[i];
    if (s && (s.def.block || s.def.home)) continue;
    const dx = x - cx, dy = y - cy, d = Math.sqrt(dx * dx + dy * dy) + w.rng.float(); // hyp() + jitter
    if (d < bd) { bd = d; best = i; }
  }
  return best;
}
