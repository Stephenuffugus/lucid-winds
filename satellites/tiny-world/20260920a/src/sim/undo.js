...
const KEEP = ['id', 'hp', 'hunger', 'face', 'breedCd', 'born', 'baby', 'bless', 'frozen', 'healT', 'abCd'];
...
export const thingOp = (s) => ['thing', s.type, s.tx, s.ty, s.food, s.cd, s.manned, s.h];
...
function apply(w, op) {
  ...
  switch (op[0]) {
    ...
    case 'thing': {
      const [, type, tx, ty, food, cd, manned, was] = op, i = ty * w.cols + tx;
      if (w.grid[i]) return; // something else stands there now
      placeStruct(w, type, tx, ty);
      const s = w.grid[i];
      if (s) { s.food = food; s.cd = cd; s.manned = manned; remap(w, 'unbuild', was, s.h); }
      return;
    }
    case 'creature': {
