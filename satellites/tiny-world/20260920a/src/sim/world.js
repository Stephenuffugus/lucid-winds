...
export function resetWorld(w) {
  ...
  w.burn.fill(0); w.burnN = 0; w.tmr.length = 0;
...
export function passPt(w, e) {
  ...
  if (tx < 0 || ty < 0 || tx >= w.cols || ty >= w.rows) return false; // inB(), written out (no Math.floor result passed)
...
export function placeStruct(w, type, tx, ty) {
  ...
  const s = { type, def, tx, ty, occ: 0, food: w.R.food.max, cd: 0, h: ++w.thingSerial * w.nTiles + i, manned: undefined, cooked: 0 }; // manned: towers, set when a percher stands on it
  w.grid[i] = s; w.topo++;
  w.structs.push(s);
...
export function removeStructs(w, list) {
  ...
  keep(w.structs); keep(w.fires); keep(w.lights);
