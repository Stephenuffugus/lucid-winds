...
export const PC_LAND = 0, PC_AMPH = 1, PC_SEA = 2, PC_LAVA = 4, PC_HOME = 8;
...
export function tilePass(w, cls, i) {
  ...
  if (cls & PC_SEA) return t === tid.water;
  const s = w.grid[i];
  if (!(s && s.def.bridge)) {
    if (t === tid.rock) return false;
    if (t === tid.lava && !(cls & PC_LAVA)) return false;
    if (t === tid.water && !(cls & PC_AMPH)) return false;
  }
  if (s) {
