...
// Clear asks "Sure?" and a second tap within 2.5 s clears. The window is measured between the taps themselves
// (event time stamps): on a slow world the second tap's click is handled late, and must still count.
let clearArm = -1e9;
clearB.onclick = guard((ev) => {
  if (ev.timeStamp - clearArm < 2500) {
    sim.command({ t: 'clear' });
    selected = 0;
    clearB.textContent = str('ui.clear');
    clearArm = -1e9;
  } else {
    clearArm = ev.timeStamp;
