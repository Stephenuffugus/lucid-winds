...
export function step(w, e, mode) {
  ...
  if (w.R.path.sepPush > 0) separate(w, e);
  return false;
}
...
function fromCache(w, e, k) {
  ...
  E.pathN[e] = n; E.pathI[e] = 0; E.pathFresh[e] = 1; E.blockT[e] = 0;
}
...
// crowd heading for one gap does not stack into one point. Not away from its own target (a mate, a
...
function separate(w, e) {
  ...
  const gk = E.goalKind[e], mine = gk === G_ATTACK || gk === G_ABDUCT || gk === G_MATE ? ent(w, E.goalA[e]) : -1;
  let sx = 0, sy = 0, fl = -1;
  for (let cy = cy0; cy <= cy1; cy++) for (let cx = cx0; cx <= cx1; cx++) {
    for (let o = head[cy * w.gw + cx]; o >= 0; o = next[o]) {
      if (o === e || o === mine || E.dead[o] || E.inside[o]) continue;
      const ox = x - E.x[o], oy = y - E.y[o], d2 = ox * ox + oy * oy;
      if (d2 >= r * r) continue;
      if (fl < 0) fl = flies(w, e) ? 1 : 0;
      if ((flies(w, o) ? 1 : 0) !== fl) continue;
      if (d2 === 0) { sx += E.id[e] < E.id[o] ? -1 : 1; continue; } // on the very same point: split by age
      const d = Math.sqrt(d2);
      sx += ox / d; sy += oy / d;
    }
  }
  if (sx === 0 && sy === 0) return;
  const m = Math.sqrt(sx * sx + sy * sy), v = P.sepPush * w.dt, pp = w.pp, nx = x + (sx / m) * v, ny = y + (sy / m) * v;
  pp[0] = Math.max(1, Math.min(w.W - 1, nx)); pp[1] = Math.max(1, Math.min(w.H - 1, ny));
