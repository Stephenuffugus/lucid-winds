...
import { log, ref, inB, flies, removeStruct, setTerrain, markDirty } from './world.js';
...
export function usePower(w, id, x, y) {
  ...
  if (id === 'bolt') {
    ...
    const e = nearPoint(w, x, y, P.radius);
    if (e >= 0) {
      const c = spawn(w, E.kind[e], E.x[e] + P.offset, E.y[e]);
      E.gear[c] = { ...E.gear[e] };
...
export function useInstant(w, id) {
  ...
  if (id === 'rain') {
    ...
    w.shake = P.shake;
    let n = 0;
    for (const st of w.structs.slice()) if (P.topple.includes(st.type) && w.rng.float() < P.toppleChance) { removeStruct(w, st.tx, st.ty); n++; }
    for (let k = 0; k < w.count; k++) {
      const e = w.order[k];
