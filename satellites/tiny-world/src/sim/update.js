...
import { setPos, gather } from './spatial.js';
...
function heal(w, e) {
  ...
  // Every hurt human within reach, in spawn order (the first one healed gets the heart).
  const n = gather(w, E.x[e], E.y[e], HL.radius), near = w.near;
  for (let j = 0; j < n; j++) {
    const o = near[j];
    if (!(E.kind[o] === 'human' && !E.dead[o] && !E.inside[o] && E.hp[o] < maxHp && dist(w, e, o) < HL.radius)) continue;
    E.hp[o] = Math.min(maxHp, E.hp[o] + HL.rate * dt);
    if (E.healT[e] <= 0) { E.healT[e] = HL.heartEvery; addFx(w, 'heart', E.x[o], E.y[o] - 10, R.fx.healHeart); }
  }
}
