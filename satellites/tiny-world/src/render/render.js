...
import { sprite, palette, tileHash, atlasRef, atlasImage } from '../art/sprites.js';
...
export function createRenderer(cv, art, cam) {
  ...
  let animPhase = 0, animT = 0, epoch = -1, ncx = 0, ncy = 0, chunks = [], world = null, cssPx = 0;
  const PAL = palette();
  ...
  function drawChunk(w, ci, p) {
    ...
    let can = c.cans[p];
    if (!can) { can = c.cans[p] = document.createElement('canvas'); can.width = tw * 8; can.height = th * 8; }
  ...
  function drawEnt(w, e, selected, k, time, lag) {
    ...
    const S = w.C.S, sp = S[kind], G = E.gear[e], ref = spriteOf(w, e, kind, sp, G), sz = E.baby[e] ? 6 : 8, X = Math.round(lerpPos(E.px[e], E.x[e], k)), Y0 = Math.round(lerpPos(E.py[e], E.y[e], k));
    const alt = E.alt[e] > 0 ? E.alt[e] : E.perch[e] ? 7 : sp.ufo ? 7 + Math.sin(time * 4 + E.born[e]) * 1.5 : 0, Y = Y0 - Math.round(alt);
  ...
  function frame(w, selected, rand, k = 1) {
    ...
    for (const it of list) {
      if (it.e !== undefined) { if (far) drawDot(w, it.e, k, z); else drawEnt(w, it.e, sel, k, time, lag); } // slot 0 is a creature too
    ...
    const dt = (time % w.R.daySec) / w.R.daySec, nf = w.R.nightFrac, ramp = art.nightRamp;
    let dark = dt > nf ? Math.min(1, (dt - nf) / ramp) : 0;
    if (dt > 1 - ramp) dark = Math.max(0, (1 - dt) / ramp);
    if (dark > 0) {
      // The night layer covers the part of the world in view (logical px), lights punched out of it.
      ...
      for (const f of w.lights) {
        const cx = f.tx * T + 4, cy = f.ty * T + 4;
        for (let r = f.def.light + flick; r > 4; r -= 5) { nctx.fillStyle = 'rgba(0,0,0,.3)'; nctx.fillRect(cx - r, cy - r + 3, r * 2, r * 2 - 6); nctx.fillRect(cx - r + 3, cy - r, r * 2 - 6, r * 2); }
      }
      for (const s of w.structs) if (s.def.home && s.occ > 0) { nctx.fillStyle = 'rgba(0,0,0,.6)'; nctx.fillRect(s.tx * T + 3, s.ty * T + 3, 5, 5); }
    ...
    for (let j = 0; j < w.fxN; j++) {
      ...
      if (f.type === 'arrow') { ctx.strokeStyle = f.col || '#f4f4f0'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(f.x, f.y); ctx.lineTo(f.x2, f.y2); ctx.stroke(); }
        ...
        let x = f.x;
        for (let y = 0; y < f.y; y += 3) { x = f.x + ((tileHash(y, Math.floor(f.t * 20), 7) * 7) | 0) - 3; ctx.fillRect(x, y, 2, 3); }
