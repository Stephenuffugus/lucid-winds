...
import { dial } from './decide.js';

export function wander(w, e) {
  const R = w.R.ai, E = w.E;
  // Design 15 B2: a curious creature ranges further than a wary one (the same two random draws, in the same
  // order, whatever the dial says: a personality must not change the stream).
  // A starving creature strikes out in longer legs instead of jittering on the spot. The leg it actually walks
  // is capped by wanderThink, so roamMul above 2 buys nothing (96 px of ground covered in 40 s at 1, 129 at 2,
  ...
  goalMove(w, e, clamp(4, w.W - 4, E.x[e] + rnd(w, -far, far)), clamp(4, w.H - 4, E.y[e] + rnd(w, -far, far)), 0);
...
export function step(w, e, mode) {
  ...
  let v = sp.spd * (mult || 1) * (E.baby[e] ? w.R.breed.babySpeed : 1) * tm * (E.gear[e].boots ? w.R.move.boots : 1) * (E.gear[e].leadboots ? w.R.move.leadboots : 1) * w.dt;
  if (v > d) v = d;
