...
export function attachInput(cv, g, getCam, onCamera, U) {
  ...
  cv.addEventListener('pointerdown', (ev) => {
    ...
    if (pts.size === 1) {
      ...
      one.timer = setTimeout(() => {
        ...
        one.timer = 0;
        // Design 15 A2: with something sprayable in hand a still finger starts pouring; otherwise the Hand lifts.
        if (g.pour && g.pour(...world(one.p))) { one.state = 'pour'; return; }
        one.state = g.longPress(...world(one.p)) ? 'carry' : 'still';
      }, g.holdMs ? g.holdMs() : U.longPressMs);
    } else if (pts.size === 2) {
      stop();
      if (one && one.state !== 'carry') { if (one.state === 'drag' || one.state === 'pour') g.dragEnd(); g.cancel(); one.state = 'gone'; }
      const [mx, my, d] = mid();
  ...
  cv.addEventListener('pointermove', (ev) => {
    ...
    if (one.state === 'pour') { g.drag(world(prev), world(p), p[0] - prev[0], p[1] - prev[1]); return; } // pouring along the path
    const slop = one.state === 'press' && g.holdSlop ? g.holdSlop() : U.tapSlop;
    if ((one.state === 'press' || one.state === 'still') && Math.hypot(p[0] - one.p0[0], p[1] - one.p0[1]) > slop) {
      stop();
      one.state = 'drag';
  ...
  const up = (ev) => {
    ...
    else if (s === 'pour' || s === 'drag') g.dragEnd();
