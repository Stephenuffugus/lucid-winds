...
import { log, setTerrain, removeStruct, placeStruct, inB, isNight, speciesCapOf } from './world.js';
...
export function compileReactions(C) {
  ...
  const idx = byTrig.map((rows) => {
    ...
    for (const r of rows) {
      ...
      else if (A.kinds) for (const k of A.kinds) aKinds.add(k);
...
function picOf(C, ref) {
  ...
  if (kind === 'thing') return C.iconOf['thing:' + id] === undefined ? -1 : C.iconOf['thing:' + id];
  if (kind === 'gear') return C.iconOf['thing:item:' + id] === undefined ? -1 : C.iconOf['thing:item:' + id];
  if (kind === 'icon') return C.iconOf[id] === undefined ? -1 : C.iconOf[id];
  return -1;
}
...
export function layEgg(w, e, poked) {
  ...
  if (countThings(w, 'egg') >= R.react.eggCap) return false;
...
export function wearing(w, e, id) {
  const g = w.E.gear[e];
  if (!g) return false;
  if (g.weapon === id) return true;
  for (let k = 0; k < GEAR_SLOTS.length; k++) if (g[GEAR_SLOTS[k]] === id) return true;
  return false;
}
...
function needsOk(w, row) {
  ...
  if (n.night !== undefined && n.night !== isNight(w)) return false;
  ...
  if (n.looks !== undefined && n.looks !== !(w.rx.A.thing && (w.R.react.lookSkip || EMPTY_LIST).indexOf(w.rx.A.thing.type) >= 0)) return false; // worth walking over for (not a fence)
  return true;
}
...
function run(w, trig, x, y) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const key = row.i + '|' + handleOf(w, R.A) + '|' + handleOf(w, R.B);
    if (row.mode === 'once' && onCooldown(w, row, key)) continue;
    if (row.globalCd && onCooldown(w, row, row.i + '|*')) continue; // once anywhere per globalCooldownSec
    if (fire(w, row, key, x, y)) { if (row.globalCd) R.cool.set(row.i + '|*', w.time + row.globalCd); return true; } // first match wins, unless it turned out to be about nothing
  }
  return false;
}

const EMPTY_LIST = [];
function countThings(w, type) { let n = 0; for (const s of w.structs) if (s.type === type) n++; return n; }
...
function fire(w, row, key, x, y) {
  ...
  if (w.R.flags.honestRows) {
    ...
    let did = 0;
    const each = w.R.flags.clockLands && (row.trig === TRIG.clock || row.each);
    for (const eff of row.subst) { const verb = VERBS[eff.do]; if (verb) did += (eff.on ? onOne(w, row, eff, verb, x, y) : each && PLACED_VERBS[eff.do] ? atEach(w, row, eff, verb) : verb(w, row, eff, x, y)) | 0; }
  ...
  const sayKey = row.i + '|say';
  const quiet = row.sayEvery > 0 && onCooldown(w, row, sayKey);
  if (row.say && !quiet) {
    const th = R.B.thing && R.B.thing.name ? R.B.thing : R.A.thing && R.A.thing.name ? R.A.thing : null;
...
const PLACED_VERBS = { fx: 1, sound: 1, spawn: 1, makeThing: 1 };
function atEach(w, row, eff, verb) {
  const R = w.rx, E = w.E, cap = w.R.react.eachCap, T = w.T;
  let did = 0, n = 0;
  if (R.tN) {
    const tN = R.tN;
    for (let k = 0; k < tN && n < cap; k++, n++) { const e = R.targets[k]; did += verb(w, row, eff, E.x[e], E.y[e]) | 0; }
  } else for (let k = 0; k < R.tileN && n < cap; k++, n++) { const i = R.tileList[k]; did += verb(w, row, eff, (i % w.cols) * T + 4, ((i / w.cols) | 0) * T + 4) | 0; }
  return did;
}
...
export const VERBS = {
  ...
  flee(w, row, eff) {
    ...
    for (let k = 0; k < R.tN; k++) {
      const e = R.targets[k];
      const from = eff.from === 'a' ? R.A : eff.from === 'b' ? R.B : R.B.e === e ? R.A : R.B;
      if (from.e === e || E.inside[e] || E.alt[e] > 0 || E.perch[e]) continue;
      const dx = E.x[e] - from.x, dy = E.y[e] - from.y, d = Math.sqrt(dx * dx + dy * dy) || 1;
      goalMove(w, e, clampW(w, E.x[e] + (dx / d) * A.fleeDist), clampH(w, E.y[e] + (dy / d) * A.fleeDist), 1);
      E.think[e] = eff.sec || w.R.react.fleeSec;
  ...
  follow(w, row, eff) {
    ...
    for (let k = 0; k < R.tN; k++) {
      ...
      goalMove(w, e, tx, ty, 0); E.think[e] = hold; went++;
  ...
  launch(w, row, eff) {
    ...
    for (let k = 0; k < R.tN; k++) {
      ...
      if (dx && w.R.flags.pokes) {
        ...
        if (lt && (lt.deep === 1 || w.terr[li] === w.C.tid.lava) && !(w.grid[li] && w.grid[li].def.bridge)) dx = 0;
      }
  ...
  makeThing(w, row, eff, x, y) {
    ...
    if (!type || !w.C.BLD[type]) return 0;
    const from = eff.from === 'a' ? w.rx.A.e : eff.from === 'b' ? w.rx.B.e : -1;
    const kind = from >= 0 ? w.E.kind[from] : null; // read before the new thing's own reaction can touch the scratch
    if (w.C.BLD[type].hatch && w.R.react.eggCap && countThings(w, type) >= w.R.react.eggCap) return 0; // a world does not fill with eggs
    let made = 0;
    for (let k = 0; k < (eff.n || 1); k++) {
      ...
      if (!put) continue;
      made++;
      if (kind) { put.who = w.C.kid[kind] + 1; if (from >= 0 && w.E.named[from]) put.name = w.E.name[from]; } // an egg remembers whose it is (update.js hatches it)
    }
    return made;
  },
  ...
  dropGear(w, row, eff) {
    ...
    for (let k = 0; k < R.tN; k++) {
      ...
      rebuildGear(w, e);
      // `throw`: it lands that many px away, the way the thrower is facing (a ball thrown for a dog).
      const lx = eff.throw ? clampW(w, E.x[e] + E.face[e] * eff.throw) : E.x[e];
      putThing(w, 'item:' + id, Math.floor(lx / T), Math.floor(E.y[e] / T));
  ...
  size(w, row, eff) {
    ...
    for (let k = 0; k < R.tN; k++) {
      ...
      else E.bigT[e] = Math.max(E.bigT[e], eff.sec || w.R.react.giantSec);
      n++;
    }
    return n;
  },
  ...
  hurry(w, row, eff) {
    ...
    s.cd = Math.min(s.def.hatchSec, s.cd + (eff.sec || w.R.react.hurrySec));
    return 1;
  },
  ...
  visit(w, row, eff) {
    const R = w.rx, E = w.E, T = w.T, r = eff.r || 120, cap = w.R.react.visitCap;
    let n = 0;
    ...
    for (let k = 0; k < R.tN && n < cap; k++) {
      ...
      if (bx < 0) continue;
      goalMove(w, e, clampW(w, bx), clampH(w, by), 0);
      // Held for as long as the walk takes and a moment more, so it gets there instead of twitching toward it and
      // forgetting; never longer than visitMax, so nothing stands blind for long.
      E.think[e] = Math.min(w.R.react.visitMax, Math.sqrt(bd) / Math.max(4, sp.spd * w.C.mv[0]) + (eff.hold || w.R.react.visitSec));
  ...
  eatGear(w, row, eff) {
    ...
    let n = 0;
    for (let k = 0; k < R.tN; k++) {
      const e = R.targets[k], g = E.gear[e], slot = eff.slot || 'head';
      if (!g[slot]) continue;
      g[slot] = slot === 'weapon' ? '' : 0;
      rebuildGear(w, e);
      addFx(w, 'block', E.x[e], E.y[e] - 9, w.R.fx.block);
