...
const tToy = (s) => !!s.def.item && !!TW.C.ITAG[s.def.item] && TW.C.ITAG[s.def.item].has('toy') && tMine(s);
...
// Beds in the world: what every home holds, added up, and kept until a thing is put up or taken down (w.topo).
function bedsOf(w) {
  if (w.bedsTopo === w.topo) return w.beds;
  let n = 0;
  for (let k = 0; k < w.structs.length; k++) { const h = w.structs[k].def.home; if (h) n += h; }
  w.beds = n + w.R.breed.bedsSpare; w.bedsTopo = w.topo;
  return w.beds;
}
...
const BEHAVIOURS = [
  ...
  { id: B_MATE, max: 30, // fed, healthy adults ready to breed, under both caps
    score(w, e) {
      ...
      if (kind === 'human' && w.R.flags.beds && w.nHumans >= bedsOf(w)) return false;
