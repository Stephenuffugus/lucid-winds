  ...
  return {
    ...
    tap(x, y, cssX, cssY) {
      ...
      if (m === 'hand') {
        ...
        if (hs.length >= U.picker.min && gap < 3) onPicker(hs, cssX, cssY);
        else if (hs.length) poke(hs[0]);
        else {
          onSelect(0);
          // Nobody under the finger: whatever THING is there is poked instead (design 17: a crate, a well, a hay
          // stack, a music box). The tile is the sim's to look up; the gesture only says where the finger was.
          const w = getSim().w, tx = Math.floor(x / w.T), ty = Math.floor(y / w.T);
          if (tx >= 0 && ty >= 0 && tx < w.cols && ty < w.rows && w.grid[ty * w.cols + tx]) { ui.thingPoke = { tx, ty, t: now() }; onPokeThing(tx, ty); }
        }
