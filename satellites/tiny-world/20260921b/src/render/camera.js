...
export function createCamera() {
  const cam = {
    ...
    clampView() {
      ...
      // (No wider than half the field: a tiny field could otherwise be panned right off the world.)
      const z = cam.zoom, halfW = cam.cw / 2 / z, halfH = cam.ch / 2 / z, m = (64 * cam.dpr) / z, mx = Math.min(m, halfW), my = Math.min(m, halfH);
      if (cam.worldW * z <= cam.cw) cam.x = cam.worldW / 2; else cam.x = Math.min(cam.worldW + mx - halfW, Math.max(halfW - mx, cam.x));
      if (cam.worldH * z <= cam.ch) cam.y = cam.worldH / 2; else cam.y = Math.min(cam.worldH + my - halfH, Math.max(halfH - my, cam.y));
    },
