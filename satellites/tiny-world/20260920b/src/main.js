...
function withSound(s) {
  ...
  const cmd = s.command;
  s.command = (c) => { cmd(c); try { audio.command(c, s.w); } catch (e) { report('sound', e); } };
  s.withSound = true;
  return s;
}
...
const ui = { held: null, pokes: new Map(), outlines: new Map(), follow: 0, now: 0, spark: null, why: new Map(), U: data.ui };
...
gestures = createGestures({
  ...
  onPoke: (h) => { const e = ent(sim.w, h); if (e >= 0) audio.chirp(sim.w.C.S[sim.w.E.kind[e]], sim.w.E.x[e], sim.w.E.y[e]); },
});
...
const doll = createDoll({ data, getSim: () => sim, command: guard((c) => sim.command(c)) });
...
attachInput(cv, g, () => (sim ? cam : null), () => {}, data.ui);
...
cv.addEventListener('pointerdown', hidePicker, true);
...
track.addEventListener('pointerdown', (ev) => { if (!sim) return; railing = true; track.setPointerCapture(ev.pointerId); railSet(ev); });
...
function useSim(s, view) {
  ...
  ui.held = null; ui.follow = 0; ui.pokes.clear(); ui.outlines.clear(); ui.why.clear(); hidePicker(); because.clear(); doll.hide();
...
function closeSheet() { if (sheet.classList.contains('on') && menuPaused) paused = false; menuPaused = false; sheet.classList.remove('on'); }
  ...
  if (!paused && !broken) {
    ...
    try {
      while (budget >= step && n < MAX_STEPS_PER_FRAME) {
        ...
        try { doll.note(sim.w, because.step(sim.w)); } catch (e) { report('because', e); } // before the sounds: it reads the events ring too
...
if (Q.has('debug')) window.__tw = { get sim() { return sim; }, get speed() { return speed; }, get paused() { return paused; }, get broken() { return broken; }, errors, cam, renderer, saver, get store() { return store; }, audio, ui, because };
