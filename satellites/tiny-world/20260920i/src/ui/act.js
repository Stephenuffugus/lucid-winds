...
export function createActor({ getSim, getTool, onSelect, spareFor = null, gid = null }) {
  ...
  function act(x, y, drag) {
    ...
    // Permission to build (design 14 §7 item 13): painted on and taken back the same way land is.
    else if (c === 'village') { const on = tool.id === 'claim'; if (!!(w.claim[tile] & 1) !== on) send({ t: 'claim', tiles: [tx, ty], on }); }
  ...
  const TILE_TOOLS = new Set(['land', 'build', 'village']);
  return {
