...
function useSim(s, view) {
  ...
  ui.held = null; ui.follow = 0; ui.pokes.clear(); ui.outlines.clear(); ui.why.clear(); hidePicker(); because.clear();
  ...
  if (!paused && !broken) {
    ...
    try {
      while (budget >= step && n < MAX_STEPS_PER_FRAME) {
        ...
        try { because.step(sim.w); } catch (e) { report('because', e); } // before the sounds: it reads the events ring too
