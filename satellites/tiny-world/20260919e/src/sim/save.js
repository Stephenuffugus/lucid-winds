...
// Per thing in rec.things: type (index into tables.things), tx, ty, occ, food, cd, manned (NaN: never manned), handle.
const THING = 8;
...
export function snapshot(w) {
  ...
  w.structs.forEach((s, k) => {
    ...
    things.set([tIdx[s.type], s.tx, s.ty, s.occ, s.food, s.cd, s.manned === undefined ? NaN : s.manned, s.h], k * THING);
  });
...
export function restore(C, rec) {
  ...
  for (let k = 0; k < rec.things.length; k += THING) {
    ...
    const s = { type, def, tx, ty, occ: T[k + 3], food: T[k + 4], cd: T[k + 5], h: T[k + 7], manned: Number.isNaN(m) ? undefined : m };
