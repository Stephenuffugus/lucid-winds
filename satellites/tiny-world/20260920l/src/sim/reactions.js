...
function only(w, sc, e) {
  ...
  if (rule.tags) {
    const ki = w.C.kid[E.kind[e]];
    if ((w.C.tags.kind0[ki] & sc.onlyM0) !== sc.onlyM0 || (w.C.tags.kind1[ki] & sc.onlyM1) !== sc.onlyM1) return false;
  }
  return true;
}
...
export const VERBS = {
  ...
  perchNear(w, row, eff, x, y) {
    ...
    if (!best) {
      if (!w.R.flags.bounceAnyway || !eff.alt) return;
      for (let k = 0; k < R.tN; k++) { const e = R.targets[k]; if (E.inside[e] || E.perch[e]) continue; E.alt[e] = eff.alt; E.chute[e] = !!w.C.S[E.kind[e]].humanoid; E.goalKind[e] = G_NONE; }
      return;
    }
    for (let k = 0; k < R.tN; k++) {
      const e = R.targets[k];
...
export function fireTick(w, dt) {
  ...
  for (let k = 0; k < w.burnN; k++) {
    ...
    if (w.rng.float() < F.spread * (burning.catch || 1) * dt && !(w.R.flags.fireCap && w.fireSpent >= F.maxTilesPerIgnition)) {
      ...
      const nx = tx + (d === 0 ? 1 : d === 1 ? -1 : 0), ny = ty + (d === 2 ? 1 : d === 3 ? -1 : 0);
      if (inB(w, nx, ny) && ignite(w, ny * w.cols + nx, 0)) w.fireSpent++;
    }
    w.burnList[write++] = i;
  }
  w.burnN = write;
