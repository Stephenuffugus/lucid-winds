...
import { placeStruct, removeStruct, setTerrain, keepPaint, struct, markDirty, log, ref } from './world.js';
...
export const tileOp = (w, i) => ['tile', i, w.terr[i], w.eaten[i], w.age[i], w.hold[i]]; // (design 19 A1: and the tile's memory)
...
function apply(w, op) {
  ...
  switch (op[0]) {
    ...
    case 'tile': {
      ...
      if (w.terr[i] !== op[2]) { keepPaint(w, w.terr[i], op[2]); setTerrain(w, tx, ty, op[2]); } // (design 19 A2b: and the water baseline back)
      w.eaten[i] = op[3];
      if (op[3] > 0 && !w.eatenIn[i]) { w.eatenIn[i] = 1; w.eatenList[w.eatenN++] = i; } // its regrowth timer runs again
      if (op.length > 4) { w.age[i] = op[4]; w.hold[i] = op[5]; } // design 19 A1: the tile remembers what it remembered (after setTerrain, which zeroes its age)
      markDirty(w, i);
      return;
    }
    case 'thing': {
