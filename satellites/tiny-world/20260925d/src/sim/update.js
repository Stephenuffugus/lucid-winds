...
import { log, ref, markDirty, tileOf, struct, flies, swims, inWater, feedAt, capOf, reach, dist, isNight, removeStruct, onFlowers, setTerrain, speciesCapOf, placeStruct, pass, wake, favourite, SLEEP_DAWN, SLEEP_DUSK } from './world.js';
...
import { fireTick, reactEnter, reactClock, reactBeam, reactEat } from './reactions.js';
import { landTick } from './land.js';
import { villageTick, finishBuild , equipVillager, worn, fadePaths, birdAte } from './village.js';
...
export function update(w, dt) {
  ...
  w.time += dt;
  w.dt = dt;
  // Design 18: scratch that lives INSIDE a step starts the step empty. `w.perc.bad` is the threat the creature
  // that is DECIDING can see, and every behaviour reads it inside that creature's own think — except goCare
  // (below), which runs in the goal loop and so was reading whatever the last creature to think had left there,
  // from earlier in this step or from the one before. A world saved and loaded starts that scratch afresh, so
  // the two played on differently: found by tools/land-fuzz.mjs the day design 18's bedtimes changed who thinks
  // when. Cleared here, it is the same in both, and a carrier drops what it is doing on its own think tick, the
  // way every other decision in this game is made.
  if (R.flags.stepScratch) w.perc.bad = -1;
  ...
  // (What the light WAS is read before this step moved the clock: a world loaded on the very step before dusk
  // used to take the dusk it woke up in for the way things had always been, and never had that dusk at all.)
  if (w.rx.wasNight === undefined) w.rx.wasNight = ((w.time - dt) % w.daySec) / w.daySec > R.nightFrac;
  if (night !== w.rx.wasNight) {
    w.rx.wasNight = night;
    // Design 18 A5: whoever was sleeping UNTIL this light wakes with one small hop, which is the morning she
    // sees, and they wake BEFORE the clock rows run, so a dawn row finds them awake. Twice a day, so the walk
    // of the store is nothing; whole numbers, no allocation.
    if (R.flags.sleeps) { const want = night ? SLEEP_DUSK : SLEEP_DAWN; for (let k = 0; k < w.count; k++) { const e = w.order[k]; if (E.sleepT[e] === want) wake(w, e, true); } }
    reactClock(w, night ? 'dusk' : 'dawn');
    if (!night) { visitor(w); fadePaths(w); } // (design 18 E3: the worn ground fades a little every morning)
  }
  fireTick(w, dt);
  landTick(w); // design 19 A2: the living land, once a game minute, sliced (land.js)
  stormTick(w, dt);
  ...
  for (let k = 0; k < structs.length; k++) {
    ...
    if (s.def.hatch && R.flags.eggs && !s.cooked) { // design 17: an egg, on its own clock (saved and hashed as every thing's cd is)
      s.cd += dt;
      // No room for another yet: it waits, well short of the last few seconds in which an egg ROCKS (render.js), so
      // an egg that cannot hatch never looks as if it is about to. It asks again every few seconds.
      const hSec = hatchSecOf(w, s);
      if (s.cd >= hSec - R.react.rockSec && !roomToHatch(w, s)) {
        s.cd = hSec - R.react.rockSec - 4;
        if (!s.day) { s.day = 1; log(w, 'log.eggWaits', { kind: hatchKind(w, s) }); } // said once for each egg, so she is never left wondering (a thing's `day` is a grave's otherwise)
      }
      else if (s.cd >= hSec) { if (hatch(w, s)) k--; else s.cd = hSec - R.react.rockSec - 4; }
    }
    if (s.def.shoot) {
      s.cd -= dt;
...
export function creature(w, e, dt, night) {
  ...
  if (sp.diet === 'filter' || (sp.amph && sp.diet === 'herb')) { // design 19 G1.1: the ground feeds (feedAt), not deep water alone
    ...
    // bare (and feeds at half) until it grows back: the picture of eating, and the pond's own brake on a crowd.
    if (fd > 0 && R.flags.plantsWear && C.TERR[t].bare && w.eaten[k] <= 0 && (w.tick + E.id[e] * 131) % w.biteEvery === 0) bite(w, k);
  }
  ...
  // An errand runs out whether or not it got there; and one that was walking to BED is asleep the moment it
  // ends, which is where the one she named learns its favourite place (design 18 A14).
  if (E.errT[e] > 0) { E.errT[e] = Math.max(0, E.errT[e] - dt); if (E.errT[e] === 0 && E.sleepT[e] !== 0) favourite(w, e); }
  if (E.sleepT[e] > 0) { E.sleepT[e] -= dt; if (E.sleepT[e] <= 0) wake(w, e, false); } // design 18 A5 (a nap of so many seconds; the two that last until the light changes are negative and wake up there)
  if (E.bigT[e] !== 0) { // design 17: a giant, until it is not. It goes back with a puff.
    // Design 18 pack 2: a NEGATIVE bigT is big to look at and nothing else (the pufferfish), so it counts the
    // other way and goes back without the giant's own sentence: nobody was told it grew, so nobody is told it shrank.
    const plain = E.bigT[e] < 0;
    E.bigT[e] += plain ? dt : -dt;
    if (plain ? E.bigT[e] >= 0 : E.bigT[e] <= 0) { E.bigT[e] = 0; if (!E.inside[e]) { addFx(w, 'magic', E.x[e], E.y[e] - 4, R.fx.heart); emitAt(w, EVI.land, e); if (!plain) log(w, 'log.shrank', { a: ref(w, e) }); } }
  }
...
export function enteredSweep(w) {
  ...
  for (let k = 0; k < w.count; k++) {
    ...
    last[e] = i;
    if (w.R.flags.wornPaths && E.vill[e]) worn(w, i); // design 18 E3: one of the village's people wore this tile a little
    reactEnter(w, e, i);
    R.landing = false;
...
// Eggs wait while there are already plenty of that kind (rules.react.layCap, far under the species cap: sixty
// chickens in a six tile pen is soup). An egg never vanishes: it sits there, and hatches when there is room.
const hatchKind = (w, s) => (s.who > 0 && w.C.kinds[s.who - 1] ? w.C.kinds[s.who - 1] : s.def.hatch);
// Design 18 A12: a thing may grow into a THING instead of hatching into a creature — a sapling into a tree, so
// a world with squirrels in it slowly grows a wood. `hatch: { sec, into: { thing } }` instead of a kind name.
const growsInto = (w, s) => (w.R.flags.growsInto && s.def.hatch && typeof s.def.hatch === 'object' ? s.def.hatch.into.thing : null);
const hatchSecOf = (w, s) => (growsInto(w, s) ? s.def.hatch.sec : s.def.hatchSec);
const roomToHatch = (w, s) => growsInto(w, s) !== null || (w.count < w.R.breed.popCap && w.kindCount[w.C.kid[hatchKind(w, s)]] < Math.min(w.R.react.layCap, capOf(w, hatchKind(w, s))));
// A bite out of a water plant (design 19 G1.7): the same regrowth clock a mouthful of grass starts (goGraze).
function bite(w, k) {
  w.eaten[k] = w.R.graze.regrowSec / (w.C.TERR[w.terr[k]].regrowMul || 1);
  markDirty(w, k);
  if (!w.eatenIn[k]) { w.eatenIn[k] = 1; w.eatenList[w.eatenN++] = k; }
}
// Design 19 G1.5 (WATER-LIFE E5): a fish lays where it can put an egg, which is the shallows or the shore beside it
// (putThing refuses deep water), and a baby fish that hatched on the grass dried there. It is put on the nearest
// water it can live in within rules.ufo.snapTiles, the same snap a parachute uses to come down on dry land.
function toWater(w, b) {
  const E = w.E, T = w.T, n = w.R.ufo.snapTiles, cx = Math.floor(E.x[b] / T), cy = Math.floor(E.y[b] / T);
  let bx = 0, by = 0, bd = 1e9;
  for (let y = cy - n; y <= cy + n; y++) for (let x = cx - n; x <= cx + n; x++) {
    const dx = x - cx, dy = y - cy, d = dx * dx + dy * dy;
    if (d < bd && pass(w, b, x * T + 4, y * T + 5)) { bd = d; bx = x; by = y; }
  }
  if (bd < 1e9) setPos(w, b, bx * T + 4, by * T + 5);
}
// An egg hatches into a baby of whoever laid it (design 17). Returns true when it did (the thing is gone).
function hatch(w, s) {
  const C = w.C, kind = hatchKind(w, s);
  if (!roomToHatch(w, s)) return false;
  // A thing that GREW (design 18 A12): the sapling is gone and a tree stands where it was. Quietly: a wood
  // coming up over a quarter of an hour is something she finds, not something the news tells her about.
  const into = growsInto(w, s);
  if (into) {
    const tx = s.tx, ty = s.ty;
    removeStruct(w, tx, ty);
    placeStruct(w, into, tx, ty);
    addFx(w, 'magic', tx * w.T + 4, ty * w.T + 2, w.R.fx.heart);
    return true;
  }
  ...
  if (b < 0) return true;
  if (w.R.flags.hatchSnap && C.S[kind].water && !pass(w, b, x, y)) toWater(w, b);
  ...
  story(w, STI.birth, x, y, b, -1, -1, C.iconOf['thing:egg'], NO, kindIcon(w, b), w.lastLog);
  return true;
}
...
// Design 18 E2: does this creature count as a bird at the village's field? (Two ANDs against the tag mask the world
// worked out once, content.js kind0/kind1.)
const isBird = (w, e) => { const ki = w.C.kid[w.E.kind[e]]; return ((w.C.tags.kind0[ki] & w.rxBird0) | (w.C.tags.kind1[ki] & w.rxBird1)) !== 0; };
function goGraze(w, e) { // goalX, goalY: the tile
  ...
  if (step(w, e, MV_GRAZE)) {
    ...
    if (gt.eatTo !== undefined && R.flags.crops) { // design 15 C1: a ripe field is picked, and starts again
      ...
      // Design 18 E2: a BIRD at the village's field is counted (before the row: a row may move the eater, and the
      // field it took the mouthful from is where the scarecrow belongs).
      if (R.flags.villageScarecrow && isBird(w, e)) birdAte(w, i);
      reactEat(w, e, i, null); // design 18 A8 (last: the row may move the eater)
      return;
    }
    let ate = false;
    ...
    E.goalKind[e] = 0; E.think[e] = R.ai.eatThink;
    if (ate) reactEat(w, e, i, null); // design 18 A8 (after the goal is cleared: the row may send it somewhere)
  }
}
...
function goBush(w, e) {
  ...
  if (hyp(s.tx * T + 4 - E.x[e], s.ty * T + 5 - E.y[e]) < R.food.reach || step(w, e, MV_FOOD)) {
    ...
    emitAt(w, EVI.eat, e);
    // Design 18 E2: and the field the village planted for itself is a thing, not ground (rules.village.fieldThings)
    if (R.flags.villageScarecrow && isBird(w, e) && R.village.fieldThings.indexOf(s.type) >= 0) birdAte(w, s.ty * w.cols + s.tx);
    reactEat(w, e, s.ty * w.cols + s.tx, s); // design 18 A8: a serving from a food thing
  }
}
...
function visitor(w) {
  ...
  // A stranger needs a bed as a baby does (design 17): nobody walks in over the hill to a place with no room.
  if (w.R.flags.beds) { let beds = 0; for (let k = 0; k < w.structs.length; k++) { const d = w.structs[k].def; beds += d.beds === undefined ? d.home || 0 : d.beds; } if (beds > 0 && folk >= beds) return; }
  sx /= folk; sy /= folk;
...
function goMate(w, e) {
  ...
  if (dist(w, e, o) < R.breed.contact) {
    ...
    const full = R.flags.capAtBirth && (w.count >= R.breed.popCap || w.kindCount[w.C.kid[kind]] >= capOf(w, kind));
