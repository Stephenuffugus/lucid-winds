...
export function createTray({ data, status, getSim, guard = (fn) => fn, onTool = () => {} }) {
  ...
  const hint = (a, b) => str('hint.' + a + '.' + b) || str('hint.' + a) || str('hint.' + b);
  ...
  // `extra`: a tile borrowed from another tab so a whole idea can live in one place (the Village flag stands in
  // the Village tab, first, but keeps cat 'build' so it is still the ordinary build tool and ★ and Recent see
  // one key, not two). Borrowed tiles come first.
  const CATS = data.tray.tabs.map((t) => ({
    id: t.id, name: str('tab.' + t.id), shelf: t.shelf || null,
    items: [
      ...(t.extra || []).map((x) => ({ ...src[x.source](x.id), cat: x.tab })),
      ...t.items.map((k) => ({ ...src[t.source](k), cat: t.id })),
    ],
  }));
  ...
  function build() {
    ...
    for (const it of items) {
      ...
      itemsEl.appendChild(mkItem('item' + (tool.cat === it.cat && tool.id === it.id ? ' on' : '') + (fav.has(key) ? ' fav' : ''), it.icon(), name, guard(() => {
        ...
        // Painting permission with no flag standing does nothing at all, forever, so say what is missing.
        const sim2 = getSim();
        status.post(it.cat === 'village' && sim2 && sim2.w && !sim2.w.vg.flag ? str('hint.village.noflag') : hint(it.cat, it.id));
