...
export function createRenderer(cv, art, cam) {
  ...
  function drawChunk(w, ci, p) {
    ...
    for (let ty = ty0; ty < ty0 + th; ty++) for (let tx = tx0; tx < tx0 + tw; tx++) {
      ...
      // Design 19 G1.7: a water plant eaten down draws its bare look (`bare` in terrain.json: its own colours, and
      // the pads or the fronds gone) until it grows back, the way the lawn does under sheep.
      const bare = w.eaten[i] > 0 && def.bare ? def.bare : null;
      const cols = t === w.C.tid.grass && w.eaten[i] > 0 ? art.eatenGrass : bare ? bare.cols || def.cols : def.cols, ph = def.anim ? p : 0, rock = t === w.C.tid.rock;
      ...
      const blade = bare ? bare.blades : def.blades, lean = blade && (ph & 1) ? 1 : 0;
  ...
  function drawEnt(w, e, selected, k, time, lag, hop = 0, outline = false, at = null) {
    ...
    // A bug (design 18 pack 1's one tag) is drawn at HALF size, whole pixels: a butterfly as big as a sheep and a
    // bee as big as a person were the dawn meadow's loudest thing (F3, QUESTIONS Q48). Every other creature keeps
    // the size it has always had.
    const S = w.C.S, sp = S[kind], G = E.gear[e], ref = spriteOf(w, e, kind, sp, G), sz = sp.tags && sp.tags.indexOf('bug') >= 0 ? 4 : E.baby[e] ? 6 : 8;
    ...
    const gz = E.bigT[e] !== 0 ? 2 : 1; // a giant is twice everything (design 17): its clip, its bars, its ring (and design 18's plain big, which is drawn the same and tagged nothing)
    ...
    const bigT = Math.abs(E.bigT[e]), big = bigT > 0 ? (bigT < 0.5 || bigT > w.R.react.giantSec - 0.4 ? 1.5 : 2) : 1;
    // Design 18 pack 5: a creature asleep in a DISGUISE is not mirrored. A crate has no left and no right, and
    // the crate sprite is not symmetric, so a mimic facing left was a crate with its dark end on the wrong side
    // — measured at 32 of its 64 pixels, against 0 facing right (dev/look-18f.mjs). Half of them were wrong
    // half the time and no gate could see it, because the swap is the renderer's. A recolour (`sleepOver`, the
    // gargoyle turning to stone) still faces the way it was facing: it is still itself, only grey.
    const hidden = sp.sleepSpr && E.sleepT[e] !== 0;
    ctx.scale((hidden ? 1 : E.face[e]) * big, big);
    ...
    // Design 18 A5: asleep. One small z rises over its head and fades, on its own clock (its id), so a barn
    // full of sleeping animals is a drift and not one flashing row. Drawn from the creature's own state, never
    // from an effect record: the sim stays out of it, and nothing here is in a hash or a save.
    // ...and never over a creature that has a look for being asleep: a crate with a z rising off it is not a
    // crate, and the whole of the mimic is that she cannot tell (design 18 pack 5).
    if (E.sleepT[e] !== 0 && !(E.errT[e] > 0) && !E.inside[e] && !sp.sleepSpr && !sp.sleepOver) {
      // Two small z's rising, a one pixel shadow and no outline (F3, QUESTIONS Q48): the old 3x5 Z in its outline
      // was a "2" on a dark badge at both widths, and a sleeping sheep read as a sheep with a number.
      const zt = ((time * 0.5 + E.id[e] * 0.37) % 1), zy = Y - 11 - Math.round(zt * 3);
      ctx.globalAlpha = 0.35 + 0.65 * (1 - zt);
      zee(X + 3, zy);
      ctx.globalAlpha *= 0.7;
      zee(X + 6, zy - 4);
      ctx.globalAlpha = 1;
  ...
  // so the swap costs one comparison a frame and not a string.
  ...
  function frame(w, selected, rand, k = 1, ui = null) {
    ...
    for (const it of list) {
      if (it.e !== undefined) { if (far) drawDot(w, it.e, k, z); else if (ui) drawMarked(w, it.e, sel, k, time, lag, ui); else drawEnt(w, it.e, sel, k, time, lag); } // slot 0 is a creature too
        ...
        else if (typeof d.hatch === 'string' && s.cd > d.hatchSec - w.R.react.rockSec && Math.floor(time * 5) % 3 === 0) x += 1; // (an EGG about to hatch rocks; a sapling growing into a tree does not: design 18 A12)
    ...
    for (let j = 0; j < w.fxN; j++) {
      ...
      if (f.type === 'arrow') { ctx.strokeStyle = f.col || '#f4f4f0'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(f.x, f.y); ctx.lineTo(f.x2, f.y2); ctx.stroke(); }
        ...
        // Wind (design 17): twelve short streaks flying outward from the tap and thinning as they go. No circle and
        // no block: a puff of air is seen by what it carries, and the stock explosion read as a pale square.
        const life = w.C.P.gust ? w.C.P.gust.sec : 0.5, g = Math.max(0, Math.min(1, 1 - age(f) / life));
        ctx.globalAlpha = 0.85 * (1 - g); ctx.fillStyle = f.col || '#dff3f8';
        for (let k = 0; k < 12; k++) {
          const a = (k / 12) * Math.PI * 2 + (k % 2) * 0.2, d = f.r * (0.25 + 0.75 * g) * (k % 3 === 0 ? 0.8 : 1), len = k % 2 ? 5 : 3;
          const cx = f.x + Math.cos(a) * d, cy = f.y + Math.sin(a) * d * 0.8;
          for (let q = 0; q < len; q++) ctx.fillRect(Math.round(cx - Math.cos(a) * q), Math.round(cy - Math.sin(a) * q * 0.8), 1, 1);
        }
        ctx.globalAlpha = 1;
      ...
      // giant, the moon cat, the rabbit out of the top hat, Bless making a giant small, the snow golem, the troll
      // waking, the wizard's flowers) and NOTHING WAS DRAWN FOR IT; `star` was the same. A row that shows a child
      // nothing does not exist. `magic` is three twinkles rising round the spot; `star` is the star sprite the
      // game already has, rising like the heart.
      else if (f.type === 'magic') {
        // Each twinkle is a dark cross with a white one inside it and a pink heart, because a white sparkle with
        // no outline is gone on snow, on sand and on her ice bridge (looked at, 21 Sep).
        const a = age(f), up = Math.round(Math.max(0, 1 - a) * 4);
        ctx.globalAlpha = Math.min(1, a / 0.4); // solid while it lasts, gone over its last four tenths
        for (let k = 0; k < 3; k++) {
          const x = Math.round(f.x) + MAGIC_DX[k], y = Math.round(f.y) + MAGIC_DY[k] - up;
          if (k) { ctx.fillStyle = PAL.p; ctx.fillRect(x, y - 1, 1, 3); ctx.fillRect(x - 1, y, 3, 1); continue; } // two small pink sparks
          ctx.fillStyle = art.glyphs.outline; // and the big one outlined, so it reads on snow and on sand
          ctx.fillRect(x, y - 3, 1, 7); ctx.fillRect(x - 3, y, 7, 1);
          ctx.fillStyle = PAL.w;
          ctx.fillRect(x, y - 2, 1, 5); ctx.fillRect(x - 2, y, 5, 1);
          ctx.fillStyle = PAL.p; ctx.fillRect(x, y, 1, 1);
        }
        ctx.globalAlpha = 1;
      }
      // The star sprite the game already has, at its own size (squeezed to the heart's six it lost its points),
      // rising the way the heart rises, and with a dark copy drawn round it so it reads on sand and on snow.
      else if (f.type === 'star') {
        starRef = starRef || atlasRef('star');
        starDarkRef = starDarkRef || atlasRef('star', { y: art.glyphs.outline });
        ctx.globalAlpha = Math.min(1, age(f) / 0.4);
  ...
  function drawMarked(w, e, sel, k, time, lag, ui) {
    ...
    // Design 18 A16: the hello a saved world says when it opens. One heart over the one she named, drawn exactly
    // as every other heart in the game is (the same 6 px picture, rising and fading), and drawn from the view's
    // own state: nothing of this is in the world. LOOKED AT on 22 Sep, which is how it came to be drawn this way
    // and not another: the first cut used the raw sprite, which is BROWN, and hung it twenty px up, where it read
    // as a thing lying in the field rather than as somebody saying hello.
    const wel = ui.welcome;
    if (wel && wel.h === h) {
      const age = ui.now - wel.t0, ms = (U.welcome && U.welcome.showMs) || 1200;
      if (age >= 0 && age < ms) {
        const E = w.E, t = age / ms;
        heartRef = heartRef || atlasRef('heart');
        ctx.globalAlpha = Math.max(0, 1 - t * t); // (it holds, then goes: a linear fade reads as broken)
        ctx.drawImage(ATL, heartRef.sx, heartRef.sy, 8, 8,
          Math.round(lerpPos(E.px[e], E.x[e], k)) - 3, Math.round(lerpPos(E.py[e], E.y[e], k)) - 11 - hop - Math.round(t * 4), 6, 6);
        ctx.globalAlpha = 1;
      }
    }
  }
  ...
  // The sleep z (art.glyphs.z): the shadow first, one pixel down and right, then the letter.
  function zee(x, y) {
    const g = art.glyphs.z, x0 = Math.round(x) - 1, y0 = Math.round(y) - 1;
    ctx.fillStyle = art.glyphs.outline;
    g.px.forEach((row, j) => { for (let i = 0; i < row.length; i++) if (row[i] === '#') ctx.fillRect(x0 + i + 1, y0 + j + 1, 1, 1); });
    ctx.fillStyle = g.col;
