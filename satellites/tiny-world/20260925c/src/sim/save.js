...
export const SAVE_V = 13;
export const SAVE_KIND = 'tiny-world';
// World-wide numbers, in this order in rec.num (booleans as 0/1).
const NUM = ['tick', 'time', 'rng', 'rainT', 'shake', 'nextId', 'thingSerial', 'topo', 'logSeq', 'safe', 'fullSaid', 'eatenN', 'stamp', 'fireSpent', 'landN', 'keepWater'];
...
export function snapshot(w) {
  ...
  num[NI.landN] = w.landN; num[NI.keepWater] = w.keepWater; // design 19 A1: the land's changes this minute (its cap), and the water baseline (A2b)
  ...
  const store = {
    ...
    // What each creature WAS the first time something changed it (design 18 A13), '' for everybody else. A plain
    // array like the name beside it, so it rides here and not in the typed block.
    was: E.was.slice(0, n).map((v) => v || ''),
    f,
  };
  const cache = {};
  ...
  return {
    ...
    // How worn the ground is (design 18 A15, written by phase E's paths), or null in a world nobody has walked a
    // path into, which keeps those saves the size they were.
    wear: w.wearN ? w.wear.slice() : null,
    // The land's memory (design 19 A1): how long each tile has been its kind, and how long she keeps what she
    // painted; null while every tile is zero, which is every world from before it.
    age: anyOf(w.age) ? w.age.slice() : null, hold: anyOf(w.hold) ? w.hold.slice() : null,
...
export const MIGRATIONS = {
  ...
  // v12 (design 18 A15): ONE bump for the whole design, with every field any of its tickets will ever write, so
  // that no later ticket changes the shape of a creature again (the law: a field added later deoptimizes every
  // reader). sleepT zero (nobody in a v11 world was asleep: design 18 is what put anybody to sleep), `was` empty
  // (nothing had been changed into anything, so Bless has nothing to turn back), favTile -1 and favN zero
  // (nobody had a place of their own yet), and no worn ground. A v11 world plays on exactly as it did.
  // v13 (design 19 A1): ONE bump for the whole design (the law of v12). The tile memory (age, hold) null: an old
  // world wakes up with every tile the kind it has been since the moment it was opened, so its rules wait their
  // days and nothing has changed by itself yet. landN and keepWater 0 (appended to the numbers, as fireSpent was).
  // bubbleT zero: nobody in a v12 world was ever in a rescue bubble.
  12: (r) => {
    if (!(r.num instanceof Float64Array)) throw saveError('damaged', 'numbers');
    const n = r.store ? r.store.nSlots : 0, num = new Float64Array(r.num.length + 2);
    num.set(r.num);
    return { ...r, v: 13, num, age: null, hold: null,
      store: r.store && r.store.f ? { ...r.store, f: { ...r.store.f, bubbleT: new Float64Array(n) } } : r.store };
  },
  11: (r) => {
    const n = r.store ? r.store.nSlots : 0;
    return { ...r, v: 12, wear: null,
      store: r.store && r.store.f ? { ...r.store, was: new Array(n).fill(''),
        f: { ...r.store.f, sleepT: new Float64Array(n), favTile: new Int32Array(n).fill(-1), favN: new Uint8Array(n) } } : r.store };
  },
...
export function check(C, rec) {
  ...
  if (rec.wear !== null && rec.wear !== undefined && !isT(rec.wear, Uint8Array, nT)) return 'worn ground'; // (design 18 A15: null in a world with no paths worn into it)
  for (const k of ['age', 'hold']) if (rec[k] !== null && rec[k] !== undefined && !isT(rec[k], Uint8Array, nT)) return 'land memory'; // (design 19 A1)
  ...
  // The slot lists come first, because the walk just below reads `order` and a `for...of` over a damaged one
  // THROWS instead of refusing the save cleanly. Found by the damaged-save sweep on 22 Sep, the second time
  // this has happened: design 15 moved the bytes and the sweep found the off-the-map hole, design 18 moved them
  // again and it found this. A check that reads a field before the check that proves it is a field is the bug.
  ...
  for (const a of ['name', 'over', 'gear', 'was']) if (!Array.isArray(st[a]) || st[a].length !== n) return `creatures (${a})`;
  if (st.name.some((s) => s != null && typeof s !== 'string')) return 'creatures (name)';
  if (st.was.some((s) => typeof s !== 'string')) return 'creatures (was)';
...
export function restore(C, rec) {
  ...
  if (rec.wear) { w.wear.set(rec.wear); for (let i = 0; i < w.nTiles; i++) if (w.wear[i]) w.wearN++; } // (design 18 A15)
  if (rec.age) w.age.set(rec.age); if (rec.hold) w.hold.set(rec.hold); // (design 19 A1)
  ...
  w.landN = num[NI.landN] || 0; w.keepWater = num[NI.keepWater] || 0; // (design 19 A1)
...
// Any byte set? (design 19 A1: a tile memory that is all zeros is saved as null.)
function anyOf(a) { for (let i = 0; i < a.length; i++) if (a[i]) return true; return false; }
...
function restoreStore(w, st, kinds) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    E.was.push(typeof st.was[i] === 'string' ? st.was[i] : ''); // design 18 A13: what it was before something changed it
