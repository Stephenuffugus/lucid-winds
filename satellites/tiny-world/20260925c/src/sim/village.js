...
import { inB, placeStruct, removeStruct, struct, markDirty, log, setTerrain } from './world.js';
...
function style(w) {
  ...
  for (let a = 0; a < n; a++) {
    ...
    pickUp(w, 'item:' + vg.style, E.x[e], E.y[e]);
    reactEquip(w, e, vg.style);
    return; // ONE of them each time
  }
}
// The village does not make its style out of nothing while its own lie on the grass: the nearest loose one is
// picked back up first (23 Sep, found LOOKING at day 12: the first world's UFO beam counts as a blow, knocked_off
// dropped a party hat every time it reached for a villager, and the village handed out a fresh one each time, so
// the meadow filled with hats). Only when none is lying anywhere is a new one made. Ties go to the one put down first.
function pickUp(w, type, x, y) {
  let best = null, bd = Infinity;
  for (const s of w.structs) {
    if (s.type !== type) continue;
    const dx = s.tx * w.T + 4 - x, dy = s.ty * w.T + 4 - y, d = dx * dx + dy * dy;
    if (d < bd) { bd = d; best = s; }
  }
  if (best) removeStruct(w, best.tx, best.ty);
}
