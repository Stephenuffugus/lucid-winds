...
export function worldHash(w) {
  ...
  // The worn ground (design 18 E3), only in a world with any: its turning to dirt shows in the terrain above, but
  // how near the next tile is to turning does not, and two worlds that differ only in that play on differently.
  if (w.R.flags.wornPaths && w.wearN) { h.str('wear').num(w.wearN); for (let i = 0; i < w.nTiles; i++) if (w.wear[i]) h.num(i).num(w.wear[i]); }
  // The land's memory (design 19 A1), only where a byte is set: two worlds that differ only in a tile's age play on
  // differently a minute later, so the hash must see it or no save gate can catch a lost byte. NOT behind the switch:
  // with flags.land off nothing may write a byte (every one stays zero, and older worlds keep their hashes), so a
  // byte that leaks past the switch MOVES every recorded world and `proof 19` says so (watched: an age count outside
  // the switch went unseen while this walk was behind it).
  for (let i = 0; i < w.nTiles; i++) if (w.age[i] || w.hold[i]) h.num(i).num(w.age[i]).num(w.hold[i]);
  if (w.landN || w.keepWater) h.str('land').num(w.landN).num(w.keepWater);
  ...
  if (w.R.flags.hashVillage && (w.claimN || w.vg.flag)) {
    ...
    // Design 18 phase E: what each village ticket remembers, behind its own switch and only once it is off its
    // default, so a world recorded without it hashes as it did (A15's law: a ticket that writes a field it does
    // not hash passes every gate, and is wrong the first time two worlds differ only in that field).
    if (w.R.flags.learnsFromScares && vg.scareT) h.str('scare').num(vg.scareT);
    if (w.R.flags.villageCopies && vg.styleT) h.str('style').str(vg.style).num(vg.styleT);
    if (w.R.flags.villageScarecrow && vg.birds) h.str('birds').num(vg.birds).num(vg.scared);
  }
  ...
  for (let k = 0; k < w.count; k++) {
    ...
    if (E.bigT[e] !== 0) h.str('big').num(E.bigT[e]); // (design 17) nobody in an older world was ever a giant, and (design 18) a negative one is big to look at only
    ...
    if (E.bubbleT[e] !== 0) h.str('bubble').num(E.bubbleT[e]); // (design 19 A7) nobody in an older world was ever in a bubble
