...
export function createAudio(data, opts = {}) {
  ...
  try { const p = JSON.parse(localStorage.getItem(MUSIC) || '{}'); if (p && typeof p === 'object') { progress.secs = Math.max(0, Number(p.secs) || 0); progress.unlocked = Array.isArray(p.unlocked) ? p.unlocked.filter((x) => typeof x === 'string') : []; progress.off = Array.isArray(p.off) ? p.off.filter((x) => typeof x === 'string') : []; } } catch (e) { /* private mode: this session only */ }
  let progressDirty = 0, playSecsAt = 0, fresh = [];
  function saveProgress() { progressDirty = 0; try { localStorage.setItem(MUSIC, JSON.stringify(progress)); } catch (e) { /* private mode */ } }
  ...
  function makeMusic() {
    ...
    // the synth loop under it all: what is on screen picks it, as it always did, but it is heard ONLY when no song
    // file is playing (offline, or nothing in the loop)
    function set(want) {
      if (want === name) return;
    ...
    // THE PLAYER (23 Sep 2026): the loop of unlocked songs she has not switched off, each played WHOLE, one after
    // another, wrapping; one song alone repeats. Nothing on screen changes the song. A file that fails (offline, a
    // wrong path) moves on, but a whole turn of failures stops there (the synth plays), until she touches the menu.
    let current = null, failed = 0, held = false, pv = null, pvSrc = null, previewId = null;
    function playFile(id) {
      drop();
      const song = SONGS.find((s) => s.id === id);
      if (!song) return;
      try {
        const mine = new Audio(new URL(song.url, document.baseURI).href);
        mine.loop = loopIds(SONGS, progress).length === 1; mine.crossOrigin = 'anonymous';
        el = mine; elSrc = ctx.createMediaElementSource(mine); elSrc.connect(out);
        current = id;
        mine.onended = () => { if (el === mine) { failed = 0; advance(); } };
        const fail = () => { if (el !== mine) return; drop(); failed++; if (failed <= loopIds(SONGS, progress).length) advance(); };
        mine.onerror = fail;
        mine.play().catch(fail);
      } catch (e) { drop(); }
    }
    function advance() { const id = nextId(SONGS, progress, current); if (id) playFile(id); else { drop(); current = null; } }
    function start() { if (el) return; failed = 0; const id = nextId(SONGS, progress, current); if (id) playFile(id); }
    function drop() {
      if (el) { el.onended = null; el.onerror = null; el.pause(); el = null; }
      if (elSrc) { elSrc.disconnect(); elSrc = null; }
      held = false;
    }
    // the menu's switches: a song in or out of the loop, and the player follows
    function songOn(id, on) {
      stopPreview();
      failed = 0;
      const play = toggleSong(SONGS, progress, id, on, current);
      saveProgress();
      if (!play) { drop(); current = null; return; }
      if (play !== current || !el) playFile(play);
      else if (el) el.loop = loopIds(SONGS, progress).length === 1;
    }
    // LISTEN in the menu: a song she has not unlocked, over the loop, which waits; stopPreview puts it back
    function preview(id) {
      stopPreview();
      const song = SONGS.find((s) => s.id === id);
      if (!song) return;
      if (el && !el.paused) { el.pause(); held = true; }
      try {
        const mine = new Audio(new URL(song.url, document.baseURI).href);
        mine.loop = true; mine.crossOrigin = 'anonymous';
        pv = mine; pvSrc = ctx.createMediaElementSource(mine); pvSrc.connect(out); previewId = id;
        mine.onerror = () => { if (pv === mine) stopPreview(); };
        mine.play().catch(() => { if (pv === mine) stopPreview(); });
      } catch (e) { stopPreview(); }
    }
    function stopPreview() {
      if (pv) { pv.onerror = null; pv.pause(); pv = null; }
      if (pvSrc) { pvSrc.disconnect(); pvSrc = null; }
      previewId = null;
    ...
    return { set, tick, start, songOn, preview, stopPreview, get name() { return name; }, get current() { return current; }, get previewId() { return previewId; }, get el() { return el; }, get pv() { return pv; } };
  }
  ...
  function frame(w, c, size, paused, night) {
    ...
    // the play clock (23 Sep): seconds with the game running and the tab in view; a song unlocks when it reaches its minutes
    if (!paused && !document.hidden) {
      if (playSecsAt) progress.secs += Math.min(1, (now - playSecsAt) / 1000);
      playSecsAt = now;
      if ((progressDirty += 1) >= 600) saveProgress();
      const nu = newlyUnlocked(SONGS, progress);
      if (nu.length) { progress.unlocked = unlockedIds(SONGS, progress); fresh.push(...nu); saveProgress(); if (music && !music.el) music.start(); }
    } else playSecsAt = 0;
    if (now - lastLook > 250) { // what is on screen: creatures, fires, tornadoes, UFOs beaming, foes, the land
      ...
      if (danger) track = 'danger';
        ...
        // After dark, and nothing dangerous in view: the night's own songs, when it has any (23 Sep).
        const nf = A.music.files.night;
        if (night && nf && nf.length) track = 'night';
      }
      music.set(track);
    }
    music.tick(night);
  ...
  function command(c, w) {
    ...
    else if (c.t === 'pokeThing') { // a poked thing clicks the way it is made (design 17)
      const s = c.tx >= 0 && c.ty >= 0 && c.tx < w.cols && c.ty < w.rows ? w.grid[c.ty * w.cols + c.tx] : null;
      if (s) { const m = A.thingMaterial[s.type] || 'cloth'; sfx('mat:' + m, c.tx * T + 4, c.ty * T + 4, A.materials[m]); haptic('place'); }
    }
  ...
  function songs() {
    const un = new Set(unlockedIds(SONGS, progress)), off = new Set(progress.off || []);
    return SONGS.map((s) => ({ id: s.id, title: s.title, unlocked: un.has(s.id), on: un.has(s.id) && !off.has(s.id), playing: !!music && music.current === s.id && !!music.el, previewing: !!music && music.previewId === s.id, minutesLeft: minutesLeft(s, progress) }));
  }
  function songOn(id, on) { unlock(); if (music) music.songOn(id, on); else { toggleSong(SONGS, progress, id, on, null); saveProgress(); } }
