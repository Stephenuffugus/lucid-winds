...
export function update(w, dt) {
  ...
  for (let k = 0; k < structs.length; k++) {
    ...
    if (s.def.hatch && R.flags.eggs && !s.cooked) { // design 17: an egg, on its own clock (saved and hashed as every thing's cd is)
      ...
      if (s.cd >= s.def.hatchSec - R.react.rockSec && !roomToHatch(w, s)) {
        s.cd = s.def.hatchSec - R.react.rockSec - 4;
      ...
      else if (s.cd >= s.def.hatchSec) { if (hatch(w, s)) k--; else s.cd = s.def.hatchSec - R.react.rockSec - 4; }
    }
    if (s.def.shoot) {
      s.cd -= dt;
...
export function creature(w, e, dt, night) {
  ...
  if (E.errT[e] > 0) E.errT[e] = Math.max(0, E.errT[e] - dt); // an errand runs out whether or not it got there
...
const roomToHatch = (w, s) => w.count < w.R.breed.popCap && w.kindCount[w.C.kid[hatchKind(w, s)]] < Math.min(w.R.react.layCap, speciesCapOf(w));
