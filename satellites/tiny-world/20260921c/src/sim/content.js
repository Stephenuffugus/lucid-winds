...
export function compileContent(d) {
  // Design 17 changed a few records that already existed (a hen lays, a dog takes toys). Those fields ride in the
  // record's `v17` and are merged here, so with flags.content17 off the record is exactly the one that shipped.
  if (d.rules.flags.content17 !== false) for (const c of Object.values(d.creatures)) if (c && c.v17) Object.assign(c, c.v17);
