...
import { reactEquip, reactPlaced, reactEnter } from './reactions.js';
...
function putItem(w, c, item, tx, ty) {
  ...
  for (let r = 0; r <= 2; r++) for (let dy = -r; dy <= r; dy++) for (let dx = -r; dx <= r; dx++) {
    ...
    reactPlaced(w, put); // design 14 §5: a hat let go on a snowman
    return put;
  }
  return null;
}
...
export function command(w, c) {
  ...
  switch (c.t) {
    ...
    case 'erase': { // tile: false once this drag has already cleared this tile
      ...
      if (!c.tile) return;
      const i = ty * w.cols + tx;
      if (w.grid[i]) { note(w, c, thingOp(w.grid[i])); removeStruct(w, tx, ty); noBuild(w, i); } // (14 §1 rule 8: the village never puts it back)
    ...
    case 'give': {
      ...
      if (wp) { G.weapon = c.item; log(w, 'log.gotItem', { a: ref(w, e), item: wp.name }); }
        ...
        const g = w.C.GEAR[c.item];
        G[g.slot] = g.id;
        rebuildGear(w, e);
        log(w, 'log.gotItem', { a: ref(w, e), item: g.name });
      }
