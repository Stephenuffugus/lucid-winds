...
import { reactPower } from './reactions.js';
...
export function usePower(w, id, x, y) {
  ...
  if (id === 'bolt') {
    ...
    if (e >= 0) { E.bless[e] = P.sec; log(w, 'log.blessed', { a: ref(w, e) }); }
      ...
      if (t === tid.rock && !w.grid[i]) { placeStruct(w, 'crystal', tx, ty); addFx(w, 'block', x, y, F.block); log(w, 'log.crystal'); }
    ...
    for (let n = 0; n < P.count; n++) {
      ...
      const ground = w.C.TERR[w.terr[i]].id, what = S[ground];
      if (!what) continue;
