...
function grown(w, type, tx, ty) {
  placeStruct(w, type, tx, ty); // (the callers have already checked the tile is on the map)
...
export function usePower(w, id, x, y) {
  ...
  if (id === 'gust' && !w.R.flags.gust) return; // (design 17: the switch that takes the wind back out)
  ...
  if (id === 'bolt') {
    ...
    // Design 15 C2, built in design 17: a puff of wind from the tap, outward. Everything it DOES is a row (hats fly
    // off and land downwind, everybody near hops back, wind over a pond rains fish, an umbrella carries its holder
    // away): the power itself is only the ring she sees, so a gust over empty grass is still a gust.
    const f = addFx(w, 'gust', x, y, P.sec);
    f.r = P.radius; f.col = P.fxColor;
  } else if (id === 'seeds') {
