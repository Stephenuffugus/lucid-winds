...
export function createAudio(data, opts = {}) {
  ...
  function makeMusic() {
    ...
    // Stephen's songs for a place (23 Sep: his eight, free, by mood). A slot holds one song, which loops, or a list,
    // played in turn; coming back to a place plays its NEXT song, not the first again. Whatever fails to load
    // (the local server and the GitHub preview do not carry the music) leaves the synth playing, as it always did.
    const turn = {};
    function play(want) {
      const f = M.files[want], list = Array.isArray(f) ? f : f ? [f] : [];
      if (!list.length) return;
      const i = (turn[want] || 0) % list.length;
      turn[want] = i + 1;
      try {
        const mine = new Audio(new URL(list[i], document.baseURI).href);
        mine.loop = list.length === 1; mine.crossOrigin = 'anonymous';
        el = mine; elSrc = ctx.createMediaElementSource(mine); elSrc.connect(out);
        mine.onended = () => { if (el === mine) { drop(); play(want); } }; // the next song of the same place
        mine.onerror = () => { if (el === mine) drop(); };
        mine.play().catch(() => { if (el === mine) drop(); });
      } catch (e) { drop(); }
    }
    function drop() {
