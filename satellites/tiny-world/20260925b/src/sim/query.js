...
export function findGrass(w, e, cropsOnly) {
  ...
  for (let y = cy - n; y <= cy + n; y++) for (let x = cx - n; x <= cx + n; x++) {
    ...
    if (t.flowers) d *= near;
    if (d < bd && !(w.R.flags.penGrazes && fenced(w, cx, cy, x, y))) { bd = d; best = i; }
  }
  return best;
}
// Design 19 G2.4 (the pen bug's other half): is there a fence or a wall on the straight line between two tiles? A
// penned sheep whose own grass was eaten picked grass on the far side of the fence and starved walking at it. Asked
// only of a tile that would beat the best so far, so it costs a few reads a search.
export function fenced(w, x0, y0, x1, y1) {
  const n = Math.max(Math.abs(x1 - x0), Math.abs(y1 - y0));
  for (let k = 1; k < n; k++) {
    const x = Math.round(x0 + ((x1 - x0) * k) / n), y = Math.round(y0 + ((y1 - y0) * k) / n), s = w.grid[y * w.cols + x];
    if (s && s.def.block) return true;
  }
  return false;
}
