...
// opts: { cols, rows, seed } for a new world, or { record } for a saved one (save.js; migrate it first).
export function createSim(data, opts) {
  const C = compileContent(data);
  const w = opts.record ? restore(C, opts.record) : createWorld(C, opts);
  const dt = C.rules.tickSec;
  let edits = 0; // commands so far: the game saves soon after a change even while paused
  return {
    w,
    C,
    ...
    command(c) {
      ...
      edits++;
      command(w, c);
      w.answered = w.logSeq; // whatever it said answers the player at once (ui/status.js)
    },
    get edits() { return edits; },
    // The whole world as one record (save.js), taken between steps.
    save: () => snapshot(w),
