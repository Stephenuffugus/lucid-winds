...
import { log, ref, markDirty, tileOf, struct, flies, swims, inWater, reach, dist, isNight, removeStruct, onFlowers, setTerrain } from './world.js';
import { spawn, ent, compact, HELD, G_MOVE, G_ATTACK, G_ABDUCT, G_GRAZE, G_BUSH, G_HOUSE, G_MATE, G_JOIN, G_FETCH, G_STORE, G_BUILD, G_TAKE, G_CARE, G_WATER, G_DOUSE } from './ents.js';
...
import { reactPlaced } from './reactions.js';
...
export function update(w, dt) {
  ...
  if (night !== w.rx.wasNight) { w.rx.wasNight = night; reactClock(w, night ? 'dusk' : 'dawn'); }
  fireTick(w, dt);
...
export function creature(w, e, dt, night) {
  ...
  if (E.hunger[e] >= N.hungerMax) { E.hp[e] -= allowed(w, e, SRC.need, N.starveDps * dt); if (E.hp[e] <= 0) { die(w, e, 'starved'); return; } } // Gentle: never (harm.js)
...
function goMate(w, e) {
  ...
  if (dist(w, e, o) < R.breed.contact) {
    ...
    const full = R.flags.capAtBirth && (w.count >= R.breed.popCap || w.kindCount[w.C.kid[kind]] >= R.breed.speciesCap);
    ...
    const cd = w.C.S[kind].breed / (R.flags.meadow && onFlowers(w, e) ? R.breed.meadowMul : 1);
