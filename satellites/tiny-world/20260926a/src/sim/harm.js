...
// Design 19 T9 (SILENCE-MAP §6): Safe covers her flock against her OWN powers, as it covers a person. A fireball on
// the pen with Safe on killed three sheep of four in her first world, where a person in the same blast keeps 1 hp.
// Only the power column: a wolf still takes a lamb (that is the world's, not her finger's).
function flock(w, e) {
  const ki = w.C.kid[w.E.kind[e]];
  return w.safe && w.R.flags.safeFlock && ((w.C.tags.kind0[ki] & w.flock0) | (w.C.tags.kind1[ki] & w.flock1)) !== 0;
}
...
export function effect(w, e, src) {
  const H = w.C.harm;
  let fx = -1;
  if (guarded(w, e) || (src === SRC.power && flock(w, e))) fx = H.safe[src];
