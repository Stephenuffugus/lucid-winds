...
// A village's job board and what it remembers, as a world starts it and as a Clear leaves it (one list, so the two
// never drift apart). Every field it will ever carry is here from the start (design 18 A15's law).
export const freshVillage = () => ({ flag: 0, store: 0, next: 0, worker: 0, site: -1, cd: 0, age: 0, told: -1, said: 0, since: 0, founding: 0, quit: 0, lostTo: 0, lostWet: 0, learned: 0, idle: 0, posted: 0, near: 1e9,
  scareT: 0, // design 18 E5: when the village may next count a near miss
  style: '', styleT: 0, // design 18 E1: what the village has taken up, and when it next looks
  birds: 0, scared: 0 }); // design 18 E2: mouthfuls of its field the birds have had, and whether it has a scarecrow up
...
export function createWorld(C, { cols, rows, seed, settings }) {
  ...
  const w = {
    ...
    // Design 18 A15 (written by phase E): how worn each tile is by the village walking over it, and how many
    // tiles carry any wear at all, so a world nobody has walked on saves nothing for it.
    wear: new Uint8Array(n), wearN: 0,
    // Design 19 A1, the tile memory (LAND-ENGINE §1): age, game minutes each tile has been its kind (255 at most);
    // hold, game minutes left in which the land leaves a tile she painted alone. landN: changes the land made this
    // minute (the cap). keepWater: the water tiles her brush has made, the baseline the wandering pond keeps (A2b).
    age: new Uint8Array(n), hold: new Uint8Array(n), landN: 0, keepWater: 0,
    land: null, // the land's compiled rules and its scratch (land.js compileLand, on the first step that asks: every field from the start)
    ...
    rxBird0: C.tags.of(['bird'])[0], rxBird1: C.tags.of(['bird'])[1], // (design 18 E2: who counts as a bird at the field)
    ...
    flock0: C.tags.of(['livestock'])[0], flock1: C.tags.of(['livestock'])[1],
    water0: C.tags.of(['water'])[0], water1: C.tags.of(['water'])[1], // design 19 A2b: the kept tag (land.js, keepPaint) // design 19 T9: who Safe covers against her powers (harm.js flock)
    spCap: 0, // how many of one kind this world holds (speciesCapOf, worked out on first use)
    layEvery: Math.max(1, Math.round(C.rules.react.layEverySec / C.rules.tickSec)), // steps between a hen's chances to lay
    biteEvery: Math.max(1, Math.round(C.rules.graze.biteSec / C.rules.tickSec)), // design 19 G1.7: steps between a filter feeder's bites of its water plant
  };
...
export function resetWorld(w) {
  ...
  w.wear.fill(0); w.wearN = 0; // (a Clear takes the paths with it)
  w.age.fill(0); w.hold.fill(0); w.landN = 0; w.keepWater = 0; if (w.land) w.land.keepLive = -1; // (design 19: and the land's memory, and its water count)
  w.burn.fill(0); w.burnN = 0; w.tmr.length = 0; w.fireSpent = 0;
  w.claim.fill(0); w.claimN = 0; w.vg = freshVillage();
...
// ---------- asleep (design 18 A5) ----------
// `sleepT` is the seconds of sleep left, or one of these two while it sleeps until the light changes.
export const SLEEP_DAWN = -1, SLEEP_DUSK = -2;
// A creature a bedtime row has SENT somewhere is on its way to bed, not asleep yet: the nap takes hold when the
// errand ends, which is the walk to the barn and then the sleeping in it, in that order, with one field.
export const asleep = (w, e) => w.E.sleepT[e] !== 0 && !(w.E.errT[e] > 0);
// Design 18 A14: where the one she NAMED likes to sleep. Written only when it falls asleep, and only for a
// creature the child has named, because it is her cat's own place and nobody else's. No favourite yet: here.
// Asleep within favNear of it again: it is surer of it (up to favSure). Somewhere else: less sure, and when it
// is sure of nothing the favourite moves to where it is now.
export function favourite(w, e) {
  const E = w.E, V = w.R.react;
  if (!w.R.flags.favourites || !E.named[e] || E.inside[e]) return;
  const tile = Math.floor(E.y[e] / w.T) * w.cols + Math.floor(E.x[e] / w.T);
  if (E.favTile[e] < 0) { E.favTile[e] = tile; E.favN[e] = 1; return; }
  const fx = (E.favTile[e] % w.cols) * w.T + 4, fy = ((E.favTile[e] / w.cols) | 0) * w.T + 4;
  const dx = E.x[e] - fx, dy = E.y[e] - fy;
  if (dx * dx + dy * dy <= V.favNear * V.favNear) { if (E.favN[e] < V.favSure) E.favN[e]++; return; }
  if (E.favN[e] > 0) E.favN[e]--;
  if (E.favN[e] === 0) { E.favTile[e] = tile; E.favN[e] = 1; }
}

// Awake, and deciding again at once. `hop`: the small jump of waking up in the morning, which is the thing she
// sees (never for somebody in a house, in the air or held). Returns whether it was asleep at all.
export function wake(w, e, hop) {
  const E = w.E;
  if (E.sleepT[e] === 0) return false;
  E.sleepT[e] = 0;
  E.think[e] = 0;
  if (hop && !E.inside[e] && !(E.alt[e] > 0) && !E.perch[e]) { E.alt[e] = w.R.react.wakeHop; E.chute[e] = false; }
  return true;
}
// Distance between two creatures' feet.
export const dist = (w, a, b) => hyp(w.E.x[a] - w.E.x[b], w.E.y[a] - w.E.y[b]);

// How many of one kind a world holds (design 17). The species cap was one number, 60, whatever the world's size:
// sixty sheep AND sixty ducks AND sixty chickens on a phone sized world is a carpet, not a meadow (measured in the
// opening world, Sep 21: sheep at 60 inside ten minutes with no help from anybody). It scales with the ground now,
// from rules.breed.capArea tiles upward, and never under rules.breed.capFloor of the full number.
export function speciesCapOf(w) {
  if (w.spCap > 0) return w.spCap; // worked out once: a world's size never changes (and this is asked on every think)
  const B = w.R.breed;
  w.spCap = !w.R.flags.worldCaps || !B.capArea ? B.speciesCap : Math.max(2, Math.round(B.speciesCap * Math.max(B.capFloor, Math.min(1, w.nTiles / B.capArea))));
  return w.spCap;
}
// Design 19 G1.2 (WATER-LIFE E3): a species may have its own ceiling under the world's (`cap` in creatures.json), so
// a pond stays a pond: two koi became thirty nine in ten minutes with nothing but the world's cap to stop them.
export function capOf(w, kind) {
  const c = speciesCapOf(w), own = w.R.flags.speciesCaps ? w.C.S[kind].cap : 0;
  return own > 0 && own < c ? own : c;
}
...
// Design 19 G1.1 (WATER-LIFE E1): what the ground under a creature feeds a filter feeder or an amphibious grazer, a
// second. It replaced "deep water feeds, nothing else does" (update.js), which starved 282 of 299 pond animals in a
// later world on the shallows and the lily pond. (A duck on a bridge is fed by the water under it: harmless.)
export function feedAt(w, k) {
  const f = k < 0 ? 0 : w.C.TERR[w.terr[k]].feed || 0;
  return f > 0 && w.eaten[k] > 0 && w.R.flags.plantsWear ? f * 0.5 : f; // design 19 G1.7: a plant eaten down feeds at half
}
...
export function placeStruct(w, type, tx, ty) {
  ...
  if (def.village) { w.vg.flag = s.h; w.vg.since = w.time; w.vg.said = 0; w.vg.quit = 0; } // (a flag, whoever planted it, ends the village having quit) // the flag: one village per world, the last one planted
...
// Design 19 A2b, the water budget: her brush (and the starter's paint) moves the baseline the wandering pond keeps,
// one tile of the `water` tag at a time; the land's own pass never does. Called with the kind before and after.
export function keepPaint(w, was, now) {
  if (!w.R.flags.land) return;
  const T = w.C.tags, a = ((T.terr0[was] & w.water0) | (T.terr1[was] & w.water1)) !== 0, b = ((T.terr0[now] & w.water0) | (T.terr1[now] & w.water1)) !== 0;
  if (a !== b) w.keepWater += b ? 1 : -1;
}
export function setTerrain(w, tx, ty, t) {
  const i = ty * w.cols + tx;
  w.terr[i] = t; w.topo++;
  w.eaten[i] = 0;
  w.age[i] = 0; // design 19 A1: the one door every ground change goes through, so a tile's age is its age as this kind
  markDirty(w, i);
