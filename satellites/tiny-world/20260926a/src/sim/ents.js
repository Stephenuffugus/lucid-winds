...
export const BLOCK = 1024; // slots are added in blocks of this many
...
// inside = HELD: in the player's hand (design 14 §3; commands.js lift / drop). Every scan skips a creature whose inside
// is set, and its step is skipped while it is held. (A UFO's passenger has inside = minus the UFO's handle, never -1.)
export const HELD = -1;
export const G_NONE = 0, G_MOVE = 1, G_ATTACK = 2, G_ABDUCT = 3, G_GRAZE = 4, G_BUSH = 5, G_HOUSE = 6, G_MATE = 7;
export const G_JOIN = 8, G_FETCH = 9, G_STORE = 10, G_BUILD = 11; // the village (14 §7 item 13)
export const G_TAKE = 12; // something lying on the ground it wants (design 15 A4)
export const G_CARE = 13; // going to another creature to feed it or play with it (design 15 B3)
export const G_WATER = 14, G_DOUSE = 15; // fetching water, and carrying it to a fire (design 15 B3)
export const GOAL_NAMES = ['', 'move', 'attack', 'abduct', 'graze', 'bush', 'house', 'mate', 'join', 'fetch', 'store', 'build', 'take'];
...
// when that path was made. hazT: seconds it has stood in lava or water it cannot bear, or dry out of water (harm.js,
// the warning before a guarded or covered creature is hurt; design 14 §6).
export const FIELDS = ['id', 'kind', 'x', 'y', 'px', 'py', 'hp', 'hunger', 'gear', 'face', 'think', 'cd', 'breedCd', 'born', 'baby',
  ...
  'blockT', 'pathN', 'pathI', 'pathGoal', 'pathQd', 'pathAt', 'pathFresh', 'pathTopo', 'hazT',
  'fol', 'folT', 'calm', // reactions (14 §5): who it follows and for how long, and how long it stays peaceful
  'named', // 1 once the child gave it its name (14 §7 T11): Pets safe guards it and the eraser spares it
  'vill', 'carry', 'work', // the village (14 §7 item 13): the flag it belongs to, food in its arms, the job it is on
  // Design 15 B1: satedT, seconds left of the rest a creature takes after a meal (it starts no hunt while it
  // runs); feeds, meals eaten since its last litter (a predator raises young on what it has caught).
  'satedT', 'feeds',
  'bigT', // design 17: seconds left of being a GIANT (a mushroom does it). Drawn twice the size, and tagged `giant`.
  // An ERRAND (design 17): somewhere a row has sent it (down to the water at dawn, over to look at what was just
  // put down, to the fire when it is poked), for errT more seconds. It is a behaviour like any other, scored under
  // eating and far under fleeing, so a sheep on its way to the pond still sees the wolf. The first cut held the
  // creature's thinking for the length of the walk instead, and the balance gate caught it: 27 seeds of 32 keeping
  // both species fell to 17, because every dawn walked a blind flock to the same shore.
  'errT', 'errX', 'errY',
  // Design 18 A15: the four fields the rest of design 18 needs, all four here from the start (the law: a field
  // added later changes the object's shape while the step's hottest functions are reading it, and the engine
  // throws their optimized code away). sleepT: seconds of sleep left, or -1 while it sleeps until the light
  // changes (A5). was: what it was the FIRST time something changed it, '' for everybody else (A13). favTile,
  // favN: where a creature the child named likes to sleep, and how sure it is of that (A14).
  'sleepT', 'was', 'favTile', 'favN',
  // Design 19 (its one save bump, v13, with every field the design writes): bubbleT, seconds left in the rescue bubble
  // of A7 (a named water creature whose last water was painted away floats in it, hunger and hazard paused).
  'bubbleT'];
...
const TYPES = { face: Int8Array, baby: Uint8Array, dead: Uint8Array, chute: Uint8Array, bounced: Uint8Array, goalKind: Uint8Array,
  ...
  named: Uint8Array, carry: Uint8Array, feeds: Uint8Array, was: Array, favTile: Int32Array, favN: Uint8Array, blockT: Uint8Array, pathN: Uint8Array, pathI: Uint8Array, pathGoal: Int32Array, pathQd: Uint8Array, pathAt: Int32Array, pathFresh: Uint8Array, pathTopo: Int32Array };
export const BOOL_FIELDS = ['baby', 'dead', 'chute'];
export const typeOf = (f) => TYPES[f] || Float64Array;
...
export function goalEnt(w, i, kind, o) {
  ...
  // The list of "somebody is attacking one of ours": anything non-human whose goal is a person or a pet (design 15
  // A1, so an armed person can be told there is something to step into without scanning the world). Whoever reads
  // it filters it again for what they care about, so a wider list changes no answer: a guardian still only takes
  // things after a person (decide.js guardFromList).
  if (kind === G_ATTACK && o >= 0 && E.kind[i] !== 'human' && (E.kind[o] === 'human' || w.C.S[E.kind[o]].pet) && !w.atkIn[i]) { w.atkIn[i] = 1; w.atkList[w.nAtk++] = i; }
}
...
// A pair's wait after a litter, and a newborn's first one. A species' number is a share of a 60 s day
// (design 14 §9.1), so a world with a longer day waits longer; and a newborn is not ready to pair the minute
// it stops being small. Without the flag this is exactly the species number, and a newborn gets 0, as before.
// A creature with no `breed` at all never pairs off (ai/decide.js B_MATE tests it), but it can still HATCH: a
// goose lays and does not mate. `undefined * anything` is NaN, and a NaN in a Float64 creature field poisons
// the hash and every save after it. Found 22 Sep by the stress gate, on the first creature in the game that
// lays without breeding (`goose#766.breedCd = NaN`).
export const breedCdOf = (w, kind) => (w.C.S[kind].breed || 0) * (w.R.flags.growUp ? w.daySec / w.R.breed.dayRef : 1);
...
export function spawn(w, kind, x, y, baby) {
  ...
  E.breedCd[s] = baby ? (R.flags.growUp ? breedCdOf(w, kind) : 0) : rnd(w, R.spawn.breedCd[0], R.spawn.breedCd[1]);
  ...
  E.blockT[s] = 0; E.pathN[s] = 0; E.pathI[s] = 0; E.pathGoal[s] = 0; E.pathQd[s] = 0; E.pathAt[s] = 0; E.pathFresh[s] = 0; E.pathTopo[s] = 0; E.hazT[s] = 0;
  E.fol[s] = 0; E.folT[s] = 0; E.calm[s] = 0; // reactions (14 §5): a slot handed back must not keep what it was doing
  E.named[s] = 0; // and it is nobody the child has named, whatever the creature before it was called
  E.vill[s] = 0; E.carry[s] = 0; E.work[s] = 0; // and it belongs to no village
  E.bigT[s] = 0; // and it is the size it was born
  E.errT[s] = 0; E.errX[s] = 0; E.errY[s] = 0; // on no errand
  // Design 18: wide awake, never changed into anything, and with nowhere of its own yet. A second-hand slot must
  // not hand a newborn the nap, the past or the favourite place of whatever was in it (the fault `cleanSlots`
  // was added for; these fields are new, so clearing them moves nothing).
  E.sleepT[s] = 0; E.was[s] = ''; E.favTile[s] = -1; E.favN[s] = 0;
  E.bubbleT[s] = 0; // (design 19 A7: in no bubble)
  // satedT and feeds were never cleared here, so a newborn in a second-hand slot inherited the rest and the meal
  // count of whatever died in it (found Sep 21 while adding bigT). Behind a switch because it moves the worlds.
  if (R.flags.cleanSlots) { E.satedT[s] = 0; E.feeds[s] = 0; }
  ...
  // The enter trigger's mark (reactions.js): a creature that appears has not entered anywhere, and the slot may
  // be a second-hand one whose old mark would otherwise decide whether its first step counts as entering a tile.
  // (A loaded world rebuilds those marks and a running one does not, which is how this showed: two worlds that
  // were the same played on differently, five hundred steps later. save-check play-on, design 15 C1.)
  if (w.rx && w.rx.lastTile.length > s) {
    const tx = Math.floor(x / w.T), ty = Math.floor(y / w.T);
    w.rx.lastTile[s] = tx >= 0 && ty >= 0 && tx < w.cols && ty < w.rows ? ty * w.cols + tx : -1;
  }
...
function release(w, i) {
  ...
  E.gear[i] = null; E.over[i] = undefined; E.name[i] = undefined; E.was[i] = '';
  w.slotH[i] += SLOT_SPAN;
  w.free[w.nFree++] = i;
}
