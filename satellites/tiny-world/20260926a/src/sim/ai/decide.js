...
import { ent, goalMove, goalEnt, G_MOVE, G_ATTACK, G_ABDUCT, G_GRAZE, G_BUSH, G_HOUSE, G_MATE, G_JOIN, G_FETCH, G_STORE, G_BUILD, G_TAKE, G_CARE, G_WATER, G_DOUSE } from '../ents.js';
...
import { canFound, maxFolkOf } from '../village.js';
...
function see(w, e, b) {
  ...
  // A meat eater has no food thing in the world (every food in buildings.json is a plant), so its only hope is
  // what it can see. One that is truly hungry looks right across the field instead of ten tiles.
  else if (b === P_PREY) v = nearest(w, e, w.R.flags.hungerFair && w.E.hunger[e] > w.R.needs.desperate ? A.huntScanFar : A.huntScan, fPrey);
...
function badOne(w, e) {
  ...
  // Design 15 B3: the first of a group to see it raises the alarm, and its own kind near by take the fright
  // from it rather than waiting for their own think tick. A flock scatters as one.
  if (threat >= 0 && w.R.flags.care && E.beh[e] !== B_FLEE && E.beh[e] !== B_FIGHT) alarm(w, e, threat);
  return threat >= 0 ? threat : E.anger[e] ? ang : -1;
}
function alarm(w, e, threat) {
  const E = w.E, C = w.R.care, h = w.slotH[threat];
  mark(w, 'warn', E.x[e], E.y[e] - 10, w.R.fx.warn);
  story(w, STI.warn, E.x[e], E.y[e], e, threat, -1, kindIcon(w, threat), w.C.whyIcon.hurt, kindIcon(w, e), null);
  const n = gatherAny(w, E.x[e], E.y[e], C.warnRange), kind = E.kind[e];
  // The alarm buys its own kind a moment, not a sixth sense: only those it would have reached anyway take it
  // (within care.warnSee of the danger themselves), and they take it now instead of on their own think tick.
  // Giving it to everyone within earshot halved what wolves caught on the balance gate, which is not a warning,
  // it is a shield (measured: 196 kills to 98 over four seeds of the gate's world).
  const see2 = C.warnSee * C.warnSee;
  for (let k = 0; k < n; k++) {
    const o = w.near[k];
    if (o === e || o === threat || E.dead[o] || E.inside[o] || E.kind[o] !== kind) continue;
    if (E.anger[o] || E.beh[o] === B_FLEE || E.beh[o] === B_FIGHT) continue; // already in it
    const dx = E.x[o] - E.x[threat], dy = E.y[o] - E.y[threat];
    if (dx * dx + dy * dy > see2) continue;
    E.anger[o] = h; // it knows what the fright is about, this step and not the next
    E.think[o] = 0;
  }
}
...
function brave(w, e, bad) {
  ...
  // Design 15 B2: bold creatures need less of an edge before they stand their ground, timid ones want more.
  const ratio = w.R.ai.braveRatio * (1 - dial(w, e, D_BOLD) * w.R.dials.brave);
  return (kind === 'human' && armed(w, e)) || (sp.atk > 0 && kind !== 'human' && sp.atk * sp.hp >= bs.atk * bs.hp * ratio);
}

// The three things B_TAKE looks for, as plain functions over a scratch pair (who is looking, in which world), not
// closures made afresh for every creature on every think: those were the biggest thing the step allocated
// (gc-check, Sep 21: 600 to 1,500 bytes a step, over the whole 1 KB budget on their own).
let TW = null, TE = -1;
const tMine = (s) => !TW.R.flags.grabs || canGrab(TW, TE, s.def.item);
const tWeapon = (s) => !!s.def.item && !!TW.C.WEAP[s.def.item] && tMine(s);
// A toy worth fetching: one that is not already lying at somebody's feet. A dog that had just brought the ball
// back picked it straight up again in the same step, so it was never seen on the ground and nobody could throw it.
const tToy = (s) => !!s.def.item && !!TW.C.ITAG[s.def.item] && TW.C.ITAG[s.def.item].has('toy') && tMine(s) && !byAPerson(TW, s);
const tToyAtFeet = (s) => !!s.def.item && !!TW.C.ITAG[s.def.item] && TW.C.ITAG[s.def.item].has('toy') && tMine(s);
function byAPerson(w, s) {
  const E = w.E, x = s.tx * w.T + 4, y = s.ty * w.T + 4, r = w.R.ai.fetchKeep;
  for (let k = 0; k < w.nHumans; k++) { const h = w.humans[k]; if (E.dead[h] || E.inside[h]) continue; const dx = E.x[h] - x, dy = E.y[h] - y; if (dx * dx + dy * dy < r * r) return true; }
  return false;
}
const tOoh = (s) => !!s.def.item && wants(TW, TE, s) && tMine(s);
...
// Design 15 B3: looking after each other. A little one of its own kind that is going hungry while it is full,
// and a grown one of its own kind that is as fed and as safe as it is, to stand about with.
const fLittleOne = (w, e, o) => {
  const E = w.E, C = w.R.care;
  return E.baby[o] && !E.baby[e] && E.kind[o] === E.kind[e] && E.hunger[o] > C.feedHungry && !E.inside[o];
};
const fPlaymate = (w, e, o) => {
  const E = w.E, C = w.R.care;
  return E.kind[o] === E.kind[e] && E.hunger[o] < C.playFull && !E.inside[o] && E.baby[o] === E.baby[e] && E.goalKind[o] !== G_ATTACK;
};
// Design 15 B2: how far a creature will go for its own kind — a sociable one keeps them nearer, a solitary one
// hardly looks (and in a crowd of its own kind, a solitary one leaves: B_IDLE).
const kinScan = (w, e, r) => r * (1 + dial(w, e, D_SOCIABLE) * w.R.dials.kin);
// How many of its own kind are around it (design 15 B2: what a crowd means to a solitary creature).
function kinAround(w, e) {
  const E = w.E, n = gatherAny(w, E.x[e], E.y[e], w.R.care.scan), kind = E.kind[e];
  let c = 0;
  for (let k = 0; k < n; k++) { const o = w.near[k]; if (o !== e && !E.dead[o] && !E.inside[o] && E.kind[o] === kind) c++; }
  return c;
}

// Design 15 B3, second half. Somebody of its own kind who is much hungrier than it is (it is carrying food);
// somebody of its own kind badly hurt, to stand beside.
const fHungrier = (w, e, o) => {
  const E = w.E;
  // Design 15 B2: a generous creature hands food over on a smaller difference, a greedy one wants a bigger one.
  const gap = w.R.care.shareGap - dial(w, e, D_GENEROUS) * w.R.dials.share;
  return E.kind[o] === E.kind[e] && !E.inside[o] && E.hunger[o] >= E.hunger[e] + gap;
};
// Design 15 B3 says "sociable only, until proven cheap": the species that live in flocks and herds, and people.
// A wolf pack that stands by a starving pack-mate starves with it (measured on the balance gate).
const fHurtKin = (w, e, o) => {
  const E = w.E, S = w.C.S;
  return E.kind[o] === E.kind[e] && !E.inside[o] && o !== e && E.hp[o] < S[E.kind[o]].hp * w.R.care.stayHp;
};
const sWater = (w) => (s) => !!s.def.water; // a well, a barrel, a fountain

// The nearest burning tile within r of a creature, or -1 (design 15 B3: the village puts fires out).
function fireNear(w, e, r) {
  const E = w.E, T = w.T;
  let best = -1, bd = r * r;
  for (let k = 0; k < w.burnN; k++) {
    const i = w.burnList[k];
    if (w.burn[i] <= 0) continue;
    const dx = (i % w.cols) * T + 4 - E.x[e], dy = ((i / w.cols) | 0) * T + 4 - E.y[e], d = dx * dx + dy * dy;
    if (d < bd) { bd = d; best = i; }
  }
  return best;
}

// ---------- design 15 B2 stage 1: personality ----------
// Four dials. The species sets the baseline (creatures.json `dials`), and the creature shifts it by at most one
// step, so a bold rabbit is a brave rabbit and not a bear. In stage 1 that step comes from the creature's own
// id: one in six low, four in six middle, one in six high, which is stable for its whole life and needs no save.
// The ground and the company bend what a dial READS, without changing what it is: beside a fire or a flag the
// timid read one step bolder, on cold ground everyone reads one step more sociable.
export const DIALS = ['bold', 'generous', 'sociable', 'curious'];
const DIAL_SALT = [0x9e3779b1, 0x85ebca6b, 0xc2b2ae35, 0x27d4eb2f];
function idStep(w, e, k) {
  let h = Math.imul(w.E.id[e] ^ DIAL_SALT[k], 0x45d9f3b) >>> 0;
  h ^= h >>> 16; h = Math.imul(h, 0x45d9f3b) >>> 0; h ^= h >>> 16;
  const r = h % 6;
  return r === 0 ? -1 : r === 5 ? 1 : 0;
}
// k: an index into DIALS. Returns -2..2 (most creatures are 0, or their species' baseline).
export function dial(w, e, k) {
  if (!w.R.flags.dials) return 0;
  const sp = w.C.S[w.E.kind[e]], base = (sp.dials && sp.dials[DIALS[k]]) || 0;
  let v = base + idStep(w, e, k);
  if (k === 0 && v < 0) { w.pp[0] = w.E.x[e]; w.pp[1] = w.E.y[e]; if (nearWarmth(w)) v += 1; } // a fire or a flag
  if (k === 2) { const t = terrAt(w, w.E.x[e], w.E.y[e]); if (t >= 0 && w.C.TERR[t].cold) v += 1; } // huddling weather
  return v;
}
// A campfire, a torch or the village flag within rules.ai.fireFear of the point in w.pp: what makes the timid
// braver (it is the same reach the fearful keep from a fire).
function nearWarmth(w) {
  const T = w.T, x = w.pp[0], y = w.pp[1], r = w.R.ai.fireFear;
  for (let k = 0; k < w.fires.length; k++) { const f = w.fires[k], dx = f.tx * T + 4 - x, dy = f.ty * T + 4 - y; if (dx * dx + dy * dy < r * r) return true; }
  if (w.vg.flag) { const f = struct(w, w.vg.flag); if (f) { const dx = f.tx * T + 4 - x, dy = f.ty * T + 4 - y; if (dx * dx + dy * dy < r * r) return true; } }
  return false;
}
const D_BOLD = 0, D_GENEROUS = 1, D_SOCIABLE = 2, D_CURIOUS = 3;

// One creature in three is the curious one, always the same one (design 15 A4; personalities take this over in B2).
const curious = (w, e) => {
  const v = dial(w, e, D_CURIOUS);
  return v > 0 || (v === 0 && w.E.id[e] % 3 === 0); // wary ones never stop for a trinket; curious ones always do
};
...
// Design 19 G2.4 (CIRCLE-OF-LIFE 1.3.1, the pen bug): a grazer takes the grass it can reach over a food thing it cannot
// (a fence or a wall on the straight line to it) or one more than rules.food.grassFirstTiles further than the grass.
// Four penned sheep starved on full grass in every later world, walking at the hay beyond the fence. (Not `blockT`:
// it is back to 0 the moment the sheep takes a step, and a sheep asked on every think flipped between the two.)
function grassFirst(w, e, food) {
  const g = see(w, e, P_GRASS);
  if (g < 0) return false;
  const s = struct(w, food), T = w.T, x = w.E.x[e], y = w.E.y[e];
  if (fenced(w, Math.floor(x / T), Math.floor(y / T), s.tx, s.ty)) return true;
  const gx = (g % w.cols) * T + 4 - x, gy = ((g / w.cols) | 0) * T + 5 - y, fx = s.tx * T + 4 - x, fy = s.ty * T + 5 - y;
  const far = Math.sqrt(gx * gx + gy * gy) + w.R.food.grassFirstTiles * T;
  return fx * fx + fy * fy > far * far;
}
...
// Is there a bed for a baby HERE? The beds under the roofs within rules.breed.bedsScan of this person, against the
// people already living within it. Local on purpose: the first cut counted the whole world, and a village with
// empty houses could then have no children because of a crowd of strangers on the far side of the map (found by
// the coverage scenes, which hold seventy five people at once). Asked only by somebody fed, grown and ready.
function bedFor(w, e) {
  const E = w.E, T = w.T, r = w.R.breed.bedsScan, x = E.x[e], y = E.y[e], r2 = r * r;
  let beds = w.R.breed.bedsSpare;
  for (let k = 0; k < w.structs.length; k++) { const s = w.structs[k], h = s.def.beds === undefined ? s.def.home : s.def.beds; if (!h) continue; const dx = s.tx * T + 4 - x, dy = s.ty * T + 4 - y; if (dx * dx + dy * dy <= r2) beds += h; } // (`beds`: a barn shelters five and is nobody's nursery)
  if (beds <= 0) return false;
  let folk = 0;
  const fr = w.R.breed.folkScan, n = gatherAny(w, x, y, fr); // (a wider look for people than for roofs: they wander off from the houses they sleep in)
  for (let k = 0; k < n; k++) { const o = w.near[k]; if (E.kind[o] !== 'human' || E.dead[o]) continue; const dx = E.x[o] - x, dy = E.y[o] - y; if (dx * dx + dy * dy <= fr * fr) folk++; } // (within fr, not merely in a cell fr touches)
  // (those asleep indoors are not in the spatial lists: they are counted by the houses' own occupancy)
  for (let k = 0; k < w.structs.length; k++) { const s = w.structs[k]; if (!s.def.home || !s.occ) continue; const dx = s.tx * T + 4 - x, dy = s.ty * T + 4 - y; if (dx * dx + dy * dy <= fr * fr) folk += s.occ; }
  return folk < beds;
}
// Standing on ground painted for the village.
function onClaim(w, e) { const i = Math.floor(w.E.y[e] / w.T) * w.cols + Math.floor(w.E.x[e] / w.T); return i >= 0 && i < w.nTiles && (w.claim[i] & 1) === 1; }
...
export const B_FEED = 15, B_PLAY = 16; // looking after each other (design 15 B3)
export const B_SHARE = 17, B_STAY = 18, B_DOUSE = 19; // and the rest of it (design 15 B3)
export const B_ERRAND = 20; // somewhere a row has sent it (design 17)
...
const BEHAVIOURS = [
  ...
  { id: B_TAKE, max: 92,
    score(w, e) {
      ...
      if (bad >= 0 && !armed(w, e)) { // grab it
        const s = findStruct(w, e, R.ai.takeGrabScan, tWeapon);
        if (s) {
      ...
      // Design 17: FETCH. A pet that can see a toy lying about goes and gets it, and that beats keeping to heel
      // (guarding scores 50 and would call the dog back the moment the ball landed further off than it likes to be).
      if (bad < 0 && R.flags.pokes && w.C.S[E.kind[e]].pet && !E.gear[e].hand) {
        const toy = findStruct(w, e, R.ai.fetchScan, tToy);
        if (toy) { w.perc.take = toy.h; return put(w, 55); }
      }
      // And a PERSON with a toy lying at their feet picks it up, curious or not, so the ball a dog has just brought
      // back gets thrown again (only one person in three stops for a trinket, and the game of fetch stalled at the
      // first person who was not one of them: the review of Sep 21).
      if (bad < 0 && R.flags.pokes && E.kind[e] === 'human' && !E.gear[e].hand) {
        const toy = findStruct(w, e, R.ai.fetchKeep, tToyAtFeet);
        if (toy) { w.perc.take = toy.h; return put(w, 30); }
      }
      // Design 15 B2: a curious creature will stop for a trinket when it is hungrier than a wary one would.
      if (bad >= 0 || E.hunger[e] > R.needs.hungry + dial(w, e, D_CURIOUS) * R.dials.oohHunger) return false;
      if (!curious(w, e)) return false;
      const s2 = findStruct(w, e, R.ai.takeOohScan, tOoh);
      if (!s2) return false;
      w.perc.take = s2.h;
      return put(w, 20);
    },
  ...
  { id: B_EAT, max: 85, // hungry: a food thing, else (grazers) grass
    ...
    act(w, e) {
      ...
      if (food && !(w.R.flags.penGrazes && sp.diet === 'herb' && grassFirst(w, e, food))) { E.goalKind[e] = G_BUSH; E.goalA[e] = food; return; }
  ...
  { id: B_DOUSE, max: 40, // a villager with water within reach of a fire: the village putting itself out
    score(w, e) {
      const E = w.E, C = w.R.care;
      if (!w.R.flags.douse || !E.vill[e]) return false;
      if (E.carry[e] === 2) return fireNear(w, e, C.douseScan * 3) >= 0 ? put(w, 40) : false;
      if (E.carry[e]) return false; // its arms are full of somebody's dinner
      if (fireNear(w, e, C.douseScan) < 0) return false;
      return findStruct(w, e, C.waterScan, sWater(w)) ? put(w, 40) : false;
    },
    act(w, e) {
      const E = w.E, C = w.R.care, T = w.T;
      if (E.carry[e] === 2) {
        const i = fireNear(w, e, C.douseScan * 3);
        if (i < 0) return;
        E.goalKind[e] = G_DOUSE; E.goalX[e] = (i % w.cols) * T + 4; E.goalY[e] = ((i / w.cols) | 0) * T + 6; E.goalRun[e] = 1;
        return;
      }
      const s = findStruct(w, e, C.waterScan, sWater(w));
      if (!s) return;
      E.goalKind[e] = G_WATER; E.goalX[e] = s.tx * T + 4; E.goalY[e] = s.ty * T + 6; E.goalRun[e] = 1;
    } },
  { id: B_MATE, max: 30, // fed, healthy adults ready to breed, under both caps
    score(w, e) {
      ...
      // Design 17: people have a baby when there is a bed for it. Every roof in the world is so many beds, and a
      // village that builds grows; two people on a bare lawn stay two until somebody puts a house up. His note of
      // Sep 20 ("two humans... pretty quickly a crowd") came back the moment villages began to feed themselves:
      // 36 people at ten minutes in the opening world. It is also a rule a child can SEE: more houses, more people.
      if (kind === 'human' && w.R.flags.beds && !bedFor(w, e)) return false;
      if (!(see(w, e, P_MATE) >= 0 && w.count < B.popCap && w.kindCount[w.C.kid[kind]] < capOf(w, kind))) return false;
      ...
      // Design 19 G1.3 (WATER-LIFE E2): a filter feeder pairs only where the water feeds it well. Bare water keeps a
      // fish alive (feed 2 against hunger 1 a second) and does not multiply it; the lily pond, the kelp and the swamp
      // do. His sentence, literally: the water plants perpetuate life.
      if (w.R.flags.breedWhereFed && sp.diet === 'filter' && feedAt(w, tileOf(w, e)) < B.filterBreedFeed) return false;
      return put(w, 30);
    },
    act(w, e) { goalEnt(w, e, G_MATE, see(w, e, P_MATE)); } },
  // Design 15 B3. These sit under eating and above idling, and they are off the moment anything dangerous is
  // about: looking after each other is what a creature does when there is nothing else to do.
  { id: B_FEED, max: 28, // full, and a hungry little one of its own kind near by: it passes a meal across
    score(w, e) {
      const E = w.E, C = w.R.care;
      if (!w.R.flags.care || w.perc.bad >= 0 || E.baby[e] || E.hunger[e] > C.feedFed) return false;
      return nearest(w, e, kinScan(w, e, C.scan), fLittleOne) >= 0 ? put(w, 28) : false;
    },
    act(w, e) { goalEnt(w, e, G_CARE, nearest(w, e, kinScan(w, e, w.R.care.scan), fLittleOne)); } },
  { id: B_SHARE, max: 26, // carrying food, and somebody of its own kind much hungrier: it hands it over
    score(w, e) {
      const E = w.E, C = w.R.care;
      if (!w.R.flags.share || w.perc.bad >= 0 || E.carry[e] !== 1) return false;
      return nearest(w, e, C.scan, fHungrier) >= 0 ? put(w, 26) : false;
    },
    act(w, e) { goalEnt(w, e, G_CARE, nearest(w, e, w.R.care.scan, fHungrier)); } },
  { id: B_STAY, max: 24, // one of its own kind badly hurt near by: it stands with them rather than wandering off
    score(w, e) {
      const E = w.E, C = w.R.care;
      // Only one that has eaten recently can afford to stand about: "not starving yet" was enough to hold a
      // whole wolf pack beside a starving pack-mate until they all went the same way (balance gate, 20 Sep).
      if (!w.R.flags.stay || !w.C.S[E.kind[e]].sociable || w.perc.bad >= 0 || E.hunger[e] > C.playFull) return false;
      return nearest(w, e, kinScan(w, e, C.stayScan), fHurtKin) >= 0 ? put(w, 24) : false;
    },
    act(w, e) {
      goalEnt(w, e, G_CARE, nearest(w, e, kinScan(w, e, w.R.care.stayScan), fHurtKin));
      w.E.think[e] = w.R.care.playSec; // it stands a while, then looks again
    } },
  ...
  { id: B_BUILD, max: 24,
    score(w, e) {
      const E = w.E, vg = w.vg;
      if (vg.site < 0) return false;
      if (!E.vill[e] && !canFound(w, e)) return false; // (while a village is being founded, anybody near may plant its flag)
  ...
  { id: B_GATHER, max: 22,
    score(w, e) {
      ...
      if (E.carry[e] === 1) return put(w, 22);
      return see(w, e, P_FOOD) ? put(w, 22) : false;
    },
    act(w, e) {
      const E = w.E;
      if (E.carry[e] === 1) { const f = struct(w, w.vg.flag); if (!f) return; E.goalKind[e] = G_STORE; E.goalX[e] = f.tx * w.T + 4; E.goalY[e] = f.ty * w.T + 6; E.goalRun[e] = 0; return; }
      const food = see(w, e, P_FOOD);
  ...
  { id: B_JOIN, max: 25,
    score(w, e) {
      ...
      // Near the flag, or simply standing on the ground that was painted for the village: the paint is the
      // invitation, however big the child made it.
      if (dx * dx + dy * dy > V.joinScan * V.joinScan && !(w.R.flags.villageFounds && onClaim(w, e))) return false;
      return folk(w) < maxFolkOf(w) ? put(w, 25) : false;
    },
    act(w, e) {
  ...
  // Design 17: an errand. A row sent it somewhere (reactions.js `visit`, and `follow` towards a thing), and it goes
  // there and stands a moment, unless anything at all matters more: it scores under eating, mating and every kind of
  // work, and it is off the moment something dangerous is about. When the danger has gone it picks the errand up
  // again if there is still time on it, which is what a flock scattered on its way to the pond ought to do.
  { id: B_ERRAND, max: 22,
    score(w, e) { return w.R.flags.errands && w.E.errT[e] > 0 && w.perc.bad < 0 ? put(w, 22) : false; },
    act(w, e) {
      const E = w.E, dx = E.errX[e] - E.x[e], dy = E.errY[e] - E.y[e];
      if (dx * dx + dy * dy < 36) { E.goalKind[e] = 0; E.think[e] = Math.min(E.errT[e], w.R.ai.think * 2); return; } // there: it stands a moment
      E.goalKind[e] = G_MOVE; E.goalX[e] = E.errX[e]; E.goalY[e] = E.errY[e]; E.goalRun[e] = 0;
    } },
  // 21, not design 15 B3's 12: the engine gives whatever a creature is already doing a +10 so it does not dither,
  // and an idling creature therefore scores 20. 12 could never beat that, so play would only ever have happened
  // in the one tick after a goal ended. 21 is the same intent — above idling, below every kind of work.
  { id: B_PLAY, max: 21, // fed, safe, and one of its own kind to stand with: the sign that all is well
    score(w, e) {
      const E = w.E, C = w.R.care;
      if (!w.R.flags.care || w.perc.bad >= 0 || E.hunger[e] > C.playFull) return false;
      // In the mood only now and then, each creature on its own clock. Without this a fed hunter plays instead
      // of wandering, never finds prey again and starves where it stands: the balance gate went from 16 seeds
      // keeping both species to 7 (measured 20 Sep, 206 kills without play against 97 with it).
      if ((w.tick + E.id[e] * 37) % C.playEvery >= C.playFor) return false;
      return nearest(w, e, kinScan(w, e, C.scan), fPlaymate) >= 0 ? put(w, 21) : false;
    },
    act(w, e) {
      goalEnt(w, e, G_CARE, nearest(w, e, kinScan(w, e, w.R.care.scan), fPlaymate));
      w.E.think[e] = w.R.care.playSec; // a game lasts a few seconds, then it thinks again
    } },
  { id: B_IDLE, max: 10, // nothing to do: stand a while or wander (enemies always wander)
    ...
    act(w, e) {
      ...
      if (E.carry[e] === 2) E.carry[e] = 0; // nothing left to put out: the water goes on the ground (15 B3)
      // Design 15 B2: in a crowd of its own kind, a solitary creature goes off on its own.
      if (dial(w, e, D_SOCIABLE) < 0 && kinAround(w, e) > w.R.dials.crowd) { wander(w, e); return; }
      if (sp.enemy) { wander(w, e); return; }
      const chance = sp.ally ? A.allyIdleChance : A.idleChance, t = sp.ally ? A.allyIdleThink : A.idleThink;
      // Design 15 B3: a hungry creature with nothing in sight ranges rather than standing about. Without this a
      // wolf that has looked after its cub and found no sheep near by simply waits where it is until it starves,
      // which is what B3's behaviours turned into a habit (the balance gate saw it).
      if (w.R.flags.roam && E.hunger[e] > w.R.needs.hungry) { wander(w, e); return; }
...
// Who it would have gone for, or -1 (design 18 E5: the one it balked at is told, so a village can learn from it).
function balkTarget(w, e) {
  const E = w.E, sp = w.C.S[E.kind[e]];
  if (!(sp.enemy || sp.hunts === 'all' || E.hunger[e] > w.R.needs.hungry)) return -1;
  return nearest(w, e, w.R.harm.balkRadius, fBalk);
}
...
export function decide(w, e) {
  ...
  if (w.C.canBalk[w.C.kid[E.kind[e]]] && (w.safe || w.petsSafe) && E.goalKind[e] !== G_ATTACK) { const t = balkTarget(w, e); if (t >= 0) { balk(w, e, t); return; } }
  const P = w.perc;
  P.think++;
  // A row set off from here may take this very creature's goal (a rabbit that has seen a giant runs). It is in the
  // middle of deciding, so whatever it picked below would overwrite that one step later: the rabbit set off at a
  // wander TOWARDS the giant. If a verb took its goal, that is its decision for this think (design 17).
  const RX = w.rx;
  RX.thinker = e; RX.took = false;
  reactWhile(w, e); // design 14 §5: `while` rows are re-checked on the wearer's think tick and nowhere else
  // Design 18 A5: a sleeping creature notices nobody (everybody else still notices IT, on their own think), but
  // what it is WEARING goes on working: a lantern in a sleeping hand still keeps the undead off.
  const naps = w.R.flags.sleeps && asleep(w, e);
  if (!naps) reactMeet(w, e); // who it has noticed standing near it (the seventh trigger)
  RX.thinker = -1;
  if (RX.took && w.R.flags.pokes) return;
  P.bad = badOne(w, e);
  // ITS EYES STAY OPEN (the law: a blind walk is a cull). The threat scan above ran exactly as it does awake, and
  // it wakes when it WOULD RUN — B_FLEE's own test, so a sheep is up and running before the wolf arrives while a
  // goblin sleeps through a villager it was never afraid of — or when it is hungry. Otherwise it stays where it
  // is: no grazing, no herding, no wandering.
  if (naps) {
    const run = P.bad >= 0 && !(brave(w, e, P.bad) && reach(w, e, P.bad));
    if (!run && E.hunger[e] <= w.R.needs.hungry) { E.goalKind[e] = 0; return; } // (0 is G_NONE: ents.js)
    wake(w, e, false);
  }
...
function why(w, e, b) {
  ...
  // Design 15 B3: a game, told when it starts rather than on every heart.
  else if (b === B_PLAY) { const o = ent(w, E.goalA[e]); story(w, STI.play, x, y, e, o, -1, kindIcon(w, e), W.love, o >= 0 ? kindIcon(w, o) : NO, null); }
}
