...
export function createHud({ data, status }) {
  const dayEl = document.getElementById('day'), countsEl = document.getElementById('counts'), info = document.getElementById('info');
  let lastCounts = '';
  ...
  function update(sim, selected) {
    const w = sim.w, R = w.R, S = w.C.S, frac = (w.time % R.daySec) / R.daySec;
    dayEl.textContent = fill(str('ui.day'), { n: Math.floor(w.time / R.daySec) + 1, night: frac > R.nightFrac ? str('ui.night') : '' });
    const n = {};
    ...
    } else info.textContent = status.text || str('ui.start');
  }
  return { update };
}
