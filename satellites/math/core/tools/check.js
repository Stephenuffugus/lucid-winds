/* The one command that says whether CORE is shippable.
 *
 *   node tools/check.js            everything
 *   node tools/check.js --fast     skips the slow gates and SAYS so
 *
 * Shape copied from satellites/fathom/tools/check.js, with its scars: fast mode
 * SKIPS a slow gate and never shrinks it; a failure prints the FAILING lines,
 * not the tail; stderr is captured, never spilled into the table; and a run
 * that left the browser gates out refuses to say ALL GATES PASSED.
 *
 * An ES module, unlike Fathom's: satellites/math/package.json says
 * "type": "module" so Node reads the runtime `.js` modules as modules
 * (plans/math/HANDOFF-CORE.md 3.4), and under that a `.js` file is ESM too.
 */
...
const GATES = [
  ...
  { name: 'pure', cmd: ['test/pure.mjs'], need: 'PURE OK' }
];
...
const BROWSER_GATES = [
  ...
  { name: 'sprite', cmd: ['test/sprite.mjs'], need: 'SPRITE OK' }
];
