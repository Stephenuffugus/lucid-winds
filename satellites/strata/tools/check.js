/* The one command that says whether Strata is shippable.
 *
 ...
 * nobody has watched fail is decoration (HANDOFF-STRATA section 5).
 *
 * Shape copied from satellites/doohickey/tools/check.js, including its two
 * scars: fast mode SKIPS a slow gate and never shrinks it, and a failure prints
 * the FAILING lines rather than the tail, because the tail of a gate that died
 * on assertion three is twenty five green ones.
 */
/* CommonJS on purpose. This file is named check.js because the plan and every
   ...
   ⛔ A SPECIMEN IS ITS SEED (plan section 0). The whole of the museum, the
   share link and the journal rest on one number regenerating one animal
   exactly, so nothing in the sim gate that holds determinism may be weakened
   to go green. The second law of this game is that the VARIETY SHEET IS A GATE
   A HUMAN READS, and no machine here can stand in for that. */
...
const GATES = [
  { name: 'sim', cmd: ['sim.js', '--test'], need: 'STRATA TEST OK' },
