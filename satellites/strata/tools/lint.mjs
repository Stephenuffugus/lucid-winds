...
let parsed = true, why = '';
try { createScript(JS, { filename: 'strata-inline.js' }); }
...
const shellM = SW.match(/const SHELL_VERSION = "strata-shell-([^"]+)"/);
...
say(!!shellM && !!stampM && shellM[1] === stampM[1],
  'and sw.js carries the same one' + (shellM ? ': strata-shell-' + shellM[1] : ': sw.js has no strata-shell version'));
...
const swOwn = SW.indexOf('strata-') >= 0 && SW.indexOf('doohickey') < 0 && SW.indexOf('deepwell') < 0;
say(swOwn, 'the worker only ever deletes its own caches');
say(MAN.indexOf('/satellites/strata/') >= 0 && JSON.parse(MAN).orientation === 'any',
...
say(CODE.length > 20000 && CODE.indexOf('the toy aisle') < 0,
  'the comment stripper leaves the code and takes the prose');
const blur = CODE.match(/shadowBlur/g) || [];
say(blur.length === 0, 'no shadowBlur anywhere: two hundred by three hundred cells is a slideshow with it ('
  + blur.length + ')');
