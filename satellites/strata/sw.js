/* STRATA service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete strata-* caches, or
...
const SHELL_VERSION = "strata-shell-20260906d";
const SFX_CACHE     = "strata-unused-v1"; // no recorded audio: every sound is synthesised
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("strata-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#6E5335;color:#EFE2C8;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#33291C'>STRATA</h1>" +
    "<p>Cannot reach the cliff right now.<br>Check your connection and reload.</p></div></body>",
