...
export function createGame(stage, uiRoot, hooks = {}, data = DATA) {
  ...
  function cues() {
    ...
    const out = { card: rectOf(hud.hand.querySelector('.card.hint')), ghost: null, pulse: null, cast: hud.cast.classList.contains('pulse') && !hud.cast.disabled ? rectOf(hud.cast) : null, button: null };
    if (hintGem && hintGem.group.visible !== false) out.ghost = screenOf(hintGem.group.position);
    if (mode === 'tutorial' && state && state.tutorial.pulse !== null && state.phase === 'night' && !playing && !pulseDone) { const v = gemViews.get(state.tutorial.pulse); if (v) out.pulse = screenOf(v.group.position); }
  ...
  function beginDrag(src, e) {
    drag = { ...src, x0: e.clientX, y0: e.clientY, t0: performance.now(), moved: false, id: e.pointerId, cell: -1 };
    // A long press opens the rule card on release, so a slow frame can never turn a drag into a press.
    drag.timer = setTimeout(() => { if (drag && !drag.moved) { drag.longPressed = true; if (drag.card) drag.card.classList.add('marked'); } }, LONG_PRESS_MS);
  }
  ...
  function endDrag(e, cancelled) {
    ...
    if (!d.moved) {
      if (cancelled) return;
      if (d.longPressed) { longPress(d); return; }
