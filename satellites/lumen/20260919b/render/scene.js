...
export function createStage(canvas) {
  ...
  function frame() {
    ...
    // distance so the width fits with a margin (the tilt makes the frame's near corners look wider than its
    // flat extent, so 12% total leaves about a 16 px gutter each side at phone width) and the depth fits the zone
    const dW = bw / (2 * tanY * camera.aspect * 0.88);
