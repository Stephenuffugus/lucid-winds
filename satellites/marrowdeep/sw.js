/* MARROWDEEP service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete marrowdeep-* caches, or
   ...
   index.html in lockstep (they are both the STAMP, and tools/lint.mjs asserts it) — this host edge-pins bare sw.js URLs for 7 days. */

const SHELL_VERSION = "marrowdeep-shell-20260908a";
const SFX_CACHE     = "marrowdeep-unused-v1"; // no recorded audio: every sound is synthesised
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("marrowdeep-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#0c0a10;color:#e6dcc6;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#c9a24a'>MARROWDEEP</h1>" +
    "<p>Cannot reach the deep right now.<br>Check your connection and reload.</p></div></body>",
