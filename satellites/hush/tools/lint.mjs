#!/usr/bin/env node
/* HUSH's laws, checked against the files a browser loads from satellites/hush/ (everything but test/, tools/, docs/ and
 * package.json). Shape from satellites/glimpse/tools/lint.mjs.
 *
 *   node tools/lint.mjs
 *
 * What it asserts (plans/hush/HANDOFF-HUSH.md P0), each watched to fail on a planted line:
 *   1. every runtime module parses as an ES module
 *   2. nothing a browser loads is a .mjs
 *   3. every relative import and local asset carries ?v= plus HUSH's stamp (STAMP.js), CORE's included
 ...
 *   9. no sound is played from inside a loop, a forEach or a map (HUSH's GL2 kept: a pose is one event, one sound)
 *  10. H5: no getUserMedia, devicemotion or DeviceMotionEvent anywhere; H6 and H8: nothing a child reads hits, smashes,
 *      shoots, kills or catches, and nothing speaks of patience, self control, calm or doing well; SIMON's commands included
 */
...
const files = walk(HUSH, []);
const rel = p => relative(HUSH, p);
...
/* 5 */
{
  const words = assertForbiddenStrings(HUSH);
  say(words.ok, 'none of the catalog\'s forbidden words anywhere (' + words.detail + ')');
  const gum = assertNoGetUserMedia(HUSH);
...
{
  const copy = [];
  const content = join(HUSH, 'content.js');
   ...
   body of every for, while, forEach, map, reduce and some around it is read; none may hold one. */
{
  ...
  say(bad.length === 0, 'no sound is played from inside a loop, so one event is one sound' + (bad.length ? ': ' + bad.join('; ') : ''));
}

/* 8: the sprite table, once there is one */
{
  const p = join(HUSH, 'sprites.js');
  if (existsSync(p)) {
    let table = null, why = '';
...
/* 10: H5, H6 and H8 */
{
  ...
  say(motion.length === 0, 'H5: no camera, microphone or motion sensor is asked for anywhere a browser loads' + (motion.length ? ': ' + motion.join(', ') : ''));
  const strings = [];
  const content = join(HUSH, 'content.js');
  ...
  for (const p of HTML) {
    ...
    for (const m of text.matchAll(/aria-label=["']([^"']*)["']|>([^<]+)</g)) strings.push((m[1] || m[2] || '').trim());
  }
  const banned = strings.filter(t => /\b(hits?|smash\w*|shoot\w*|shot|kill\w*|catch\w*|caught|patien\w*|self control|calm\w*|well done|good job|good girl|good boy)\b/i.test(t));
