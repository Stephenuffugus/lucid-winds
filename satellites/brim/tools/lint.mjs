#!/usr/bin/env node
/* BRIM's laws, checked against the files a browser loads from satellites/brim/ (everything but test/, tools/, docs/ and
 * package.json). Shape from satellites/crease/tools/lint.mjs.
 *
 *   node tools/lint.mjs
 *
 * What it asserts (plans/brim/HANDOFF-BRIM.md P0 step 2), each watched to fail on a planted line:
 *   1. every runtime module parses as an ES module
 *   2. nothing a browser loads is a .mjs
 *   3. every relative import and local asset carries ?v= plus BRIM's stamp (STAMP.js), CORE's and CREASE's bank included
 *   4. engine.js and pairs.js name no document, window, Date, performance, Math.random or timer
 ...
 *   9. B8: no cross multiplication anywhere a browser loads: not the words, not a helper, not a caption (3.8)
 */
...
const files = walk(BRIM, []);
const rel = p => relative(BRIM, p);
...
/* 5 */
{
  const words = assertForbiddenStrings(BRIM);
  say(words.ok, 'none of the catalog\'s forbidden words anywhere (' + words.detail + ')');
  const gum = assertNoGetUserMedia(BRIM);
...
{
  const copy = [];
  const content = join(BRIM, 'content.js');
...
/* 9: B8. Comments are prose about the rule and may name it; code, strings and page text may not. */
{
  const WORDS = [[/\bcross[\s_-]*multipl/i, 'cross multiplication'], [/\bmultipl(y|ied|ies|ication)\b/i, 'multiply'], [/\bcommon[\s_-]*denominator/i, 'common denominator'], [/\bbutterfly\b/i, 'butterfly'], [/\bcross\b/i, 'cross']];
  const bad = [];
  for (const p of JS.concat(HTML, files.filter(f => ['.css', '.json', '.webmanifest'].includes(extname(f))))) {
    const src = extname(p) === '.html' ? read(p).replace(/<!--[\s\S]*?-->/g, ' ') : extname(p) === '.js' ? stripComments(read(p)) : read(p);
    for (const [re, name] of WORDS) if (re.test(src)) bad.push(rel(p) + ' names ' + name);
  }
