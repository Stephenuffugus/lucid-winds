...
// ---- 7. the feedback fab must not sit on a control (standing classes 2 and 8)
{
  ...
  ok('menu sheet: nothing tappable under the feedback fab', hits.length === 0, JSON.stringify(hits).slice(0, 200));
  await page.click('#btnShop');
  await new Promise(r => setTimeout(r, 2600));
  hits = await page.evaluate(FAB_PROBE + '(' + JSON.stringify(box) + ')');
  ok('nursery sheet: nothing tappable under the feedback fab', hits.length === 0, JSON.stringify(hits).slice(0, 200));
  await ctx.close();
}
