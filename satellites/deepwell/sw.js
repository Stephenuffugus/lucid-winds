/* DEEPWELL service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete deepwell-* caches, or
...
const SHELL_VERSION = "deepwell-shell-v1";
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("deepwell-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#0a0b0f;color:#e8dcc8;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#f0a742'>DEEPWELL</h1>" +
    "<p>Cannot reach the shaft right now.<br>Check your connection and reload.</p></div></body>",
