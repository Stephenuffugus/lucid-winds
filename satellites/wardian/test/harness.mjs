...
export async function open(base, opts = {}) {
  ...
  const browser = await puppeteer.launch({
    ...
    args: ['--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu',
      /* ⛔ THE FLAG THE PHONE DOES NOT HAVE. Under this flag an AudioContext
         made before a gesture works; on a real phone it stays suspended and
         every scheduled note lands in silence with no error. No gate here can
         see that bug, which is why the engine is opened inside pointerdown and
         why the phone test in the plan's section 11 exists. */
  ...
  const q = opts.query === undefined ? '' : opts.query;
  await page.goto(base + path + '?probe=' + Math.floor(Math.random() * 1e9) + q, { waitUntil: 'load', timeout: 60000 });
  /* ⛔ a frame count from the title screen is already past any absolute
     threshold, so every wait in here is RELATIVE (waitFrames) and this one only
     asks that the loop has started at all */
  await page.waitForFunction(() => window.WARDIAN_TEST && window.WARDIAN_TEST.frames() > 2, { timeout: 30000 });
...
export async function drag(page, x0, y0, x1, y1, steps = 8, holdMs = 0) {
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    await page.evaluate((x, y) => {
      const el = document.elementFromPoint(x, y) || document.getElementById('jar');
...
export async function waitFrames(page, n) {
  const now = await page.evaluate(() => window.WARDIAN_TEST.frames());
  await page.waitForFunction((f) => window.WARDIAN_TEST.frames() > f, { timeout: 40000 }, now + n);
}
