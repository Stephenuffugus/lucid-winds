...
let parsed = true, why = '';
try { createScript(JS, { filename: 'wardian-inline.js' }); }
...
const shellM = SW.match(/const SHELL_VERSION = "wardian-shell-([^"]+)"/);
...
say(!!shellM && !!stampM && shellM[1] === stampM[1],
  'and sw.js carries the same one' + (shellM ? ': wardian-shell-' + shellM[1] : ': sw.js has no wardian-shell version'));
const swOwn = SW.indexOf('wardian-') >= 0 && SW.indexOf('deepwell') < 0;
...
/* case insensitive: the law is the brand appears and is singular, not that it
   is shouted. Fathom sets it in caps, Wardian sets it in a serif. */
...
say(blur.length <= 4, 'shadowBlur is used a handful of times, not per segment (' + blur.length + ')');
const frondFn = CODE.slice(CODE.indexOf('function drawFrond'), CODE.indexOf('function drawPlant'));
say(frondFn.length > 200 && frondFn.indexOf('shadowBlur') < 0,
  'and never inside the frond, which runs once per segment');
const plantFn = CODE.slice(CODE.indexOf('function drawPlant'), CODE.indexOf('function drawFruit'));
say(plantFn.length > 200 && plantFn.indexOf('shadowBlur') < 0, 'nor inside the plant loop');
/* the promise, as a grep over the shipped file. ⛔ the markers are LINE
   COMMENTS, so they are the first thing the comment stripper eats: slice out of
   the raw script and strip afterwards, or this reads fifty characters and every
   assertion under it passes for nothing. */
...
say(SIM.length > 8000, 'the SIM block is in the shipped file (' + SIM.length + ' characters)');
say(SIM.indexOf('plants.splice') < 0 && SIM.indexOf('agents.splice') < 0,
  'and nothing in the jar is ever spliced out of it');
say(SIM.indexOf('Math.random') < 0 && SIM.indexOf('Date.now') < 0 && SIM.indexOf('document.') < 0,
  'and the SIM has no clock, no die and no document of its own');
