...
export async function open(base, opts = {}) {
  ...
  await page.waitForFunction(() => window.WHISTLESTOP_TEST && window.WHISTLESTOP_TEST.frames() > 2, { timeout: 30000 });
...
export async function waitFrames(page, n) {
  const now = await page.evaluate(() => window.WHISTLESTOP_TEST.frames());
  await page.waitForFunction((f) => window.WHISTLESTOP_TEST.frames() > f, { timeout: 40000 }, now + n);
}
