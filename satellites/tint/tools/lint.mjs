#!/usr/bin/env node
/* TINT's laws, checked against the files a browser loads from satellites/tint/ (everything but test/, tools/, docs/ and
 * package.json). Shape from satellites/notch/tools/lint.mjs.
 *
 *   node tools/lint.mjs
 *
 * What it asserts (plans/tint/HANDOFF-TINT.md P0), each watched to fail on a planted line:
 *   1. every runtime module parses as an ES module
 *   2. nothing a browser loads is a .mjs
 *   3. every relative import and local asset carries ?v= plus TINT's stamp (STAMP.js), CORE's included
 ...
 *   9. no sound is played from inside a loop, a forEach or a map (TINT's GL2 kept: a pose is one event, one sound)
 *  10. T6: no string a child or teacher reads teaches cross multiplication (cross multiply, cross multiplication, butterfly, the
 *      means and extremes, multiply across); no camera, microphone or motion sensor
 */
...
const files = walk(TINT, []);
const rel = p => relative(TINT, p);
...
/* 4 */
for (const name of ['engine.js', 'colour.js']) {
  const p = join(TINT, name);
...
/* 5 */
{
  const words = assertForbiddenStrings(TINT);
  say(words.ok, 'none of the catalog\'s forbidden words anywhere (' + words.detail + ')');
  const gum = assertNoGetUserMedia(TINT);
...
{
  const copy = [];
  const content = join(TINT, 'content.js');
   ...
   body of every for, while, forEach, map, reduce and some around it is read; none may hold one. */
{
  ...
  say(bad.length === 0, 'no sound is played from inside a loop, so one pour is one sound' + (bad.length ? ': ' + bad.join('; ') : ''));
}

/* 8: the sprite table, once there is one */
{
  const p = join(TINT, 'sprites.js');
  if (existsSync(p)) {
    let table = null, why = '';
...
/* 10: T6 and no sensors */
{
  ...
  const taught = strings.filter(([, t]) => /cross[\s-]*multipl\w*|butterfly|means and extremes|multiply across|times across/i.test(t)).map(([f, t]) => f + ': ' + JSON.stringify(t));
  say(strings.length > 0 && taught.length === 0, 'T6: no string teaches cross multiplication' + (taught.length ? ': ' + taught.join(', ') : ' (' + strings.length + ' strings)'));
}
