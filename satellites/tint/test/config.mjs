#!/usr/bin/env node
/* TINT's teacher's links (plans/tint/HANDOFF-TINT.md P3; the shape of the catalog's earlier config gates). The
 * builder at satellites/math/config registers TINT with the SAME schema the page parses (satellites/tint/config.js), so a
 ...
 *   1. the builder has an entry for TINT going to ../tint/
 ...
 *   4. main.js imports TINT_SCHEMA from ./config.js and hands it to parseConfig
...
import { TINT_SCHEMA } from '../config.js';
import { dealCompare, dealFill, dealScales } from '../engine.js';

const TINT = join(MATH, '..', 'tint');
...
const entry = GAMES.tint;
say(!!entry && entry.path === '../tint/', 'the builder has an entry for TINT going to ../tint/' + (entry ? ' (' + entry.path + ')' : ' (none)'));
if (entry) {
  const differ = [];
  for (const [k, rule] of Object.entries(entry.schema)) {
    const mine = TINT_SCHEMA[k];
  ...
  const unoffered = Object.keys(TINT_SCHEMA).filter(k => k !== 'seed' && !entry.schema[k]);
  ...
  for (const [k, rule] of Object.entries(entry.schema)) {
    for (const v of valuesOf(rule)) {
      const q = buildQuery({ [k]: v }, entry.schema), back = parseConfig(q, TINT_SCHEMA)[k];
...
{
  const src = readFileSync(join(TINT, 'main.js'), 'utf8');
  const imports = /import\s*\{[^}]*\bTINT_SCHEMA\b[^}]*\}\s*from\s*['"]\.\/config\.js\?v=/.test(src);
  const parses = /parseConfig\(\s*location\.search\s*,\s*TINT_SCHEMA\s*\)/.test(src);
  say(imports && parses, 'main.js imports TINT_SCHEMA from ./config.js and hands it to parseConfig' + (imports ? '' : ' (it does not import it)') + (parses ? '' : ' (it parses something else)'));
}
...
if (entry) {
  ...
  for (const [label, values] of [['the builder\'s defaults', defaults], ['other values', others]]) {
    ...
    const opened = await open(s.base, Object.assign({}, SIZES[1], { path: '/tint/index.html' + (q ? q + '&' : '?'), ready: 'window.TINT && window.TINT.ready' }));
    await sleep(150);
    const played = await opened.page.evaluate(() => window.TINT.config());
    const wrong = Object.keys(values).filter(k => !played || String(played[k]) !== String(values[k]));
    say(!!played && wrong.length === 0, 'TINT plays what a link of ' + label + ' asked for (asked ' + JSON.stringify(values) + ', the page plays ' + JSON.stringify(played) + ')');
    /* the page deals when a door opens: a link with a mode shows its one door, one without shows FLASH's first */
    /* a named mode shows its one door; the others are hidden */
