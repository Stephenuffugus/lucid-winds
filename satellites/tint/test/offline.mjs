#!/usr/bin/env node
/* TINT offline: the installable shell (plans/tint/HANDOFF-TINT.md P3; CATALOG-PLAN D2; the shape and scars of the
 ...
 *   1. sw.js names its cache tint-shell-<stamp>, main.js registers ./sw.js?v=<stamp>, and the page links the manifest
 *   2. straight after a first visit, the worker's cache holds every address the page asked for (read off the requests)
 *   3. a worker installing again deletes older tint caches and leaves every other cache on the origin alone
 *   4. with the server DOWN, a reload plays a SAME COLOUR round from the cache (answered and poured), and a reload opens DOES IT
 *      SCALE by its door and answers an item
...
const ROOT = join(MATH, '..'), TINT = join(ROOT, 'tint');
const { fails, say } = reporter();
const STAMP = (readFileSync(join(TINT, 'STAMP.js'), 'utf8').match(/STAMP = '([0-9]{8}[a-z])'/) || [])[1];
const CACHE = 'tint-shell-' + STAMP;
...
{
  const sw = read(join(TINT, 'sw.js'));
  const version = (sw.match(/SHELL_VERSION\s*=\s*['"]([^'"]+)['"]/) || [])[1];
  say(version === CACHE, 'sw.js names its cache for TINT\'s stamp (' + (version || 'no SHELL_VERSION') + ' for ' + CACHE + ')');
  say(read(join(TINT, 'main.js')).indexOf("register('./sw.js?v=" + STAMP + "'") >= 0, 'main.js registers ./sw.js?v=' + STAMP);
  say(/<link[^>]*rel=["']manifest["'][^>]*href=["'][^"']*manifest\.webmanifest\?v=/.test(read(join(TINT, 'index.html'))), 'the page links the manifest at a stamped address');
  let manifest = null;
  try { manifest = JSON.parse(read(join(TINT, 'manifest.webmanifest'), 'null')); } catch (e) { manifest = null; }
  const png = f => { const p = join(TINT, f.replace(/\?.*$/, '')); if (!existsSync(p)) return null; const b = readFileSync(p); return b.slice(1, 4).toString() === 'PNG' ? [b.readUInt32BE(16), b.readUInt32BE(20)] : null; };
  ...
  say(!!manifest && manifest.name === 'Tint' && manifest.start_url === './' && manifest.display === 'standalone' && missing.length === 0,
...
const server = createServer((req, res) => {
  ...
  res.end(clean === '/tint/sw.js' ? readFileSync(p, 'utf8') + swExtra : readFileSync(p));
});
...
await page.goto(base + '/tint/index.html?seed=4242&', { waitUntil: 'load', timeout: 60000 });
...
/* 2 */
{
  ...
  const lost = Array.from(new Set(sub)).filter(u => !inCache.has(u)).concat(Array.from(inCache).some(u => u.split('?')[0] === '/tint/index.html') ? [] : ['/tint/index.html']);
...
/* 3 */
{
  await page.evaluate(async () => { await caches.open('tint-shell-19990101a'); await caches.open('brim-shell-kept'); });
  swExtra = '\n/* a deploy */\n';
  ...
  const keys = await page.evaluate(() => new Promise(resolve => {
    const t0 = performance.now();
    const tick = () => caches.keys().then(k => { if (k.indexOf('tint-shell-19990101a') < 0 || performance.now() - t0 > 10000) resolve(k); else setTimeout(tick, 150); });
    tick();
  }));
  say(keys.indexOf('tint-shell-19990101a') < 0 && keys.indexOf('brim-shell-kept') >= 0 && keys.indexOf(CACHE) >= 0,
    'a worker installing again deletes older tint caches and leaves every other cache alone (' + JSON.stringify(keys) + ')');
}
...
/* 4 */
{
  ...
  try {
    ...
    await page.waitForFunction(() => window.TINT.phase() === 'answer', { timeout: 15000 });
    await page.evaluate(() => document.getElementById('same').click());
    await page.waitForFunction(() => window.TINT.pourDone(), { timeout: 30000 });
    played = JSON.stringify({ choice: (await page.evaluate(() => window.TINT.results[0])).choice });
  } catch (e) {
    const seen = await page.evaluate(() => ({ url: location.pathname, controlled: !!(navigator.serviceWorker && navigator.serviceWorker.controller), ready: !!window.TINT })).catch(err => ({ unreadable: err.message.split('\n')[0] }));
    played = 'failed: ' + e.message.split('\n')[0] + '; the page shows ' + JSON.stringify(seen);
  }
  say(/"choice":"same"/.test(played), 'with the server down, a reload plays a SAME COLOUR round from the cache (' + played + ')');
  ...
  try {
    ...
    await page.waitForFunction(() => window.TINT.phase() === 'answer', { timeout: 15000 });
    await page.evaluate(() => document.getElementById('scales-no').click());
    await page.waitForFunction(() => window.TINT.results.length > 0, { timeout: 10000 });
    framed = await page.evaluate(() => window.TINT.results[0].mode);
  } catch (e) { framed = 'failed: ' + e.message.split('\n')[0]; }
  say(framed === 'scales', 'and a reload opens DOES IT SCALE by its door and answers an item (' + framed + ')');
}
