/* TINT service worker: the offline shell (plans/tint/HANDOFF-TINT.md P3; CATALOG-PLAN D2; the shape and scars of
   the catalog's earlier workers).

   The fleet's host law, learned the hard way:
   - caches.keys() is origin wide: only tint- caches are ever deleted, or this worker wipes every other game.
   ...
   - SHELL_VERSION moves with TINT's stamp, and so does the ?v= on the registration in main.js.
   - ⛔ a module imported at two addresses must be precached at both: core.js imports pure.js and STAMP.js at CORE's stamp;
     TINT's modules import core.js and each other at TINT's stamp, never pure.js directly. */

const SHELL_VERSION = 'tint-shell-20260916g';
...
const SHELL_ASSETS = [
  ...
  './index.html',
  './main.js?v=20260916g',
  './engine.js?v=20260916g',
  './colour.js?v=20260916g',
  './render.js?v=20260916g',
  './content.js?v=20260916g',
  './config.js?v=20260916g',
  './manifest.webmanifest?v=20260916g',
  './icon-192.png',
  './icon-512.png',
  './icon-maskable-512.png',
  '../math/core/core.css?v=20260916g',
  '../math/core/core.js?v=20260916g',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k.indexOf('tint-') === 0 && k !== SHELL_VERSION).map(k => caches.delete(k))))
...
function offlinePage() {
  return new Response(
    ...
    + '<body style="margin:0;height:100vh;display:flex;align-items:center;justify-content:center;background:#efe6d8;'
    + 'color:#2b2a26;font-family:system-ui,sans-serif;text-align:center"><p>Tint needs one visit with the internet first.</p></body>',
