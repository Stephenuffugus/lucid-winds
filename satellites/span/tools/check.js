/* The one command that says whether SPAN is shippable.
 *
 *   node tools/check.js            everything
 *   node tools/check.js --fast     skips the slow gates and SAYS so
 *
 * Shape copied from satellites/math/core/tools/check.js: fast mode SKIPS a slow gate and never
 * shrinks it; a failure prints the FAILING lines, not the tail; stderr is captured; and a run that
 * left the browser gates out refuses to say ALL GATES PASSED. An ES module, because
 * satellites/span/package.json says "type": "module" so Node reads engine.js as one.
 * Browser gates are run in the foreground, one per call, on this box (plans/span/HANDOFF-SPAN.md 0.4).
 */
