...
let parsed = true, why = '';
try { createScript(JS, { filename: 'swell-inline.js' }); }
...
const shellM = SW.match(/const SHELL_VERSION = "swell-shell-([^"]+)"/);
...
say(!!shellM && !!stampM && shellM[1] === stampM[1],
  'and sw.js carries the same one' + (shellM ? ': swell-shell-' + shellM[1] : ': sw.js has no swell-shell version'));
const swOwn = SW.indexOf('swell-') >= 0 && SW.indexOf('deepwell') < 0;
...
/* case insensitive: the law is the brand appears and is singular, not that it
   is shouted. Fathom sets it in caps, Swell sets it in a serif. */
