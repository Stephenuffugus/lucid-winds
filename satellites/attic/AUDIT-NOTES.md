...
- Sep 05 (later), records era depth: `priceSticker` lives in sleeve-render.js (exported; object-render.js calls `S.priceSticker`), the sleeve ERA_LOOK carries `ta`/`ls`, `fit()` takes the spacing, the sleeve's cream oval is gone. Gate era group covers RECORD now. Pins 20260905f. 149 ok.
- Sep 05 (later), the shelf seat: `.shObj[data-cls]` translateY per class (GAME 13%, LUNCHBOX 12.5%), the gate measures each card's drawn bottom (getBBox, capped at 300 because the dust paints past the square) against its plank. Album sub lines fit. Pins 20260905g. 150 ok.
- Sep 05 (late), flip card foot: `#fcFront` is a flex column, `.fcFoot` (provenance + class/era/year/found) at margin-top:auto. Gate group 'the flip card front is used to its foot'. 152 ok.
