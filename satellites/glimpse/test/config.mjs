#!/usr/bin/env node
/* GLIMPSE's teacher's links (plans/glimpse/HANDOFF-GLIMPSE.md P3; the shape of the catalog's earlier config gates). The
 * builder at satellites/math/config registers GLIMPSE with the SAME schema the page parses (satellites/glimpse/config.js), so a
 ...
 *   1. the builder has an entry for GLIMPSE going to ../glimpse/
 ...
 *   4. main.js imports GLIMPSE_SCHEMA from ./config.js and hands it to parseConfig
 *   5. in a browser: a link of the builder's defaults and a link of other values play the mode, the flash length and the run
 *      length the link asked for, and the page's first round (after its door) is the one Node deals for that mode at the seed
 */
...
import { GLIMPSE_SCHEMA } from '../config.js';
import { dealSession } from '../engine.js';

const GLIMPSE = join(MATH, '..', 'glimpse');
...
const entry = GAMES.glimpse;
say(!!entry && entry.path === '../glimpse/', 'the builder has an entry for GLIMPSE going to ../glimpse/' + (entry ? ' (' + entry.path + ')' : ' (none)'));
if (entry) {
  const differ = [];
  for (const [k, rule] of Object.entries(entry.schema)) {
    const mine = GLIMPSE_SCHEMA[k];
  ...
  const unoffered = Object.keys(GLIMPSE_SCHEMA).filter(k => k !== 'seed' && !entry.schema[k]);
  ...
  for (const [k, rule] of Object.entries(entry.schema)) {
    for (const v of valuesOf(rule)) {
      const q = buildQuery({ [k]: v }, entry.schema), back = parseConfig(q, GLIMPSE_SCHEMA)[k];
...
{
  const src = readFileSync(join(GLIMPSE, 'main.js'), 'utf8');
  const imports = /import\s*\{[^}]*\bGLIMPSE_SCHEMA\b[^}]*\}\s*from\s*['"]\.\/config\.js\?v=/.test(src);
  const parses = /parseConfig\(\s*location\.search\s*,\s*GLIMPSE_SCHEMA\s*\)/.test(src);
  say(imports && parses, 'main.js imports GLIMPSE_SCHEMA from ./config.js and hands it to parseConfig' + (imports ? '' : ' (it does not import it)') + (parses ? '' : ' (it parses something else)'));
}
...
if (entry) {
  ...
  for (const [label, values] of [['the builder\'s defaults', defaults], ['other values', others]]) {
    ...
    const opened = await open(s.base, Object.assign({}, SIZES[1], { path: '/glimpse/index.html' + (q ? q + '&' : '?'), ready: 'window.GLIMPSE && window.GLIMPSE.ready' }));
    await sleep(150);
    const played = await opened.page.evaluate(() => window.GLIMPSE.config());
    const wrong = Object.keys(values).filter(k => !played || String(played[k]) !== String(values[k]));
    say(!!played && wrong.length === 0, 'GLIMPSE plays what a link of ' + label + ' asked for (asked ' + JSON.stringify(values) + ', the page plays ' + JSON.stringify(played) + ')');
    /* the page deals when a door opens: a link with a mode shows its one door, one without shows FLASH's first */
    await opened.page.evaluate(() => document.getElementById('start').click());
    await opened.page.waitForFunction(() => !!window.GLIMPSE.current(), { timeout: 10000 });
    const seed = GLIMPSE_SCHEMA.seed.default;
    const first = dealSession(rng(seed >>> 0), { mode: values.mode, tier: 0, session: 0 })[0];
    const got = await opened.page.evaluate(() => ({ mode: document.body.dataset.mode, round: window.GLIMPSE.current() }));
    const key = x => JSON.stringify(x.mode === 'spread' ? [x.a.dots, x.b.dots] : [x.count, x.dots]);
    say(got.mode === values.mode && key(got.round) === key(first), label + ': the page is in that mode and its first round is the one Node deals for it at the seed');
