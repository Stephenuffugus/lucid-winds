/* GLIMPSE service worker: the offline shell (plans/glimpse/HANDOFF-GLIMPSE.md P3; CATALOG-PLAN D2; the shape and scars of
   the catalog's earlier workers).

   The fleet's host law, learned the hard way:
   - caches.keys() is origin wide: only glimpse- caches are ever deleted, or this worker wipes every other game.
   ...
   - SHELL_VERSION moves with GLIMPSE's stamp, and so does the ?v= on the registration in main.js.
   - ⛔ a module imported at two addresses must be precached at both: core.js imports pure.js and STAMP.js at CORE's stamp;
     GLIMPSE's own modules import only core.js and each other, at GLIMPSE's. */

const SHELL_VERSION = 'glimpse-shell-20260916c';
...
const SHELL_ASSETS = [
  ...
  './index.html',
  './main.js?v=20260916c',
  './engine.js?v=20260916c',
  './content.js?v=20260916c',
  './config.js?v=20260916c',
  './render.js?v=20260916c',
  './sprites.js?v=20260916c',
  './journal.js?v=20260916c',
  './manifest.webmanifest?v=20260916c',
  './icon-192.png',
  './icon-512.png',
  './icon-maskable-512.png',
  '../math/core/core.css?v=20260916c',
  '../math/core/core.js?v=20260916c',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k.indexOf('glimpse-') === 0 && k !== SHELL_VERSION).map(k => caches.delete(k))))
...
function offlinePage() {
  return new Response(
    ...
    + '<body style="margin:0;height:100vh;display:flex;align-items:center;justify-content:center;background:#10161f;'
    + 'color:#e9eef2;font-family:system-ui,sans-serif;text-align:center"><p>Glimpse needs one visit with the internet first.</p></body>',
