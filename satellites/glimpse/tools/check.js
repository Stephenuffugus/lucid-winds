/* The one command that says whether GLIMPSE is shippable.
 *
 *   node tools/check.js            everything
 *   node tools/check.js --fast     skips the slow gates and SAYS so
 *
 * Shape copied from satellites/crease/tools/check.js: fast mode SKIPS a slow gate and never shrinks it; a failure prints
...
const GATES = [
  ...
  { name: 'generator', cmd: ['test/generator.mjs'], need: 'GENERATOR OK' },
...
/* Mode 4 MORE is parked (plans/glimpse/HANDOFF-GLIMPSE.md 3.3); its decorrelation gate is its ship gate and went BLOCKED (3.14).
   ...
   and 5). Mode 4 does not ship while this line says red. */
const PARKED_GATES = [
  { name: 'more (Mode 4)', cmd: ['test/more.mjs'], need: 'MORE OK' }
];
...
for (const g of PARKED_GATES) {
  let out = '', code = 0;
  try {
    out = execFileSync('node', g.cmd, { cwd: ROOT, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024, timeout: 30 * 60 * 1000, killSignal: 'SIGKILL', stdio: ['ignore', 'pipe', 'pipe'] });
  } catch (e) { out = (e.stdout || '') + (e.stderr || ''); code = 1; }
  const green = code === 0 && out.indexOf(g.need) >= 0;
  console.log('\nPARKED, not counted: ' + g.name + ' ' + (green ? 'green (Mode 4 could ship on Stephen\'s call)' : 'RED, BLOCKED (Mode 4 does not ship)'));
  if (!green) console.log(out.split('\n').filter(l => /FAIL/.test(l)).map(l => '  ' + l.trim().slice(0, 200)).join('\n'));
}
if (skipped.length) {
