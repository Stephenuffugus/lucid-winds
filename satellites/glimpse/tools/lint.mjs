#!/usr/bin/env node
/* GLIMPSE's laws, checked against the files a browser loads from satellites/glimpse/ (everything but test/, tools/, docs/
 * and package.json). Shape from satellites/brim/tools/lint.mjs.
 *
 *   node tools/lint.mjs
 *
 * What it asserts (plans/glimpse/HANDOFF-GLIMPSE.md P0), each watched to fail on a planted line:
 *   1. every runtime module parses as an ES module
 *   2. nothing a browser loads is a .mjs
 *   3. every relative import and local asset carries ?v= plus GLIMPSE's stamp (STAMP.js), CORE's included
 ...
 *   9. GL2 as the code is written: no sound is played from inside a loop, a forEach or a map, so a flash can never make one
 *      sound per firefly (the handoff's easiest fatal mistake)
 */
...
const files = walk(GLIMPSE, []);
const rel = p => relative(GLIMPSE, p);
...
/* 3 */
const stampFile = join(GLIMPSE, 'STAMP.js');
const STAMP = existsSync(stampFile) ? (read(stampFile).match(/export const STAMP = '([0-9]{8}[a-z])'/) || [])[1] : null;
say(!!STAMP, 'STAMP.js names GLIMPSE\'s one stamp' + (STAMP ? ': ' + STAMP : ''));
{
  const specs = [];
...
/* 5 */
{
  const words = assertForbiddenStrings(GLIMPSE);
  say(words.ok, 'none of the catalog\'s forbidden words anywhere (' + words.detail + ')');
  const gum = assertNoGetUserMedia(GLIMPSE);
...
{
  const copy = [];
  const content = join(GLIMPSE, 'content.js');
...
/* 9: GL2. A sound played inside a loop is a sound per thing counted. Every call to audio.play or sound( is found, and the
   body of every for, while, forEach, map, reduce and some around it is read; none may hold one. */
{
  ...
  say(bad.length === 0, 'GL2: no sound is played from inside a loop, so a flash can never sound once per firefly' + (bad.length ? ': ' + bad.join('; ') : ''));
}

/* 8: the sprite table, once there is one */
{
  const p = join(GLIMPSE, 'sprites.js');
  if (existsSync(p)) {
    let table = null, why = '';
