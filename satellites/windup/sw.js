/* WINDUP service worker — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete windup-* caches, or
...
const SHELL_VERSION = "windup-shell-20260906e";
const SFX_CACHE     = "windup-unused-v1"; // no recorded audio: every sound is synthesised
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("windup-") === 0 && k !== SHELL_VERSION && k !== SFX_CACHE
...
function offlineFallback() {
  return new Response(
    ...
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#E8C97A'>WINDUP</h1>" +
