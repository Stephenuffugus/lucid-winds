...
| `draw` | 54 assertions with real pointers: taps nine pixels off centre, the pick radius from both sides, the label, undo, THE CLOSE (the first star tapped again: three stars, THREE lines, features() says loop, the line pulses, the label says so, and no line of the shape runs through the label), a branch counted in LINES not stars, UNDO taking back the last thing done twice, a two finger pinch to both limits, a drag that turns the dome by the width of the drag, a typed name, the myth carrying a SHAPE.loop line, and the almanac entry holding the three Hipparcos numbers and three lines | `PICK_PX` at 2 |
