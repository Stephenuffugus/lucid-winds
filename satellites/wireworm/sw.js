/* WIREWORM service worker. All audio is synthesised, so there is no media cache — offline shell, single app file.

   Host law (learned the hard way across this fleet):
   - caches.keys() is ORIGIN-wide. Only ever delete wireworm-* caches, or
...
const SHELL_VERSION = "wireworm-shell-v1";
    ...
    caches.keys()
      .then(keys => Promise.all(
        keys.filter(k =>
          ...
          k.indexOf("wireworm-") === 0 && k !== SHELL_VERSION
...
function offlineFallback() {
  return new Response(
    ...
    "<body style='background:#0a0b0f;color:#e7ecdd;font-family:system-ui;display:flex;align-items:center;" +
    "justify-content:center;height:100vh;margin:0;text-align:center'><div><h1 style='color:#a3e635'>WIREWORM</h1>" +
