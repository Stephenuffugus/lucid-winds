...
function mark(scale) {
  const s = scale;
  /* the game in one picture: the brass arm and bob hanging over a rosette the
     pen has drawn. No letter, nothing that stops reading at 48 px. */
  const loops = [];
  for (let i = 0; i < 5; i++) {
    const rot = i * 36;
    loops.push(`<ellipse cx="256" cy="330" rx="150" ry="62" fill="none" stroke="${LINE}"
      stroke-width="13" opacity="${(0.9 - i * 0.09).toFixed(2)}"
      transform="rotate(${rot} 256 330)"/>`);
  }
  return `
  <g transform="translate(256,256) scale(${s}) translate(-256,-256)">
    <rect x="54" y="196" width="404" height="268" rx="18" fill="#FBF6E8"
          stroke="${INK}" stroke-width="9" opacity=".92"/>
    ${loops.join('')}
    <path d="M256 40 L256 232" stroke="${BRASS}" stroke-width="22" stroke-linecap="round"/>
    <circle cx="256" cy="52" r="18" fill="${BRASS}"/>
    <circle cx="256" cy="256" r="34" fill="${BRASS}"/>
    <circle cx="246" cy="246" r="11" fill="#F6E6BC" opacity=".7"/>
  </g>`;
}
...
for (const [name, size, body] of shots) {
  ...
  if (lit.red < 0.004) throw new Error(name + ' has no drawing in it (' + (lit.red * 100).toFixed(2) + ' percent ink)');
