/* Slice 3D service worker — installability + offline art. NETWORK-FIRST (house rule),
    ...
    return new Promise(function (r) { setTimeout(r, 3000); }).then(function () {
      return caches.keys().then(function (keys) {
        return Promise.all(keys.map(function (k) {
          return (k.indexOf("slice3d-") === 0 && k !== CACHE) ? caches["delete"](k) : null;
        }));
      });
    });
  }));
...
function fromCache(req) {
  return caches.match(req, { ignoreSearch: true }).then(function (hit) {
    ...
    if (req.mode === "navigate") {
      return caches.match("/satellites/slice-3d/", { ignoreSearch: true }).then(function (shell) {
