...
(async () => {
  ...
  try {
    ...
    const idr = await idPage.evaluate(async () => {
      ...
      /* the chip must be fully INSIDE its row: a shrinking scroll row clipped the chips to a third and the chip's own rect never knew */
      const rowR = document.getElementById('x-chips').getBoundingClientRect(), c0 = chips[0].getBoundingClientRect();
      const chipInRow = c0.top >= rowR.top - 1 && c0.bottom <= rowR.bottom + 1;
      ...
      return { nameSame, loreKept, fourLines, fxN: fx.length, partOk, n, wingLeak, keysUsed: Object.keys(keys).length, chips: chips.length, chipH, headH, oneRow, chipInRow, total, filterOk, detail, back, famLine: famLine ? famLine.textContent : null, famRow,
    ...
    ok(idr.chips === 5 && idr.chipH >= 48 && idr.oneRow && idr.chipInRow, 'five family chips over the Bugdex, 48px tall, on one row, fully inside the row', { chips: idr.chips, chipH: idr.chipH, oneRow: idr.oneRow, chipInRow: idr.chipInRow });
