/* NOTCH service worker: the offline shell (plans/notch/HANDOFF-NOTCH.md P3; CATALOG-PLAN D2; the shape and scars of
   the catalog's earlier workers).

   The fleet's host law, learned the hard way:
   - caches.keys() is origin wide: only notch- caches are ever deleted, or this worker wipes every other game.
   ...
   - SHELL_VERSION moves with NOTCH's stamp, and so does the ?v= on the registration in main.js.
   - ⛔ a module imported at two addresses must be precached at both: core.js imports pure.js and STAMP.js at CORE's stamp;
     NOTCH's engine.js ALSO imports pure.js at NOTCH's stamp, so pure.js is precached at both addresses (CREASE's scar). */

const SHELL_VERSION = 'notch-shell-20260916e';
...
const SHELL_ASSETS = [
  ...
  './index.html',
  './main.js?v=20260916e',
  './engine.js?v=20260916e',
  './pieces.js?v=20260916e',
  './project.js?v=20260916e',
  ...
  '../math/core/core.css?v=20260916e',
  '../math/core/core.js?v=20260916e',
  '../math/core/pure.js?v=20260916e',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k.indexOf('notch-') === 0 && k !== SHELL_VERSION).map(k => caches.delete(k))))
...
function offlinePage() {
  return new Response(
    ...
    + '<body style="margin:0;height:100vh;display:flex;align-items:center;justify-content:center;background:#2a2119;'
    + 'color:#efe3cf;font-family:system-ui,sans-serif;text-align:center"><p>Notch needs one visit with the internet first.</p></body>',
