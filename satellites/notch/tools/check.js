/* The one command that says whether NOTCH is shippable.
 *
 *   node tools/check.js            everything
 *   node tools/check.js --fast     skips the slow gates and SAYS so
 *
 * Shape copied from satellites/hush/tools/check.js: fast mode SKIPS a slow gate and never shrinks it; a failure prints
...
const GATES = [
  ...
  { name: 'shapes', cmd: ['test/shapes.mjs'], need: 'SHAPES OK' },
  { name: 'project', cmd: ['test/project.mjs'], need: 'PROJECT OK' },
  { name: 'engine', cmd: ['test/engine.mjs'], need: 'ENGINE OK' }
];

const BROWSER_GATES = [
  { name: 'turn', cmd: ['test/turn.mjs'], need: 'TURN OK' },
  { name: 'reveal', cmd: ['test/reveal.mjs'], need: 'REVEAL OK' },
  { name: 'numerals', cmd: ['test/numerals.mjs'], need: 'NUMERALS OK' },
  { name: 'pace', cmd: ['test/pace.mjs'], need: 'PACE OK' },
  { name: 'find', cmd: ['test/find.mjs'], need: 'FIND OK' },
