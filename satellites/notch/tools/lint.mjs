#!/usr/bin/env node
/* NOTCH's laws, checked against the files a browser loads from satellites/notch/ (everything but test/, tools/, docs/ and
 * package.json). Shape from satellites/hush/tools/lint.mjs.
 *
 *   node tools/lint.mjs
 *
 * What it asserts (plans/notch/HANDOFF-NOTCH.md P0), each watched to fail on a planted line:
 *   1. every runtime module parses as an ES module
 *   2. nothing a browser loads is a .mjs
 *   3. every relative import and local asset carries ?v= plus NOTCH's stamp (STAMP.js), CORE's included
 *   4. engine.js names no document, window, Date, performance, Math.random or timer
 ...
 *   8. the sprite table, once sprites.js exists: sixteen colours, literal rows of one width
 *   9. no sound is played from inside a loop, a forEach or a map (NOTCH's GL2 kept: a pose is one event, one sound)
 *  10. N7: no digit in any string a child reads (content.js's strings, the pages' text, aria labels and titles); N5: no word
 *      that makes spatial ability fixed (talent, gifted, natural, spatial type); no camera, microphone or motion sensor
 */
...
const files = walk(NOTCH, []);
const rel = p => relative(NOTCH, p);
...
/* 3 */
const stampFile = join(NOTCH, 'STAMP.js');
const STAMP = existsSync(stampFile) ? (read(stampFile).match(/export const STAMP = '([0-9]{8}[a-z])'/) || [])[1] : null;
say(!!STAMP, 'STAMP.js names NOTCH\'s one stamp' + (STAMP ? ': ' + STAMP : ''));
{
  const specs = [];
...
/* 4 */
for (const name of ['engine.js']) {
  const p = join(NOTCH, name);
...
/* 5 */
{
  const words = assertForbiddenStrings(NOTCH);
  say(words.ok, 'none of the catalog\'s forbidden words anywhere (' + words.detail + ')');
  const gum = assertNoGetUserMedia(NOTCH);
...
{
  const copy = [];
  const content = join(NOTCH, 'content.js');
...
/* 9: GLIMPSE's GL2, kept. A sound played inside a loop is a sound per thing. Every call to audio.play or sound( is found, and the
   body of every for, while, forEach, map, reduce and some around it is read; none may hold one. */
{
  const bad = [];
  for (const p of JS) {
    const code = stripComments(read(p));
    const loops = [];
    for (const m of code.matchAll(/\b(for|while)\s*\([^)]*\)\s*\{|\.(forEach|map|reduce|some|every|filter)\s*\([^)]*=>\s*\{/g)) {
      let i = m.index + m[0].length, depth = 1;
      for (; i < code.length && depth; i++) { if (code[i] === '{') depth++; else if (code[i] === '}') depth--; }
      loops.push([m.index, i]);
    }
    for (const m of code.matchAll(/\b(audio\.play|sound)\s*\(/g)) {
      if (/function\s+$/.test(code.slice(Math.max(0, m.index - 10), m.index))) continue;
      if (loops.some(([a, b]) => m.index > a && m.index < b)) bad.push(rel(p) + ' plays a sound inside a loop: ' + code.slice(m.index, code.indexOf('\n', m.index)).trim());
    }
  }
  say(bad.length === 0, 'no sound is played from inside a loop, so one seat is one thunk' + (bad.length ? ': ' + bad.join('; ') : ''));
}

/* 8: the sprite table, once there is one */
{
  const p = join(NOTCH, 'sprites.js');
  if (existsSync(p)) {
    let table = null, why = '';
...
/* 10: N7, N5 and no sensors */
{
  const motion = [];
  for (const p of JS.concat(HTML)) if (/getUserMedia|devicemotion|DeviceMotionEvent|deviceorientation/i.test(stripComments(read(p)))) motion.push(rel(p));
  say(motion.length === 0, 'no camera, microphone or motion sensor is asked for anywhere a browser loads' + (motion.length ? ': ' + motion.join(', ') : ''));
  const strings = [];
  const content = join(NOTCH, 'content.js');
  if (existsSync(content)) for (const m of stripComments(read(content)).matchAll(/(['"`])((?:\\.|(?!\1)[^\\])*)\1/g)) strings.push(m[2]);
  for (const p of HTML) {
    const text = read(p).replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<!--[\s\S]*?-->/g, ' ');
    for (const m of text.matchAll(/(?:aria-label|title|alt)=["']([^"']*)["']|>([^<]+)</g)) strings.push((m[1] || m[2] || '').trim());
  }
  /* a colour, a stamp or a stamped address in content.js is not copy; an entity like &#9654; is a glyph, not a numeral */
  const readable = strings.filter(t => !/^#[0-9a-f]{3,8}$/i.test(t) && !/^\d{8}[a-z]$/.test(t) && !/\?v=\d{8}[a-z]$/.test(t)).map(t => t.replace(/&#\d+;/g, ''));
  const digits = readable.filter(t => /[0-9]/.test(t));
  say(strings.length > 0 && digits.length === 0, 'N7: no digit in any string a child reads' + (digits.length ? ': ' + digits.map(d => JSON.stringify(d)).join(', ') : ' (' + readable.length + ' strings)'));
  const fixed = readable.filter(t => /\b(talent\w*|gifted|natural\w*|spatial type|born with|smart)\b/i.test(t));
  say(fixed.length === 0, 'N5: no string makes spatial ability fixed' + (fixed.length ? ': ' + fixed.map(d => JSON.stringify(d)).join(', ') : ''));
}
