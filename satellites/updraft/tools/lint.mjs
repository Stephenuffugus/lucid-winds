...
let parsed = true, why = '';
try { createScript(JS, { filename: 'updraft-inline.js' }); }
...
const shellM = SW.match(/const SHELL_VERSION = "updraft-shell-([^"]+)"/);
...
say(!!shellM && !!stampM && shellM[1] === stampM[1],
  'and sw.js carries the same one' + (shellM ? ': updraft-shell-' + shellM[1] : ': sw.js has no updraft-shell version'));
const swOwn = SW.indexOf('updraft-') >= 0 && SW.indexOf('fathom') < 0;
