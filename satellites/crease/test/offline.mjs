#!/usr/bin/env node
/* CREASE offline: the installable shell (plans/crease/HANDOFF-CREASE.md P3; CATALOG-PLAN D2, the service worker header law:
 * delete only your own caches, every fetch settles a real Response, navigations refetch no-cache, the shell's version moves
 * with the stamp). The shape and scars of satellites/yonder/test/offline.mjs.
 *
 ...
 *   1. sw.js names its cache crease-shell-<stamp>, main.js registers ./sw.js?v=<stamp>, and the page links the manifest
 *   2. straight after a first visit, the worker's cache holds every address the page asked for (read off the requests)
 *   3. a worker installing again deletes older crease caches and leaves every other cache on the origin alone
 *   4. with the server DOWN, a reload plays a FREEHAND round from the cache (the clip down, the reveal done), and a reload
 *      opens CREASE mode by its door and folds the strip
...
const ROOT = join(MATH, '..'), CREASE = join(ROOT, 'crease');
const { fails, say } = reporter();
const STAMP = (readFileSync(join(CREASE, 'STAMP.js'), 'utf8').match(/STAMP = '([0-9]{8}[a-z])'/) || [])[1];
const CACHE = 'crease-shell-' + STAMP;
...
{
  const sw = read(join(CREASE, 'sw.js'));
  const version = (sw.match(/SHELL_VERSION\s*=\s*['"]([^'"]+)['"]/) || [])[1];
  say(version === CACHE, 'sw.js names its cache for CREASE\'s stamp (' + (version || 'no SHELL_VERSION') + ' for ' + CACHE + ')');
  const reg = read(join(CREASE, 'main.js')).indexOf("register('./sw.js?v=" + STAMP + "'") >= 0;
  say(reg, 'main.js registers ./sw.js?v=' + STAMP);
  say(/<link[^>]*rel=["']manifest["'][^>]*href=["'][^"']*manifest\.webmanifest\?v=/.test(read(join(CREASE, 'index.html'))), 'the page links the manifest at a stamped address');
  let manifest = null;
  try { manifest = JSON.parse(read(join(CREASE, 'manifest.webmanifest'), 'null')); } catch (e) { manifest = null; }
  const png = f => { const p = join(CREASE, f.replace(/\?.*$/, '')); if (!existsSync(p)) return null; const b = readFileSync(p); return b.slice(1, 4).toString() === 'PNG' ? [b.readUInt32BE(16), b.readUInt32BE(20)] : null; };
  ...
  say(!!manifest && manifest.name === 'Crease' && manifest.start_url === './' && manifest.display === 'standalone' && missing.length === 0,
...
const server = createServer((req, res) => {
  ...
  res.end(clean === '/crease/sw.js' ? readFileSync(p, 'utf8') + swExtra : readFileSync(p));
});
...
await page.goto(base + '/crease/index.html?seed=4242&', { waitUntil: 'load', timeout: 60000 });
...
/* 2 */
{
  ...
  const lost = Array.from(new Set(sub)).filter(u => !inCache.has(u)).concat(Array.from(inCache).some(u => u.split('?')[0] === '/crease/index.html') ? [] : ['/crease/index.html']);
...
/* 3 */
{
  await page.evaluate(async () => { await caches.open('crease-shell-19990101a'); await caches.open('yonder-shell-kept'); });
  swExtra = '\n/* a deploy */\n';
  ...
  const keys = await page.evaluate(() => new Promise(resolve => {
    const t0 = performance.now();
    const tick = () => caches.keys().then(k => { if (k.indexOf('crease-shell-19990101a') < 0 || performance.now() - t0 > 10000) resolve(k); else setTimeout(tick, 150); });
    tick();
  }));
  say(keys.indexOf('crease-shell-19990101a') < 0 && keys.indexOf('yonder-shell-kept') >= 0 && keys.indexOf(CACHE) >= 0,
    'a worker installing again deletes older crease caches and leaves every other cache alone (' + JSON.stringify(keys) + ')');
}
...
/* 4 */
{
  ...
  try {
    ...
    await sleep(250);
    await page.evaluate(() => {
      const st = document.querySelector('#strip .lw-stone'), r = st.getBoundingClientRect(), y = r.top + r.height / 2, x0 = r.left + r.width / 2;
    ...
    await page.waitForFunction(() => window.CREASE.revealDone(), { timeout: 15000 });
    played = JSON.stringify({ pae: (await page.evaluate(() => window.CREASE.results[0])).pae });
  } catch (e) {
    const seen = await page.evaluate(() => ({ url: location.pathname, controlled: !!(navigator.serviceWorker && navigator.serviceWorker.controller), ready: !!window.CREASE })).catch(err => ({ unreadable: err.message.split('\n')[0] }));
    played = 'failed: ' + e.message.split('\n')[0] + '; the page shows ' + JSON.stringify(seen);
  }
  say(/"pae":\d/.test(played), 'with the server down, a reload plays a FREEHAND round from the cache (' + played + ')');
  let folded = 'not reached';
  try {
    await page.reload({ waitUntil: 'load', timeout: 20000 });
    await page.waitForFunction(READY, { timeout: 15000 });
    await tap(page, '#start-crease');
    await sleep(200);
    await tap(page, '#fold-more');
    await sleep(120);
    folded = await page.evaluate(() => document.getElementById('strip').dataset.parts);
  } catch (e) { folded = 'failed: ' + e.message.split('\n')[0]; }
  say(folded === '2', 'and a reload opens CREASE mode by its door and folds the strip (' + folded + ' parts)');
}
