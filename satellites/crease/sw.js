/* CREASE service worker: the offline shell (plans/crease/HANDOFF-CREASE.md P3; CATALOG-PLAN D2; YONDER's sw.js, whose shape
   and scars this copies).

   The fleet's host law, learned the hard way:
   - caches.keys() is origin wide: only crease- caches are ever deleted, or this worker wipes every other game.
   ...
   - SHELL_VERSION moves with CREASE's stamp, and so does the ?v= on the registration in main.js. The precache list
     names CORE's own modules by CORE's stamp, because that is the address core.js imports them from. */

const SHELL_VERSION = 'crease-shell-20260916a';
...
const SHELL_ASSETS = [
  ...
  './index.html',
  './main.js?v=20260916a',
  './engine.js?v=20260916a',
  './bank.js?v=20260916a',
  './content.js?v=20260916a',
  './config.js?v=20260916a',
  './render.js?v=20260916a',
  './draw.js?v=20260916a',
  './sprites.js?v=20260916a',
  './shelf.js?v=20260916a',
  './manifest.webmanifest?v=20260916a',
  './icon-192.png',
  './icon-512.png',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k.indexOf('crease-') === 0 && k !== SHELL_VERSION).map(k => caches.delete(k))))
...
function offlinePage() {
  return new Response(
    ...
    + '<body style="margin:0;height:100vh;display:flex;align-items:center;justify-content:center;background:#f3ecdc;'
    + 'color:#2b2721;font-family:system-ui,sans-serif;text-align:center"><p>Crease needs one visit with the internet first.</p></body>',
