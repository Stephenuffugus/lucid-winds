/* The one command that says whether CREASE is shippable.
 *
 *   node tools/check.js            everything
 *   node tools/check.js --fast     skips the slow gates and SAYS so
 *
 * Shape copied from satellites/span/tools/check.js: fast mode SKIPS a slow gate and never shrinks it; a failure prints
 * the FAILING lines, not the tail; stderr is captured; and a run that left the browser gates out refuses to say ALL GATES
 * PASSED. An ES module, because satellites/crease/package.json says "type": "module" so Node reads engine.js as one.
 * Browser gates are run in the foreground, one per call, on this box (plans/crease/HANDOFF-CREASE.md 0.4).
 */
...
const GATES = [
  ...
  { name: 'bank', cmd: ['test/bank.mjs'], need: 'BANK OK' },
...
const BROWSER_GATES = [
  ...
  { name: 'stack', cmd: ['test/stack.mjs'], need: 'STACK OK' },
