...
8. orb_loaded_ring — OPTIONAL / NEW-JUICE (needs a small code addition): a soft cream #e8dcc8 steady ready-glow HALO RING only (transparent center), a NEW loaded-orb glow to sit under the orb seated at the pod mouth. The engine's ONLY mouth ring today is the burst-armed pulse (index.html ~560), and sheet 02 cell 4 (burst_ring) already covers that — this is a distinct, dimmer, non-pulsing addition, so keep it visually quieter than burst_ring. Thin, luminous, no orb inside.
...
10. orb_prism — OPTIONAL / needs-code (like sheet 02's comet_head — the engine draws NO prism orb today): an iridescent rainbow star-berry (round) cycling rose→amber→teal→violet across its skin with a bright cream #ffe9a8 core and a strong cream rim-glow; a future premium flourish (candidate surface: a fancier Pollen-Burst projectile skin) that only appears if a new draw call is added.
...
16. orb_ghost — OPTIONAL / needs-code: a dim greyed/hollow round orb (moss #8a9178 outline, near-transparent fill). Nothing in the engine draws a ghost orb — wardrobe locks dim the WHOLE card (.wardcard.locked, index.html ~62), not individual orb slots — so this is a spare for a future locked/preview slot treatment that would need its own draw.
