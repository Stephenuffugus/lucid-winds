...
|---|---|---|---|---|---|
| Flock the World (Victory Pack) | Procurement Brochure, night sodium | 15 assets (3 end backdrops, 4 capstone icons, 4 path icons, 4 win seals) | Doc in 012Assets https://docs.google.com/document/d/1b_ZZGjcONwGHboNB0g7kEp0k5BXHSdz2EmQ8FNqczdo/edit · repo mirror art-asset-lists/flock-the-world/11-victory-pack.md | 2026-08-24 | LISTED. The four win doors, capstones and Paths to Victory panel ship today on placeholders (three endings reuse bg_end_win, capstones use the emoji fallback, path rows colour coded but iconless). Filenames ARE the wiring: end backdrops and seals key off the win reason in finish(), capstone icons load by node id via NODE_ART, path icons enter UI_ART. |
| Puppy Dash | Subway Surfers style dog park runner, toy-like chunky cel shading, **3/4 REAR view for every gameplay frame**, front 3/4 for menu and card frames | **3 sheets**, `art-asset-lists/puppy-dash/` — [00 art direction](../art-asset-lists/puppy-dash/00-art-direction.md), [01 character frames](../art-asset-lists/puppy-dash/01-character-frames.md), [02 world and backgrounds](../art-asset-lists/puppy-dash/02-world-backgrounds.md) | **Docs in 012Assets 2026-08-24:** Sheet 1 https://docs.google.com/document/d/1YaokW-d3xAFWKhgd_zoKQumQgoIUT2liQ_X1lEInXlc/edit · Sheet 2 https://docs.google.com/document/d/1J_aeuvMdslRV2jXf04FXzAf-Zwl2Yt5aCO2SsYhuSd8/edit | 2026-08-24 | LISTED, nothing generated yet. **Budget: CORE 24 frames per animal (96 for all four) makes every animal shippable; FULL is 46 each; plus 22 SHARED FX frames drawn once for everybody.** Sheet 1 is the frame list Stephen asked for: one canonical pose set with a shared rig (512 canvas, contact point fixed at y=488, 76% standing height, 12fps, no baked shadow) so one animal's sheet swaps for another's with zero code change. It adds BANK LEFT (3 frames, the missing lane change lean, engine flips it for right), LAND RECOVER, STUMBLE and the JETPACK set, all of which the bible lacks. Sheet 2 is the world: 7 layer parallax stack with rates, 6 biomes you run through at distance milestones, a 6 role prop library, the lane dividers the game has never had, and telegraph road decals. ⛔ **Sheet 2 carries the law: nothing decorative is ever on the running surface**, and a list of what obstacles must never be confusable with. ⚖ These two sheets SUPERSEDE `PUPPY_DASH_ART_BIBLE.md` sections 3 and 6 where they disagree; the bible's palette, resolution and naming rules still stand. Game ships today on the procedural vector fallback (`drawAnimal`/`drawObstacle`/`drawBiscuit` in `satellites/puppy-dash/index.html`) which must keep working once real art lands. The bible's obstacle list predates the P4 pass which added puddle (jump) and trash can (dodge); those two still want their own art rows. |
| Flock the World (refusal ending) | *Procurement Brochure*, but daylight | 1 image | Stephen, direct, 2026-08-23; raw kept at `satellites/flock-the-world/art-drop/Assets/refusal.png` | see commit | 2026-08-23 | **WIRED.** A sunny street, ordinary people, the camera pole grown over and turned into a bird box. Fills the one gap the pack had. It is the ONLY daylight image in the game, which is the point: everything else is a night city under sodium light. ⛔ It needed its own treatment: fading a bright picture back to 22% turned it into another dark smudge, so it is held at 62% with the IMAGE darkened instead, which keeps the sky blue and the trees green at a value pale text can sit on. |
| Flock the World (drop 1, screens) | *Procurement Brochure* | menu backdrop + 2 of 3 end backdrops | same zip | see commits | 2026-08-23 | **WIRED.** Menu = `bg_menu` (6c, the pyramid over a lit city). Win = `bg_end_win` (6h, a night city with every street lit, the exact brief). Coalition = `bg_end_coalition` (6k, the corporate lobby, institutional, matches "dragged into a hearing room"). ⚠ **REFUSAL HAS NO BACKDROP ON PURPOSE.** Sheet 7 asked for a daylight street where the camera poles have become bird boxes, and that image was never generated. Every candidate in the drop is a night city, a control room or a corporate lobby; none of them says "ordinary people won". ⚖ **ART REQUEST: one daylight image for the refusal ending.** `bg_warroom` (6i) and 6j remain unused. |
| Flock the World (drop 1, UI kit) | *Procurement Brochure* | 20 UI glyphs | same zip | see commits | 2026-08-23 | **WIRED, 19 of 20.** Six protest state chips, six region actions, six Ledger cell headers, and the two map bubbles preloaded into the canvas. `hq_marker` is cut but unplaced: the HQ is drawn as a radial glow rather than a marker, so there is nothing to swap. State chips are now built by one shared `stateChip()` used by the World cards, the map popover and the Ledger rows. |
| Flock the World (drop 1, rest) | *Procurement Brochure* | 16 synergy badges, 20 UI icons, 8 cards, 16 cast/resistance, 9 backdrops, wordmark | same zip | see commit | 2026-08-23 | **CUT. Badges, cast and backdrops WIRED**; UI kit and cards cut and manifested but not yet placed. ⛔ Sheet 10 was transposed like sheet 2: klein/anchor swapped and the countermeasure order different from the prompt. Caught by cutting with positional names and reading each picture. ⛔ Packed sheets (badges, cards, portraits) fused into 3 blobs until the dilation was made grid aware: bucketing already rejoins fragments, so a grid sheet needs almost none. All art converted to WebP: 5.4 MB of PNG became 0.8 MB, backdrops 20.5 MB became 316 KB. |
| Flock the World (drop 1, trees) | *Procurement Brochure* | 4 sheets to **46 node icons** + 3 UI plates | `Assets-20260823T045721Z-1-001.zip`, raw in Drive › Github › FTW › Assets and vaulted (see notes) | see commit below | 2026-08-23 | **WIRED.** Sheets 1 to 4 cut by `scripts/cut_ftw.py` into `satellites/flock-the-world/art/tree/`, 160px squares, magenta knocked out and de-fringed. All 46 render in the four skill trees. ⛔ Two icons on sheet 2 were transposed by the generator against the prompt order: check↔fusion and patrol↔quota. Caught by LOOKING at the contact sheet, not by the count, which matched at 12 either way. ⚠ `mesh`, `retail` and `drone` do not read at icon size, too sparse and too fine; they want a chunkier redraw. Remaining from this drop and NOT yet cut: sheet 5 badges (`5.png`), the wordmark (`7.png`), UI kit (`8.png`), cards (`9.png`), cast (`10.png`), and 11 backdrops (`6a` to `6k`). |
