...
| Cosmic Cadets — STYLE COLLECTIONS (6 sheets, 96 unlockables) | LISTED Jul 17 (+ uploaded to 012Assets Drive doc "COSMIC CADETS — Style Collections skins") | Prompts: `seed-flutter/10-style-collections-00.md` (index) + `11..16-collection-*.md`. UNLIKE 06-09 (recolors), these are the cadet drawn in 6 DRAMATICALLY DIFFERENT ART STYLES: Pixel Arcade, Neon Synthwave, Crayon Kids, Stained Glass (matches chess court), Claymation, Chrome Holo. Each sheet = a full 16-cell collection (4 comets+4 tails+4 buddies+4 stars) in one style. Fixed silhouette law so all wire onto the same draw calls; capstone star per style unlocks on owning all 4 of its comets. Wire pass extends SEEDS/TRAILS/COMPS + star roster; thresholds at wire time. |
