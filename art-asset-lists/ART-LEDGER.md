...
| Cosmic Cadets — STYLE COLLECTIONS (6 sheets) | **DROPPED Jul 17** → cutting. Raw: `art-asset-lists/Cosmic-Cadet-Extras-20260717.zip` + committed PNGs in `satellites/seed-flutter/art-drop/style-collections/` (clay/crayon/glass/metallic/neon/retro). | Prompts were `seed-flutter/10..16`. ⛔Generator did NOT follow the flat 4x4 spec: each sheet = comets as **4 skins × 3 POSES** (burst/streak/settle) in 2 rows, then tails/buddies/stars rows. Layouts VARY per sheet (square vs 1536×1024 landscape pack buddies/stars differently). ⛔crayon comet poses near-touching — cut clean. The 3 poses map 1:1 to the game's comet_cadet/_flap/_settle → wire as ANIMATED skins. Cutter `scripts/cut_cadet_styles.py`. |
