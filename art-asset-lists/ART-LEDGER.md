...
|---|---|---|---|---|---|
| Puppy Dash | Subway Surfers style dog park runner, toy-like chunky cel shading, 3/4 rear view gameplay + front 3/4 portraits | ~50 frames (the minimum viable set) | `satellites/puppy-dash/drop/PUPPY_DASH_ART_BIBLE.md` section 9 | 2026-08-24 | LISTED, nothing generated yet. Section 9 minimum set: Puppy run(6)/jump(6)/slide(4)/stumble(3)/portrait(1), biscuit spin(6), 4 obstacle stills (hydrant/cone/fence/banner), 4 environment tiles (path/grass/cloud/skyline), 5 UI pieces (logo/biscuit icon/play/retry/panel), 2 FX (collect pop 4, dust puff 4). Full style bible + locked palette + prompt stub in the same doc section 1. Game ships today on the procedural vector fallback (`drawAnimal`/`drawObstacle`/`drawBiscuit` in `satellites/puppy-dash/index.html`) which must keep working once real art lands — every draw seam is documented in `PUPPY_DASH_BUILD_SPEC.md` section 5 for the swap. Note the bible's obstacle list (hydrant/cone/fence/banner) predates this session's P4 content pass, which added puddle (jump) and trash can (dodge) procedurally; those two want their own art rows once section 9 is underway. |
