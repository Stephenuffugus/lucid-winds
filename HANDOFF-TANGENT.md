...
- [ ] **⚖️ FOR STEPHEN, and it changes what T4 is: gates cannot be made to require the deck.** T4's plan assumes that moving a gate off the ball's natural sweep makes parts mandatory. I tested that assumption directly and **it is false with the current mechanics**, so the levels are unchanged.
  - Measured per bearing, the deepest the ball reaches with a bare deck versus with one part placed anywhere on a coarse polar grid: **the largest gap anywhere on the deck is 10 units, against a gate capture radius of 17.4.** Parts deflect the ball; they do not open territory. So there is no gate position a bare deck cannot also reach.
  - Confirmed the hard way first: my initial attempt moved "Around the heavy"'s gate into the region a bare deck never enters, and `test/solve.js` immediately reported the system **UNSOLVABLE** with any single part. That is the tool paying for itself before the change was committed.
  - Why it happens: a gate is a POSITION in the deck frame, and position is dominated by the throttle, which is a radius dial. Parts change the route's shape, not the set of places the ball can be.
  - **Options, all yours.** (a) Gates that must be crossed in a given ORDER or DIRECTION, which parts genuinely control. (b) Gates that require a SPEED or a radius band at the crossing: the dish couples radius to speed for a bare deck, and a brake or booster is the only thing that decouples them, so this is the one that makes parts structurally necessary. (c) Accept that parts serve balance and score rather than access, and let the build phase be an optimisation layer rather than a gate. My recommendation is (b), because it uses the parts that already exist and it is one condition on the gate record.
  - Two tools now exist for whichever way you go: `node test/parts.js` (can a bare deck clear each system, and how often) and `node test/solve.js` (cheapest single part clear, and whether a system is solvable at all). Both understand a `needsParts:true` flag on a level, which currently no level can honour.
