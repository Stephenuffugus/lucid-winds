...
const PLANTS = {
  ...
  lint: [
    ...
    ['l3 a key twice', 'content.js', "  next: 'Next',", "  next: 'Next',\n  next: 'Onward',"]
  ]
};
const which = process.argv[2] || 'bank'; const only = process.argv[3];
for (const [name, file, from, to] of PLANTS[which].filter(p => !only || p[0].startsWith(only))) {
