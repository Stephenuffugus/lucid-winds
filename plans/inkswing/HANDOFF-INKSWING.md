...
- 2026-09-08 00:40 UTC, Fable: **THE SORT of his Sep 07 notes** (verified by one read-only agent per game and a
  ...
  **13 FAULT, confirmed and worse than he said:** undoLayer (`:1781-1791`) pops the last CREATED colour canvas and drops every throw of that colour; a one ink drawing is one layer so UNDO empties it; with irongall, oxblood, irongall it removes the MIDDLE throw. The button says UNDO, the toast says "That ink is off", the plan says layer = ink canvas; the label and the player disagree with both. A sheet is its throw list and redrawAll rebuilds from it, so per throw undo is pop plus redraw at today's cost. Gate: fling.mjs:85-113 tested UNDO only after TWO inks, the one case where colour and throw coincide. An hour with the one ink two throw assertion. 14 NEW WORK + Director call 59: the rig card click (`:2933-2941`) clears the sheet silently, by structure (rig belongs to the sheet, the link carries one rig byte); mixed rigs is a day and link v6; a toast before the clear is ten minutes regardless. 15 NEW WORK call 60: a throw strip along the bottom (highlight, remove; "adjust" needs his word). 16 KNOWN call 30, and the Twin (call 31) is LOCKED at 20 kept drawings so he has not seen it; DOUBLE_LINK2 k 0.7 from call 30 was never set (still 1). **17 FAULT, confirmed by re running the page's SIM block:** "the three point" is the Gimbal (third card, "3 axes"). flingToThrow (`:738-760`) feeds the throw's x to pendulum 0 and y to pendulum 1 always, but the Gimbal hangs pen x AND y from pendulum 0, so every slow release collapses onto the sheet's midline: bottom left release at (37, 615) starts the pen at (29, 398), 217 px straight up, the bob teleports the same frame. The Crossed Pair mirrors y (bottom left to top left), the Twin pulls to its home; the Single and Double Link are exact, which is why the Sep 06 edge gate passed. The code's own law `:731` "THE FLING IS MAPPED EXACTLY" and the sim assertion `:1137` are pinned to rig single. Half a day: a per rig inverse, the assertion over RIG_ORDER and every corner, a Gimbal bottom left release in fling.mjs. Side finding by reading: on the Twin a TAP on a resting bob is a throw (release measures reach from the sheet centre, not penHome).
