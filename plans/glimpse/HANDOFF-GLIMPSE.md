...
- SHOT p3-spread-answer-1366x768.png OPENED. The empty window is the flash having closed, which is the game, so it is not counted as a fault. Faults: (1) the window is a 360 px box centred on a 1366 px screen with roughly 500 px bare on each flank and 340 px below, so the page reads as mostly nothing; (2) the closed window still cannot be told from one that failed to load, at any width; (3) the three answers are about 55 px squares holding pips smaller than the punctuation in this sentence, on a screen with room for anything; (4) no words appear anywhere on the page.
- SHOT p3-flash-shown-412x915.png OPENED. Faults: (1) the four dots sit left of centre in a 715 px tall frame that is otherwise empty, so the thing to count is small and off axis inside a very large window; (2) the five answers wrap three then two with the second row nudged right, crooked at this width too; (3) the digits are mid grey on near black and the pips smaller than the digits, the weakest contrast in the catalog at every size; (4) the bottom 40 per cent of the screen is empty below the answers.
- SHOT p3-journal-412x915.png OPENED. Faults: (1) the one earned card sits in the top left corner of the frame, the corner disease at the tall size as well; (2) the frame itself floats in the middle of a 915 px screen with roughly 590 px of empty page above it; (3) the card is a pale grey rectangle with a few blue marks and no border of its own, so it reads as an image that failed to load; (4) the frame is a thin outline barely lighter than the page, so the journal has no body at any size.
- SHOT p3-frame-reveal-412x915.png OPENED, and it is the clearest GLIMPSE screen at any size: the ten frame is large, its cells are drawn plainly and six dots in a ten frame can genuinely be counted. Faults: (1) the eleven answers still speak two languages, loose pips for 1 to 6 and little grids for 0 and 7 to 10; (2) the chosen 0 wears a pale ring while the truth 6 is a numeral inside the frame, unconnected; (3) the digits are mid grey on near black while the revealed 6 is bright white, three text weights; (4) the answers wrap four, four, three so no row lines up with another.
- SHOT p3-doors-412x915.png OPENED, and the tall size makes the fault plainer: FOUR ways to play, not one word on any of them, and three of the four thumbnails are scattered blue pips that differ only in how many, so a child must count the pictures on the doors to guess what the doors do. The fourth is a little ten frame. Other faults: (1) the window above holds three large blue squares while the doors hold tiny pips, the same idea at two scales with nothing connecting them; (2) the top 65 per cent of a 915 px screen is empty; (3) the fourth door’s grid is the only one that reads at thumbnail size, which quietly makes it look like the important one.
- SHOT p3-flash-answer-412x915.png OPENED. The empty window is the flash having closed, which is the game. Faults: (1) the closed window still cannot be told from one that failed to load, at the fourth width as at the other three; (2) the window holds nothing across 700 px of screen while the answers are squeezed into a band beneath it; (3) the five answers wrap three then two with the second row nudged right, crooked; (4) the pips inside the buttons are about 8 px and sit below the digits, so the picture that should help a child count is the smallest thing on the page.
- SHOT p3-spread-answer-412x915.png OPENED. The empty window is the flash having closed. Faults: (1) the window keeps 710 px of a 915 px screen while holding nothing, and still cannot be told from one that failed to load; (2) the three answers are about 110 px squares holding pip clusters so alike they must be compared pip by pip; (3) those pips are pale grey on near black, the weakest contrast in the catalog; (4) no words appear anywhere on the page.
- SHOT p3-flash-reveal-1366x768.png OPENED. Faults: (1) the chosen 1 wears a pale ring while the truth 4 is a numeral inside the frame, the unconnected reveal at the fourth width; (2) the four dots sit in a clean two by two square while the pips on the answer buttons are scattered, so the one arrangement that would let a child match picture to number is thrown away on the buttons; (3) the digits are mid grey on near black, the weakest contrast in the catalog, while the revealed 4 is bright white; (4) go on sits far to the right and below the answers, aligned to nothing, on a page whose flanks are about 500 px of nothing each.
