...
- 2026-09-06 builder (fit centre and Swap, 15:52 to 16:52 UTC, shared tree, one gate lock): **both fit steps done, shot at 412x915 and 375x667 and OPENED before any gate, and PUZZLE 6 built.** (1) `fitView` centred `sceneBounds`, so one tree beside the loop dragged the railway left (ten pixels on the loop's left, eighty between it and the tree, the loop at 78% of the width). `sceneBounds` is now split: `railBounds()` is the track and its platforms with no scenery in it, and it is what the camera CENTRES on; a prop is allowed to push only the ZOOM, by taking the half extent from the railway's middle to whichever scene edge is further away. The loop is now dead centre at 412x915 and at 375x667. (2) The Crossing runs TWO straights after the red curve, so Red goes right and down while Blue goes up; every index past piece 11 moved by one (stations 13 and 22, levers 4 and 19, blue train piece 16, solution 4, 19, 19); `--solve` still 6.73 s three stars, `--race` unchanged. Stamp 20260906g in all three places, lint green. **Shot and OPENED** `p3-412x915.png`, `p3-375x667.png`, `p2-crossing-tall.png`, `p2-crossing-wide.png` (24 shots regenerated). What I saw at 412x915: the loop is horizontally centred on the screen at last, top chrome clear, tray on wool, nothing in the music chip corner. Three faults, named: (a) **the railway got SMALLER for it**, about 65% of the screen width where it was 78%, because a lone tree on the right and the house group on the left now push the zoom out symmetrically from the railway's middle rather than sliding the picture; that is the honest price of a centred fit with lopsided scenery, and the fix is the PROPS, not the camera; (b) the props are unmotivated: a house, a tower and two bushes are jammed within twenty pixels of the loop's top left while a single tree floats a hundred and sixty pixels off to the right with nothing near it, and nothing faces anything; (c) the cow and two bushes stand INSIDE the loop where the cars sweep past, and the strip of wool below the tray (about 95 px at 412x915) holds nothing at all. In The Crossing: Blue's station is now top and Red's is right and well down, which reads as two directions; still wrong are that the two spurs are still mirror images at the same forty five degrees, the two lever dots are still the smallest marks on the rug (smaller than the cow), and the buffer stub at the end of Red's line reads as a loose piece rather than a buffer. (3) **Swap is built** as data, `id 5`, on The Passing Loop's shape with a THIRD switch under the lower one choosing between Red's station and a short dead end curve. **Par is 2, not the plan's 4:** every constant lever setting over the three switches was searched and two flips is the floor, with two different two flip answers because either train may be the one that takes the loop. A four switch version with a second spur above the loop was built, measured and thrown away because no solution ever needed that lever (DECISIONS). Nothing at all bumps. Watched to fail: with the loop's two curves deleted from Swap's build the row reads `never 0 NO WIN CRASHED HAD TO STOP NOT THREE STARS`, exit 1. `PUZZLE_COUNT` in `test/layout.mjs` widened 5 to 6 before the puzzle went in. **Gates:** `tools/check.js` `ALL GATES PASSED`, twelve of twelve, 16:50 UTC, first run, nothing rerun alone; output in section 13. Commits `46102157` and `c9ee9a77`; everything is committed. **Next action:** `flock -w 1800 /tmp/sws-gate.lock node tools/shots.mjs p2` and OPEN the Swap card (Swap is the sixth puzzle and has NEVER BEEN SHOT, so nobody has seen it; check the dead end curve reads as a buffer and not a loose piece, and that the layout, 3.3 U wide by 15 U tall, is not a thread on a 412x915 phone); then the props, which are now the thing binding the zoom: give `makeProps` a keep out band around the railway's bounding box and group them in motivated pairs instead of scattering, then reshoot 412x915 and measure the loop's width as a fraction of the screen before and after; then the two lever dots, which are smaller than the cow.
...
### 2026-09-06 builder, 15:52 to 16:52 UTC (the fit centre, and Swap)
...
PASSING LOOP is what lets two trains change ends, so the mutation takes the
loop's two curves out of Swap's build and leaves everything else alone. It is
not a rename and not a nudged constant; it is the design's own premise removed.

```
$ cp index.html $SD/mutant.html    # then delete ["from",2,2],["p","curveL"],["p","curveL"]
$ WHISTLESTOP_HTML=$SD/mutant.html node sim.js --solve
...
**The full suite, 16:50 UTC, first run, nothing rerun alone.**
...
`test/layout.mjs` `PUZZLE_COUNT` was widened 5 to 6 BEFORE Swap went in; a grep
of `test/`, `tools/` and `sim.js` for a hardcoded puzzle count found that line
and nothing else, and `sim.js --solve` and `suitePuzzles` both loop over
`PUZZLES.length`.
