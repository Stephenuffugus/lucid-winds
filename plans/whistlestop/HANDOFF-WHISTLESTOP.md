...
- 2026-09-16 (UTC), Opus (HANDOFF-OPUS-SEP15 lane B, B7; the row is `HANDOFF-OPUS-SEP07-NIGHT.md` T2.3):
  **PUZZLES 7 TO 10 BUILT AND LIVE, stamp `20260916a`. Deploy proof: main `b83268bc` (parent verified as
  ...
  lap, mutants, boot, build, run, share, sound, layout, thumb, ALL GATES PASSED (13). New laws watched red:
  par (a padded par that solve passes), the window (Gold 2.2 U behind Green), the puzzle list count
  (now read from the page, floor 6). Shots opened: `docs/shots/p4-puzzle7..10-tall.png` and `-run-tall.png`.
  **Faults named, left:** Four Stations' blue station building sits over the neighbouring track and its
  three levers stack in the middle; The Figure Eight's two levers and the crossing knot together at the
  centre and the eight is 13.6 U square, so it draws about a third smaller than The Crossing on a phone;
  The Timed Loop's two stations sit close enough to read as one cluster. D-2026-09-16 in DECISIONS.
  **Next action:** puzzles 11 and 12 need mechanics. Two Loops: a lever shared by two switches (a
  `links` field on the puzzle, `flipLever` flipping the partner, one flip counted, a drawn tie between
  the two levers, `--par` branching both at once). The Shunt: uncoupling (T2.3's note: a `cars` count on
  the train and a siding piece that keeps them). Each is a design first, then data, then the par gate.
...
- 2026-09-06 builder (fit centre, Swap, levers; 15:52 to 16:25 UTC, shared tree, one gate lock): **both fit steps, PUZZLE 6, and one fault off the section 15 list. Four commits, `46102157` `c9ee9a77` `6fc2dcfb` `da77976e`; nothing uncommitted.** (1) `fitView` centred `sceneBounds`, so one tree beside the loop dragged the railway left (ten pixels on the loop's left, eighty between it and the tree, the loop at 78% of the width). `sceneBounds` is split: `railBounds()` is the track and its platforms with NO scenery, and it is what the camera centres on; a prop pushes only the ZOOM, by taking the half extent from the railway's middle to whichever scene edge is further. The loop is dead centre at 412x915 and 375x667. (2) The Crossing runs TWO straights after the red curve; every index past piece 11 moved by one (stations 13 and 22, levers 4 and 19, blue train 16, solution 4, 19, 19); `--solve` still 6.73 s three stars, `--race` unchanged. (3) **Swap is built**, `id 5`, The Passing Loop's shape with a THIRD switch under the lower one choosing Red's station over a short dead end curve, and two curves at each end that swing the termini out in opposite directions. **Par is 2, not the plan's 4:** every constant lever setting over the three switches was searched and two flips is the floor, with TWO different two flip answers because either train may be the one that takes the loop; a four switch version with a second spur above the loop was built, measured and thrown away because no answer needed that lever (DECISIONS). Home in 9.53 s, three stars, nothing at all bumps. (4) The lever glyph was drawn in pure world units while `TAP_R` is 26 CSS px of screen, so in a puzzle the one thing you tap was the one thing that shrank; its scale is floored at `CONFIG.LEVER_MIN_PX` (20 CSS px across the base) with its world position untouched, so the tap test and `junctions()` are unchanged and the build screen is pixel identical. Stamp 20260906g in all three places, lint green. **Gates:** `tools/check.js` `ALL GATES PASSED`, twelve of twelve, FIVE times (15:50, 16:13, 16:18, 16:22 and 16:31 UTC), every one a first run with nothing rerun alone, every one a FIRST run with nothing rerun alone; output in section 13. **Shot and OPENED:** `p3-412x915.png`, `p3-375x667.png`, `p2-crossing-tall.png`, `p2-crossing-wide.png`, and the new `p2-swap-tall.png` and `p2-swappass-tall.png` (twice each, before and after the termini). What I saw at 412x915: the loop is horizontally centred at last, chrome clear, tray on wool, nothing in the music chip corner. Three faults there, named: (a) **the railway got SMALLER for it**, about 65% of the screen width where it was 78%, because a lone tree on the right and the house group on the left now push the zoom out symmetrically from the railway's middle instead of sliding the picture; that is the honest price of a centred fit with lopsided scenery and **the fix is the PROPS, not the camera**; (b) the props are unmotivated, a house, a tower and two bushes jammed within twenty pixels of the loop's top left while one tree floats a hundred and sixty pixels off to the right with nothing near it, and nothing faces anything; (c) the cow and two bushes stand INSIDE the loop where the cars sweep past, and the strip of wool below the tray (about 95 px) holds nothing. In Swap's card: the first version WAS a thread, a railway a hundred pixels across a 375 wide screen with a hand of wool either side, which is why the termini were curved out; after it, the railway fills 55% of the width and the two stations sit in opposite corners. Three faults left in that frame: the dead end curve has no buffer cap so the trap reads as an unfinished piece rather than a dead end; switches B and F sit thirty pixels apart on the spine so the lower two levers read as one cluster; and a red engine standing under a blue roof is the whole point of a swap but nothing on the card says so, so a child's first read is that the game put the train at the wrong station. In The Crossing: Blue's station is top and Red's is right and well down, which reads as two directions; still wrong are that the two spurs are mirror images at the same forty five degrees and the buffer stub at the end of Red's line reads as a loose piece. **The lever floor IS gated** (`test/layout.mjs`, a canvas reader, watched to fail at all five sizes with the floor taken out; section 13). **Next action, in order:** (i) the props, which are now the thing binding the zoom, so give `makeProps` a keep out band around `railBounds()` and place them in motivated pairs instead of scattering, then reshoot 412x915 and measure the loop's width as a fraction of the screen before and after; (iii) **DONE in this session:** `drawBuffers` puts a buffer stop on every open end and not just the ends with a station, because the sim turns a train at every one of them; six of the six puzzles had a bare end and The Crossing had four, so this also closes the crossing shot's 'the buffer stub reads as a loose piece'. What is left after it: the two spurs in The Crossing are still mirror images at the same forty five degrees, and switches B and F in Swap sit thirty pixels apart so the lower two levers read as one cluster.
...
### 2026-09-06 builder, 15:52 to 16:25 UTC (the fit centre, Swap, the levers)
...
$ node sim.js --solve
  ...
  Three Trains               3     3     3     4.58 s      3   bumps
  Round and Round            1     1     1    12.28 s      3   never gets home
  Swap                       2     2     2     9.53 s      3   bumps

WHISTLESTOP SOLVE OK

$ node sim.js --test | tail -2
PASSED 175 / FAILED 0   (total 175)
WHISTLESTOP TEST OK
...
**The solve gate, watched to fail on Swap.** The claim Swap makes is that the
PASSING LOOP is what lets two trains change ends, so the mutation attacks that
and nothing else. The first attempt DELETED the loop's two curves and was
thrown away: it shifted every piece index and `makePuzzle` threw, which proves
nothing about the design. The mutation that counts REPLACES the two `curveL`
with two `straight`, so the piece count and every index are identical and the
only thing gone is the loop's shape.

```
$ cp index.html $SD/mutant.html   # ["p","curveL"],["p","curveL"] -> ["p","straight"],["p","straight"]
$ WHISTLESTOP_HTML=$SD/mutant.html node sim.js --solve; echo exit=$?
  Swap                       2     2     2      never      0   bumps   NO WIN   CRASHED   HAD TO STOP   NOT THREE STARS

4 PUZZLE PROBLEM(S)
exit=1
```

**Swap's par, measured rather than asserted.** Every constant lever setting
over the three switches was run against every subset of flips. Eight starts,
eight scripts each. Only one start gives a floor of two flips and no way to
win by doing nothing:

```
{"2":0,"9":0,"11":0} nothing: bumps   wins: 1:[2->1]@4.97 | 1:[9->1]@4.97
{"2":0,"9":1,"11":0} nothing: WINS ITSELF
{"2":0,"9":0,"11":1} nothing: bumps   wins: 2:[2->1,11->0] | 2:[9->1,11->0]   <- shipped
{"2":1,"9":0,"11":1} nothing: bumps   wins: 1:[11->0]
```

A four switch version with a second spur above the loop was built and measured
first; no winning script ever touched that lever, so the switch came out.

**The full suite, three times, every one a first run with nothing rerun alone.**

```
$ timeout 900 flock -w 1800 /tmp/sws-gate.lock node tools/check.js
sim  pass 0s   lint  pass 0s   solve pass 0s   lap    pass  0s
mutants pass 7s  boot pass 3s  build pass 5s   run    pass  3s
share  pass 7s  sound pass 2s  layout pass 11s thumb  pass  2s

ALL GATES PASSED
```

Run once after the fit centre and The Crossing, once after Swap's termini were
curved out, and once after the lever floor. `test/layout.mjs` `PUZZLE_COUNT`
was widened 5 to 6 BEFORE Swap went in; a grep of `test/`, `tools/` and
`sim.js` found that line and nothing else, and `--solve` and `suitePuzzles`
both loop over `PUZZLES.length`.

**The lever floor's own gate, and it is a canvas reader.** A test written from
`CONFIG.LEVER_MIN_PX` would only restate the constant, so this one opens
puzzle 5, takes `junctions()[0].screen`, reads the canvas `ImageData` in a tap
radius around it and measures the extent of `PAL.gold` (#D9A441), which within
26 px of a lever is the handle and the knob and nothing else. The threshold was
MEASURED and then set between the two clusters:

```
$ flock -w 1800 /tmp/sws-gate.lock node test/layout.mjs      # the floor in place
  ok  667x375   22.5 px of gold across, 93 gold pixels, fit zoom 0.25
  ok  915x412   22.5 px, 91 px, zoom 0.25
  ok  375x667   22.1 px, 98 px, zoom 0.34
  ok  320x568   22.1 px, 102 px, zoom 0.26
  ok  412x915   22.5 px, 92 px, zoom 0.41
LAYOUT OK

# MUTATION: var gz = V.zoom, the floor taken out, one line
$ flock -w 1800 /tmp/sws-gate.lock node test/layout.mjs
  FAIL  667x375   8.2 px of gold across, 12 gold pixels, fit zoom 0.25
  FAIL  915x412   8.2 px, 12 px
  FAIL  375x667  11.4 px, 26 px
  FAIL  320x568   8.2 px, 15 px
  FAIL  412x915  14.6 px, 46 px
5 LAYOUT FAILURE(S)     exit=1
```

The fit zoom in a puzzle runs 0.25 to 0.41, so the floor is doing work at every
size the game is shot at, and the line at 17 px is nowhere near either cluster.
The mutation was reverted and the full suite is green again at 16:22 UTC.
