...
- 2026-09-08 (UTC), Fable's builder: **DONE, THE DOODADS SHELF (docs/GEAR-DOODADS-SEP08.md, Airworthy
  ...
  **Gates:** `timeout 590 flock -w 200 /tmp/sws-gate.lock node tools/check.js` on the committed file: **ALL GATES PASSED, eleven of eleven** (sim 228 assertions, 39 new in `suiteDoodads`; lint with dupkeys clean at 53 literals; throw; fold; doodads, the new gate, 62 lines at 375x667, 412x915 and over a hangar saved before the shelf; tunnel; challenge; sound; audio; play; layout with the trim loop reading the shelf off the page and the corner scanned with the sheet up), 49 s on an idle box. `node tools/dupkeys.mjs` by name: 0 duplicate keys in 346 literals. **The first browser runs were red and the gates were wrong, not the game:** every chip tap in the new gate read "clip none" because the harness's `tapAt` sends pointerdown and pointerup and no click, which is right for the where canvas (pointerdown) and wrong for a button (the gate's `tapChip` now clicks the element under the thumb, the way `tap` does); layout read "covered" at 667x375, 915x412 and 320x568 because `centre()` had scrolled THROW IT into view and pushed the shelf above the sheet's box, so the shelf is measured at the top of the sheet where it opens and the line says how far THROW IT had scrolled it; and at 320 "Paperclip" did not fit beside its drawing in an 89 px chip, so under 340 px the chips go two to a row.
  **Browser mutations, each planted in the live file by a runner with a restoring trap, watched red, md5 identical after:** B1 shelf chips 48 to 30 px (layout 5 red, one per size, "115x30"; doodads 4 red); B2 the locked check taken out of the chip tap (doodads 4 red: "still carries nose", the feat toast empty, the seat not lit, two rings); B3 `cloneSpec` dropping `doodad` (doodads 2 red: "clip mid" and 0 copper pixels on the hangar card; **the fold link stayed GREEN under it, which is true and worth knowing: the link packs `G.spec` straight from `shopStart` and never passes the whitelist**); B9 `packSpec` dropping the tenth byte (fold 1 red: "doodad"), so the fold line can fail; B4 THROW IT's 106 px reserve to 0 (layout 3 red, btnTrimDone at 18, 18 and 26 on the three portrait sizes; doodads 1 red); B5 the import strip taken out (doodads 1 red: "spinner wing"); B7 the badge line removed (1 red); B8 the penny drawn grey (1 red, 0 copper pixels). ⛔ **B6 `BOUNCE_MAX` 0: the first run came back GREEN over the plant**, "bounced once (1)", its log differing from a clean run in the three ball lines only. A diagnostic under the lock with the same plant showed the served text carrying `BOUNCE_MAX: 0`, `fly()` at 0 bounces and the field at 0 bounces; the identical second run (B6b) was red on all three ball lines. I cannot explain the first run and am not calling it explained: the plant is proven to reach the page and the assertion is proven to fail under it, once each.
  **Shots opened** (`docs/shots/p6-doodads-412.png`, `p6-doodads-375.png`, `p6-doodads-flight.png`, `p6-ball-card.png`, `p6-brick-card.png`, all under 60 KB). First pass, three things wrong and what I did: the where box had a DASHED border, the same language as the locked chips and the allowed place rings, so the plane's box read as one more locked thing (fixed, a solid hairline); the HUD said 4.5 g with a penny on, because the shot poked the spec instead of tapping the chip (the shot taps the chip now and the HUD reads 7.0 g; the game always went through the tap); the Brick card had no badge line because the contact sheet's spinner panel landed on its own during the two waited frames and took the badge (the shot resets it; the gate had already proved the line). Reshot and opened again: the hairline, 7.0 g, the badge line. **Still seen and left:** the "middle" label sits over the plane's body on the where canvas (legible, crowded); at 375 the HUD wraps to three lines with "· 7.0 g" alone on the third (the HUD's own line, not the shelf's); the locked subtitles "a silver", "3 silver", "a gold" are terse; the sheet covers the whole field at 375 so the plane in the hand is not seen while trimming (as before the shelf); in flight every doodad is an honest sized dot on a 40 px plane (the ball, the penny and the spinner read, the eyes and the band are specks), a painted sheet would help; the ball card and the Brick card each say "1 stall" and both are true by the sim (the ball pitches up after the bounce, the Brick stalls as it falls).
  ⛔ **One thing I did wrong tonight, for the next builder:** to stop a suite I had started before a CSS fix I killed by parent PID chain without reading the names, and the chain above `check.js` was the `claude` process this session runs in (2545) and its terminal (1719); both survived the SIGTERM. Never kill a chain; kill the one PID whose name you have read, or let the run finish (it had, on its own, at 03:49:56).
  **Next action:** his thumb on the shelf on the phone (tape the penny on, move it to the middle, throw; tape the spinner on once), then the note's question to him: is eight the right size for the bank. The Updraft section of the note is next in the note's own order.
...
- 2026-09-08 00:40 UTC, Fable: **THE SORT of his Sep 07 notes** (verified by one read-only agent per game and a
  ...
  **18, 19, 22 are ONE FAULT, confirmed three times:** shopStart (`:2633-2644`) sets SHOP.sweeping false and never calls shopStartSweep; the only callers are NEXT, BACK (which at step 0 exits) and the Steady Hands toggle (hidden off the field). So on crease 1 the marker sits frozen at the left under "tap when the marker is in the middle", shopTap (`:2705`) drops every tap before any sound or label, NEXT toasts "Pick one, then press the crease", and no plane has ever been foldable by a thumb since the workshop was committed (a597dff4, Sep 06 03:01 UTC). Live page identical. Hangar stays empty so TO THE GYM hands him the starter forever; 12 m is the starter's designed ceiling (sim: 12.50 m at angle 0; TRIM's best is 13.28; the Dart is 22.7; the earned wing 24.6), and the workshop is the only lever. ONE LINE to fix. Gate: fold.mjs:59-62 and play.mjs:52-54 call AIRWORTHY_TEST.shopMarker(0.5) before every tap and that hook SETS sweeping true; fold.mjs even asserts "the bar is a 48 px target on crease 1" then presses through the hook. The plan's "eleven of eleven green" stood over a workshop no thumb can pass. The rider in 19 ("is that the mechanic you want") is TASTE on the kept design (3.3, Steady Hands). 20 KNOWN calls 34/48 (the ladder is built additive; he never saw it because crease 1 blocks) and the wind tunnel exists. 21 NEW WORK call 61: materials do not exist; the only weight is the paperclip and it lives in TRIM after a throw, NOT in the workshop (calls 34 and 48 say "in the workshop" and are wrong about where); four courses exist but a free throw is gym only.
