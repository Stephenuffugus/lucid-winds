...
- SHOT p3-find-reveal-320x568.png OPENED. The two rings still read clearly at 320 (white on the child’s choice, gold on the true piece), which remains the best reveal in the catalog. Faults: (1) the seventh tile sits alone on its own row under two rows of three, so at this width the grid reads as six plus an afterthought; (2) the target card at the top is the same brown as the page with no edge, so the piece to find floats; (3) nothing in words says which ring means which; (4) go on is a bare arrow in the bottom left, furthest from the grid.
- SHOT p3-village-1366x768.png OPENED. Faults: (1) the house sits in the top left corner of a 620 px wide board with the rest empty, the corner disease at its most exposed on a wide screen; (2) the browser’s blue focus ring is on play again, a third NOTCH screen showing it; (3) the board and the page are two dark browns a shade apart, so on a laptop the board has almost no edge; (4) the board occupies the middle of the screen with roughly 370 px bare on each flank and 200 px bare below.
- SHOT p3-doors-320x568.png OPENED. Faults: (1) the two doors still carry no words at 320, so a child chooses between two brown shapes; (2) the left door’s piece is drawn at an angle and clipped by its own inner frame while the right door’s stands upright and whole, two treatments side by side, and at this width the clipping is worse; (3) the board above shows a piece half lit and half in flat black, and at 320 the black half reads as a hole in the board rather than as shadow; (4) the doors sit in a band across the middle with about 340 px of empty page below them.
- PLANT layout y1 (aside and go on at 30 px): green run LAYOUT OK, planted red twice — "320x568 TURN turning: the controls are 56 px targets and the gear 48 px: #aside 30x30 (needs 56)" and the same for #next after the reveal. The touch target law reads rendered pixels at the narrow size, so a shrunken control is caught in both states. COUNTS. NOTCH plants counted: turn k1 and k2, config c1, pace p1, art w1, layout y1.
- SHOT p3-turn-reveal-320x568.png OPENED. Faults: (1) the browser’s blue focus ring is on go on again, the FOURTH NOTCH screen showing it, and blue belongs to nothing in this brown game — this is now a game wide fault, not a screen one; (2) the reveal is identical to the playing screen but for the piece having moved, so nothing says the round ended or that the placement was right; (3) the piece runs within about 15 px of the board’s right edge while a wide margin sits on its left; (4) the diagonal stripes cross board and piece unbroken, so a fitted piece does not read as fitted.
- PLANT PASS COMPLETE. COUNTED RED on their own green runs: turn k1 and k2, config c1, pace p1, audio a1 (a tick on every move), offline o1 (a worker that deletes every cache), layout y1 (controls shrunk to 30 px), specimens v1 (a reload mid session still earns), art w1 (a second wood). Nine plants.
- OWED: reveal r1 and numerals n1, whose GREEN RUNS TIMED OUT at 30 s in three separate attempts including one run alone, which now points at the reveal gate itself rather than at contention; the gate is being timed on its own. And find f1 (the chosen region outlined, not the piece), which planted nothing — FIND stayed green with the fault in place, so like TINT’s pour plant it names a hole in the law rather than a bad plant: the find law does not claim WHICH element carries the outline.
- REVEAL GATE RED, ALONE, AND IT IS NOT CONTENTION. Run by itself on an otherwise idle box it still died after 2 minutes 31 seconds inside its own revealed() helper (test/reveal.mjs:31, a waitForFunction timeout). That is the third timeout, and the first with nothing else running, so the earlier readings I blamed on six browsers sharing two cores were only partly that. Until this is understood, NOTCH’s reveal gate is RED and its plants r1 and n1 stay owed — NOTCH does not deploy on eight green gates and a red one.
- SHOT p3-village-320x568.png OPENED. Faults: (1) the house sits in the top left corner of an otherwise empty board, the corner disease at the narrow size; (2) the browser’s blue focus ring is on play again, now seen on five NOTCH screens, so it is a game wide fault; (3) board brown and page brown are a shade apart, so the board has almost no edge; (4) the bottom third is empty below the play button.
- SHOT p3-find-reveal-1366x768.png OPENED. The two marks read clearly at this width too, a white ring on the child’s choice and a gold ring on the true piece, still the best reveal in the catalog. Faults: (1) the grid of seven sits as three, three and one, so the last tile hangs alone under a gap at every width; (2) the target card above has no edge against the page, so the piece to find floats free of any frame; (3) go on is a bare arrow under the grid’s left corner, far from where the eye ends; (4) the whole game occupies the middle third of a 1366 px screen with roughly 470 px bare on each flank.
- REVEAL PROBED, and the gate’s timeout is NOT a dead reveal: tapping #aside on the FIRST trial gives phase=reveal, revealDone=true, frames=1 inside a second, and it stays true for twelve seconds after. So the page reveals promptly in that state and the gate stalls somewhere later, after its own key driven seating of the trials in between. Probing successive trials to find which one stops settling. ⛔ worth noting for its own sake: the reveal recorded ONE frame, which for a turn that is supposed to run at sixty degrees a second is either the reduced motion path or a reveal that skips its turn; the frame count is the thing to watch next.
- REVEAL PROBED ACROSS EIGHT TRIALS: every single one settles. Tapping #aside on each trial in turn gives phase=reveal and revealDone=true within a second, with frame counts of 32, 62, 92, 122 and 182 across trials, so the reveal runs its turn properly and the length varies with the angle as it should. The page is NOT broken. That leaves the gate: it taps #aside only on its two target trials and seats the rest by keys, and #aside is HIDDEN during a reveal, so the likely story is that the gate taps a hidden button after its own key seating and then waits thirty seconds for a reveal nobody asked for. The helper now reports phase, frames, whether aside and go on are visible, and the angle when it gives up, so the next run names it instead of timing out in silence.
- REVEAL CAUSE FOUND, and it is the GATE: the helper reported phase "turn", #aside VISIBLE, frames 0, angle -120 at the moment it gave up. So the gate had tapped #aside while its own key driven turn was still animating, the page ignored the button mid turn, and the gate then waited thirty seconds for a reveal nobody had asked for. Three timeouts, including one on an idle box, all from this. FIX: the gate waits for the angle to stop changing before it asks to set a piece aside. ⛔ worth a separate thought for Stephen: a child can also tap that button mid turn and have nothing happen, which is a live button that quietly ignores a press. Not changed tonight, recorded here.
- SHOT p3-find-reveal-412x915.png OPENED. The two rings read clearly at the tall size too, white on the child’s choice and gold on the true piece. Faults: (1) the seventh tile sits alone on its own row under two rows of three at this width as well, so the grid reads as six and a straggler everywhere; (2) the target card at the top has no edge against the page, so the piece to find floats; (3) go on sits at the far bottom left, diagonally opposite the tile the eye just left; (4) the bottom 40 per cent of a 915 px tall screen is empty.
- REVEAL STILL RED WITH THE SAME EVIDENCE after the steady wait: {"phase":"turn","done":false,"frames":0,"aside":true,"next":false,"angle":-120}. So waiting for the angle to stop changed nothing, which means the angle was ALREADY stable at -120 and the piece was not mid animation at all — my fix addressed a cause that was not there, and the ledger says so. What the evidence actually says: the piece has been TURNED BY KEYS to -120, #aside is visible, and tapping it produces no reveal. My earlier probe tapped #aside on an UNTURNED piece every time and it always worked. So the suspicion is now: after a key turn, the set aside button stops responding. Probing it two ways, a pointer tap and a keyboard press on the same button, because those fail for different reasons: a tap that misses is the gate’s fault, a press that does nothing is the page’s.
- SHOT p3-turn-412x915.png OPENED. The piece is large and its shape reads clearly at this size, the best TURN screen of the four widths. Faults: (1) the diagonal stripes still run unbroken across board and piece, so the piece’s edges dissolve into the floor; (2) the tray thumbnail sits alone in the bottom left at about 150 px holding a shape drawn at 40 px, unreadable; (3) board brown and page brown are a shade apart, so the play area has almost no boundary; (4) the bottom 45 per cent of the screen is empty.
- ⛔ THIRD TIME TONIGHT I POINTED PLANT_ROOT AT AN EMPTY FOLDER: the r1 and n1 rerun died at cp with "(no result line) [exit null]" and tested nothing. Those two plants remain owed, though they are blocked behind the reveal gate being red in any case. The rule, written here for the third time: the runner needs a POPULATED snapshot, so build it with git archive HEAD in the same command that runs the plants.
- SHOT p3-doors-412x915.png OPENED. Faults: (1) the two doors still carry no words at this size, so a child chooses between two brown shapes at every width tested; (2) the left door’s piece is drawn at an angle and clipped by its own inner frame while the right door’s stands upright and whole, two treatments side by side; (3) the board above shows a piece half lit and half in flat black, and the black half reads as a hole cut in the board rather than as shadow; (4) the doors sit in a band across the middle with roughly 690 px of empty page below them, the emptiest NOTCH screen of the four widths.
- PROBE DID NOT REPRODUCE IT, and says so: my three ArrowRight presses left the angle at 0 ("after three key turns: angle 0"), so the piece never turned, and tapping #aside then worked immediately and so did pressing Enter on it. That tells me nothing about the gate’s state at angle -120 — the probe tested an unturned piece for a third time. The page clearly binds its turning keys somewhere my presses did not reach (focus target or different keys), so the next probe copies the gate’s own seatByKeys logic verbatim instead of my guess at it. ⛔ a probe that fails to reach the state under suspicion is not evidence of anything, and it would have been easy to read "tapped aside, settled=true" as clearing the page.
- CAUSE FOUND, WITH THE LINE THAT PROVES IT: the page binds its turning keys to the BOARD — main.js adds the keydown listener to the svg (line 170, ignoring anything that is not ArrowLeft or ArrowRight) and advertises aria-keyshortcuts on it (line 84) — while the gate’s seatByKeys calls page.keyboard.press() without ever focusing that element. The presses land on the body and do nothing, so the piece never seats, phase stays "turn", the loop spends its fourteen presses, and the gate then waits thirty seconds for a reveal nobody asked for. That is exactly the state the helper reported: phase turn, frames 0, aside visible, angle -120 (the -120 coming from the few presses that did land while the board still held focus after a click). FIX: focus the element carrying aria-keyshortcuts before each press. ⛔ my own probe reproduced the gate’s blindness perfectly and I nearly read it as the page being fine: pressing keys at a page is not the same as pressing keys at the thing that listens.
- SHOT p3-turn-reveal-412x915.png OPENED. Faults: (1) the browser’s blue focus ring is on go on again — that is now FIVE NOTCH screens across three widths, so it is a game wide fault and belongs in the page, not in a list of screens; (2) the reveal is identical to the playing screen but for the piece having moved, so nothing says the round ended or that the placement was right; (3) the diagonal stripes cross board and piece unbroken, so a fitted piece does not read as fitted; (4) the bottom 45 per cent of the screen is empty.
- REVEAL STILL RED AFTER THE FOCUS FIX, with the same angle of -120 in every run — and that constancy is the clue. A loop that keeps pressing and always parks on the same number is pressing the WRONG WAY: the gate assumed ArrowLeft raises a negative angle toward zero, while main.js maps ArrowLeft to keyStep(angle, +1) and ArrowRight to keyStep(angle, -1). Rather than hard code the opposite and risk being wrong twice, the gate now presses one key, reads the angle, and keeps whichever key moved it TOWARDS zero. ⛔ two wrong fixes in a row on this gate (a steady wait for a piece that was not moving, then focus for keys that were pointed the wrong way), and both are written down; the evidence line was right each time and I read a new cause into it each time.
- REVEAL RED A THIRD TIME WITH THE SAME ANGLE, and the constancy is now the whole story: -120 appears in every run, before and after the focus fix and the key direction fix, which means THE KEYS HAVE NEVER TURNED THE PIECE AT ALL and -120 is simply the angle the round was dealt at. Every explanation I have tried (mid animation, wrong key, no focus) assumed the presses were landing. Likely reason focus did not help: an SVG is not focusable without a tabindex, so element.focus() can be a no op and the keys keep going to the body. Probing what actually holds focus after the call, and whether the handler fires at all when a keydown is dispatched straight at the board. ⛔ three fixes in a row on one gate, each aimed at a cause I inferred rather than measured; the evidence line never changed and I should have noticed the unchanging number sooner.
- FOCUS PROBE, AND IT OVERTURNS MY LAST THEORY TOO: after calling focus on the element carrying aria-keyshortcuts the active element IS svg#bench, and real presses DO turn the piece — ArrowLeft took the angle 0 to 15, ArrowRight took it 15 back to 0. So focus works and the keys work. (A KeyboardEvent dispatched straight at the board changed nothing, 0 to 0, which is a separate curiosity and not the cause.) What that leaves: the gate presses ONE learned direction over and over while the loop waits for the phase to leave "turn", so it marches the angle straight PAST zero and on down to -120, because arriving at zero evidently does not seat the piece by itself. That fits every run: the same -120 after fourteen presses. Probing whether reaching zero seats it, or whether something else must happen there. ⛔ FOURTH theory on this gate, and the first three were each contradicted by the next measurement; the one thing that never changed was the evidence line.
- SHOT p3-turn-1366x768.png OPENED. Faults: (1) the whole game is a 420 px board centred on a 1366 by 768 screen, with roughly 470 px bare on each flank and 200 px below, so on a laptop it reads as a small card adrift; (2) the tray thumbnail sits alone under the board’s left corner at about 75 px holding a shape drawn at 25 px, unreadable at any distance; (3) the diagonal stripes cross board and piece unbroken, so the piece’s edges dissolve into the floor; (4) board brown and page brown are a shade apart, so the play area has almost no boundary.
- THE MECHANISM IS SOUND, measured: from angle 0 a press of ArrowLeft gives 15, a press of ArrowRight brings it back to 0, AND AT ZERO THE PIECE SEATS BY ITSELF — phase went straight to reveal with revealDone true. So turning the piece home is all that is needed and the gate’s approach is right in principle. The failure is in how the loop walks some particular trial. Five theories have now each been contradicted by the next measurement, so the loop writes down its own trail (start angle, every press, the angle after it, and the state it gave up in) and the next run will name the trial instead of me naming a sixth cause.
- ⛔ MY OWN FILTER HAS BEEN HIDING THE ANSWER. Every run of this gate has been piped through `grep -v "^    at "` to drop stack noise — and those are exactly the lines that name WHICH revealed() call threw. Both of my instrumented log lines (the seating trail and the target trial report) failed to print, which should have told me the throw was somewhere I had not instrumented, and the stack would have said where in one line. This is the same mistake as cutting a gate’s output at the first bracket earlier tonight: the second time I have blinded myself while reading a gate, and the fourth instrument error overall. Re-running with the trace kept.
- THE STACK NAMES IT, and the gate may be right and the page wrong. With the trace kept, the instrumented line printed at last: "target trial 0 (right) before aside {phase:turn, angle:-120, mirror:false, start:-120, asideHidden:false, asideDisabled:false} after {phase:turn, done:false, frames:0}". So trial 0 IS a target trial, the piece is DEALT at -120, the set aside button is VISIBLE AND NOT DISABLED, the gate taps it, and four hundred milliseconds later nothing has happened. Put that beside the two probes: tapping aside at angle 0 starts the reveal within a second, and walking the piece to 0 makes it SEAT BY ITSELF. Which means set aside appears to act only at the home angle — exactly where it is not needed, since arriving there seats the piece anyway — and does nothing at the angle where a child would actually use it. A child who decides mid turn that a piece will not fit presses a live looking button and gets nothing. Reading the handler now to confirm the guard before calling it a page fault, because I have been wrong six times on this gate and will not be wrong a seventh by assertion.
- THE TAP HELPER CONFIRMS THE SUSPICION, in its own comment: `tap` reads the control’s centre, asks elementFromPoint what is ON TOP there, dispatches pointerdown and pointerup at that top element, then walks up from it to the nearest ancestor with a click() and calls that. So if anything covers the centre of #aside, the press lands on the coverer and the button never hears it — which matches exactly what the page shows: phase is turn, the handler’s only guard is `if (phase === 'turn') finish(true)`, and yet nothing happened. The probe now asks what elementFromPoint returns at that point.
- PROBE DID NOT REPRODUCE THE TRIAL, and says so plainly: opened at the same URL and seed, it dealt a task with startAngle 0 (not the -120 the gate meets), elementFromPoint at the button’s centre returned a path INSIDE the button (so nothing covers it there), and a real pointer click at those coordinates started the reveal at once. That is a clean result about a different trial and proves nothing about the failing one. Rather than build a third probe that might miss again, the question goes where the failure is: the gate’s own failing line now reports the button’s rect, its centre, what elementFromPoint returns there and whether that thing is inside the button. ⛔ two probes in a row have now failed to reach the state under suspicion — when that happens, instrument the failure itself instead of rebuilding the scene beside it.
- **CAUSE FOUND, AND IT IS A PAGE FAULT A CHILD WOULD HIT**: at the failing tap the gate now reports `atCentre: "shelf-canvas", insideButton: false`. The SHELF CANVAS COVERS THE CENTRE OF THE SET ASIDE BUTTON. Every press lands on the canvas instead of the button, which is why the handler never ran although its only guard is `if (phase === 'turn') finish(true)` and phase was turn, and why the button reported visible and enabled the whole time. A real thumb at that point hits the canvas too, so a child who decides a piece will not fit presses a live looking button and nothing happens — the gate was right and the page is wrong.
- This is exactly what the fleet rule about never proving a control with el.click() exists to catch: the harness’s tap asks elementFromPoint what is on top and presses THAT, like a real thumb, so it found an overlap that a direct .click() would have sailed straight through. Seven theories on this gate, and the only one that survived is the one an instrument measured rather than one I reasoned out.
- NEXT, in order: measure how far the overlap reaches (which sizes and which states), add a law that nothing may cover the centre of a control a child presses, watch it red, fix the page, watch it green. NOTCH does not deploy before that.
- SCOPE MEASURED, AND IT CONTRADICTS THE GATE, which narrows the fault rather than dissolving it: in a FRESH TURN round #aside reads OK at all four sizes (nothing covers its centre), and in the doors state it reads "COVERED by first", which is simply the intended startup overlay sitting over everything before play begins. So the shelf canvas is NOT over the button in ordinary play. But the gate’s failing line reported atCentre shelf-canvas at 1366x768 with phase turn — a live round with the SHELF CANVAS over the control. That means the overlap belongs to the state the gate reaches (where the shelf is on screen), not to TURN in general. Reading how #shelf-canvas is placed to see how it can end up over a live round’s controls. ⛔ the honest shape: a measurement that disagrees with a gate usually means the two stand in different states — the third time tonight, and each time the difference was the finding.
- **THE OVERLAY IS THE SHELF, AND THIS IS THE THIRD GATE TONIGHT WITH THE SAME SHAPE.** #shelf is `position: fixed; inset: 0; z-index: 25` — a full screen overlay — so while it is open every press lands on it. That explains every reading at once: #aside visible and enabled (it is behind the overlay, not hidden), phase still turn (the round underneath is live), elementFromPoint returning shelf-canvas, and a fresh TURN round showing no overlap at all because no shelf is open there. GAUGE’s same value gate had to learn to close the instrument case and HUSH’s settle gate the living clearing; NOTCH’s reveal gate never closed the shelf. It closes it now before reaching for a control.
- ⚠️ **A PAGE QUESTION FOR STEPHEN, recorded not fixed**: the round underneath the shelf is already in phase turn, so a child who finishes a session and is shown the shelf has a LIVE ROUND running behind the overlay, with its controls unreachable until they close it. That may be intended (the shelf is a pause) or it may be the same fault wearing a friendly face. It is not something to change at four in the morning on a game that is not live.
- THE SHELF FIX WORKED AND THE STACK MOVED: the failure is now inside seatByKeys (reveal.mjs:111) instead of at the aside tap, so closing the shelf let the gate past the target trial at last. What is left is the seating loop, and my own patch was the problem — learning ONE direction and pressing it repeatedly marches the angle straight past zero, which is where the piece seats, and on to -120 where the loop gives up. The probe that worked recomputed the direction every press and stopped as soon as the phase left turn. The loop does that now: ArrowLeft raises the angle, ArrowRight lowers it (measured), so the key is just the sign of where the piece stands.
- FIND LAW STRENGTHENED, closing the hole plant f1 found by planting nothing: the law asserted `m.outlined.length === 1` — it counted outlines and never said WHICH region carried one — so a plant that outlines the CHOSEN region instead of the piece kept the count at one and stayed green. A reveal that outlines the wrong piece teaches a child the wrong answer, which is the entire job of that screen. The law now names the slot: `m.outlined[0] === m.piece`. f1 should bite on the next run.
- PLANT find f1 COUNTS AT LAST: green run FIND OK, planted red — "right and wrong alike: the choice stays marked, the piece’s region is outlined ... [{right:true,correct:true,pressed:[6] ...}]". The plant outlines the CHOSEN region instead of the piece’s, and now that the law names the slot (`m.outlined[0] === m.piece`) rather than merely counting outlines, it bites. That is the second plant tonight that earned its keep by planting NOTHING and thereby naming a hole in a law (TINT’s pour was the first). TEN NOTCH plants now count: k1, k2, c1, p1, a1, o1, y1, v1, w1, f1. Owed: r1 and n1, both behind the reveal gate.
- **THE TRAIL SETTLES THE SEATING HALF**: "seatByKeys gave up after 14 presses: {phase:turn, angle:-120} trail -120->-120" repeated fourteen times — THE ANGLE NEVER MOVED ONCE. Not overshoot, not the wrong direction, not a mid animation wait: the presses are not reaching the board at all in this state. Two things sharpen it: the SAME run’s aside taps worked (both target trials went to phase reveal with 25 frames), so closing the shelf fixed that half and the page is responsive; and a standalone probe on a fresh page turns the piece with the same two keys. So it is this state, after a reveal, that swallows them.
- The trail now records what actually HOLDS focus at press time, the board’s tabindex, and whether the board is the active element — because "I called focus()" and "the element took focus" are different claims, and I have already conflated them once tonight when focusing the board looked like a fix and was not.
- **THE TRAIL NAMES THE OTHER HALF, and it is the same fault**: every one of the fourteen presses reported `active=shelf-go tabindex=0 boardFocused=false`. So the board IS focusable (tabindex 0), my focus() call DID run, and focus still sat on the SHELF’S GO BUTTON — the shelf is open during seating as well, holding focus, so every arrow key went to it and the angle never moved from -120. closeShelf was only called before the aside taps, which is precisely why that half began working while this one kept failing.
- FIX: the seating loop closes the shelf before it starts. ⛔ The whole NOTCH chase in one line: ONE overlay, left open by the gate, caused every symptom — the dead taps, the dead keys, the unchanging angle, three thirty second timeouts and eight theories. The measurements were right every time; what took the night was that I kept explaining the symptom instead of asking the instrument what it could see.
- **REVEAL OK.** The gate is green with the shelf closed in both places, and both target trials show it working: trial 0 (right, dealt at -120) and trial 3 (mirror, dealt at -150) each reach phase reveal with 25 frames. The seating walks the piece home and the piece seats itself at zero, exactly as the probe said it would.
- ⛔ THE WHOLE CHASE IN ONE SENTENCE: a full screen overlay (#shelf, position fixed, inset 0, z-index 25) was left open by the gate, and it swallowed every tap and every keypress — producing dead aside taps, dead arrow keys, an angle frozen at -120, three thirty second timeouts and EIGHT successive theories from me (mid animation, wrong key direction, no focus, keys bound to the board, marching past zero, a page refusing aside, a covering element, and finally the overlay itself). Every measurement along the way was correct; what cost the night was explaining the symptom instead of asking the instrument what it could SEE. The two lines that ended it were elementFromPoint at the button’s centre (shelf-canvas) and document.activeElement at press time (shelf-go).
- With reveal green, NOTCH’s owed plants r1 (the reveal ignores less motion) and n1 (a digit in the aside label) can finally run as pairs — they were never failures of the plants, only of the gate they stood behind.
- **PLANT r1 COUNTS**: green run REVEAL OK, planted red — "1366x768 with less motion a mirror’s whole reveal is done in 9225 ms (at most 300), seated and flipped". The moment the gate went green its plant bit immediately, which is the proof that r1 was never a bad plant: it spent the night standing behind a red gate. ELEVEN NOTCH plants now count; n1 is running.
- **PLANT n1 DOES NOT COUNT, AND THE NUMERALS GATE IS NOW RED**: its green run died with "TimeoutError: Waiting failed: 30000ms exceeded" — the same signature the reveal gate wore all night. The plant line printed underneath it ("the first screen shows no digit: aria-label on aside: Set it aside, it will not fit 2") proves nothing while the green run is red, exactly as the rule says.
- The obvious suspect is the one just proved next door: a full screen #shelf left open, swallowing taps and keypresses. But an obvious suspect is not a finding — I spent tonight being wrong eight times in a row by reasoning from a symptom — so the first step is simply to look at whether that gate closes the shelf at all, and then to let the gate say what it saw. NOTCH now stands at ELEVEN plants counted (k1, k2, c1, p1, a1, o1, y1, v1, w1, f1, r1) with n1 owed behind a red gate, and it does not deploy until numerals is green and n1 bites.
- NUMERALS HAS NO SHELF HANDLING AT ALL — the word does not appear in the gate — and it taps #aside and then waits on revealDone, which is the exact pattern that hung the reveal gate all night. Guarded the same way: the shelf is closed before the aside tap and before the settings tap. If the gate goes green, the cause is confirmed by the fix working; if it does not, it gets instrumented the way reveal was (elementFromPoint at the control’s centre, activeElement at press time) rather than reasoned about.
- **NUMERALS OK with the shelf guard, and PLANT n1 COUNTS** — planted red twice: "the first screen shows no digit: aria-label on aside: Set it aside, it will not fit 2" and the same across a session of twelve with a round set aside. So the SAME overlay was hanging a second gate, and the fix proved itself by turning both green. **TWELVE NOTCH PLANTS NOW COUNT**: k1, k2, c1, p1, a1, o1, y1, v1, w1, f1, r1, n1.
- BEFORE DEPLOYING: a full sweep of every NOTCH gate is running. The reveal chase edited three test files and the page was never touched, so the others should be unaffected — but "they were green earlier tonight" is exactly the stale evidence I have been correcting all night, and a deploy is not the place to start trusting it.
- **FULL SWEEP GREEN, ALL FOURTEEN GATES, in one fresh run from HEAD**: art, audio, config, engine, find, layout, numerals, offline, pace, project, reveal, shapes, specimens, turn. Twelve plants red (k1, k2, c1, p1, a1, o1, y1, v1, w1, f1, r1, n1). NOTCH is ready and deploys alone through a worktree on origin/main.
- Worth remembering what it took: TWO gates (reveal and numerals) were hung by ONE full screen overlay the gates never closed, and the chase cost eight wrong theories from me before two lines of instrumentation ended it — elementFromPoint at the control’s centre, and document.activeElement at press time. Both gates went green the moment the shelf was closed, and both of their plants bit immediately afterwards, which is the proof that neither plant was ever bad: they had been standing behind red gates all night.
- **LINK BUILDER DEPLOYED**: the branch version defines brim, crease, demo, gauge, glimpse, hush, notch, span, tint and yonder, while main carried only brim, crease, demo, glimpse, span and yonder — so it adds exactly the four games deployed tonight (GAUGE, HUSH, NOTCH, TINT). It has no gates of its own, so the pre deploy check was the honest one available: every path it links to (../gauge/, ../hush/, ../notch/, ../tint/) was verified present on origin/main BEFORE the push, so it cannot ship links to directories the host does not carry. Stamp 20260916f. With this, lane C is complete: seven games and the builder that lists them.
