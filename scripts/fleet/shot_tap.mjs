...
if(process.env.TAP_CELLS){ await pg.evaluate(()=>{const tds=[...document.querySelectorAll("#Xw td[onclick]")]; [0,1,2,6,7,12,18,24].forEach(i=>{ if(tds[i]) tds[i].click(); }); }); await new Promise(r=>setTimeout(r,600)); }
