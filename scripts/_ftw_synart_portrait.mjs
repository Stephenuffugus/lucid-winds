...
await sleep(2200);
await p.evaluate(()=>{setSpeed(0);S.inf=500;S.owned.add("sat");recompute(S);S.owned.add("graph");recompute(S);checkCombos(S);});
await sleep(900);
const st=await p.evaluate(()=>{const img=document.querySelector("#modalCard .synart");
  const vis=img?getComputedStyle(img).display:null;
  return{src:img?img.getAttribute("src"):null,display:vis,rect:img?img.getBoundingClientRect().height|0:0};});
console.log("portrait synergy plate:",JSON.stringify(st));
await p.screenshot({path:`${OUT}/19-synergy-plate-portrait.png`});
await br.close();
