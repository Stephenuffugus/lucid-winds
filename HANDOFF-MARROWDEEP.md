...
| `plans/marrowdeep/proto/engine.js`, `sim.mjs` | The prototype engine and its harness, a HEAD START rather than a finished thing: it was written while the audit was still correcting the rules, so it lags them. Its state is whatever `node plans/marrowdeep/proto/sim.mjs --test` says today, and P0 step 0 checks that first. `PROTO-REPORT.md` exists only if the tuning pass ran; without it, BALANCE holds the audit's reasoned defaults rather than measured ones. |
