...
**Still open, in order:**
0. **The re-verification above**: 412, 375 and 320 overflow sweeps DONE and fixed (two rows in §3, nine pages, all clean at all three widths). Dash sweep DONE (row in §3). Long tail 8 DONE (rows 61 to 70). Next: the exclamation pass (180, judged), then long tail 9 at row 71 (Dew Trail, Go, Vine Words, Root Flow, Four in a Row are the natives through row 99).
...
| **Satellite overflow sweeps, 375 and 320, after a start tap** (Fable, Sep 05) | `scripts/fleet/overflow.mjs` | 375: 119 of 119 clean (Letter Launch probed at `docs/`). 320: see SESSION STATE if this row is not updated |
