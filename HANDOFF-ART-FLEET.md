...
| **Long tail 13: Master Pollinator, 15 Puzzle, Root Rush, Pyramid, Farkle, Flood Fill, Five in a Row, Garden Spades (rows 141 to 158)** (Fable, second terminal, Sep 05) | `games/{pollen,slider,rootrush,pyramid,flood,vinecross,gardenspades}.js`, `games/_inline/farkle.js`, `shared.css` (`.gb[disabled]`, `.gc-empty`), pins `shared.css?v=20260905k` | Master Pollinator: the seat name inputs clipped "Computer" to "Comp" at 0.7rem and zoomed on focus, and the first fix (a wider flex share) left the field 18px wide at 320 because HUMAN, CPU and the ✕ cannot shrink below 48; the three buttons travel as one unit now and wrap to a second line when the field would starve, so the field measures 282/245/190px at 412/375/320 and "Computer" fits at all three at 0.9rem, rows 6px, and the setup modal carries a sage glow at its foot so the first screen is a meadow with a form on it. 15 Puzzle: fifteen tiles shared one silhouette, a per tile brightness jitter (0.93 to 1.07 by value) gives the board rhythm, the empty square is a recessed socket (`.Dsock`, positioned like a tile) instead of a hole, Undo dashed when dead. Root Rush: the ten browns spanned #654020 to #9a5836 and were one colour at 375, a pale birch, a grey bark and a dark peat are in the rotation, the corduroy grain is two irregular layers, the board sinks into the page on a 70px shadow, Next legible when dead, the exit gate 16px. Pyramid: covered cards were dimmed by opacity .5 and 21 of 28 merged into one white haze; they are dimmed by tone now (brightness .5 saturate .65 at opacity 1) so every card keeps its edge, the waste slot has a gold hairline inset (`.gc-empty`, fleet wide), STYLE 0.75rem. Farkle: the tray caption is the panel's serif at 0.75rem tracked instead of the only mono on the screen, the hint warmer, the pre roll pips real 10px dots, the panel feathers into the page on a 40px shadow and its pan is `min(100vw - 16px,560px)`. Flood Fill: the grid sits on a framed plate (gold hairline, inset shadow), unselected packs step back to .72 so the colourblind row stops shouting, section labels cream at .62, sage up and slate down in AUTUMN so the 17 wide board separates, and the three fill styles preview themselves (a leaf, an enamel disc, a faceted gem) instead of two glyphs beside one picture. Five in a Row: wood grain (26 faint strokes and two knots drawn under the grid, no image), a stepped lip and a deeper cast shadow on the goban, the stats line a header band with a rule, HINT gold bordered, UNDO/REDO dashed when dead (`.gb[disabled]` fleet wide). Garden Spades: the blue felt is the house green (#12271c to #0b1a12, noise re-tinted), the panel feathers into the page on a 24px 14px spread shadow, the trick well is 120px with a soft gold centre, every badge under the floor (0.48, 0.5, 0.6, 0.62rem) is 0.7rem, the team colour sage instead of a blue that vanished on blue. Before and after shot at 412x915 and looked at (folder `docs/games-pass-sep05/lt13-*.png`); `overflow.mjs` clean at 412/375/320 on all eight. Not done: the art lane rows (meadow plate, bench, soil, felt, table, dice sheet), the shell status line Root Rush prints its hint into (fleet chrome), emoji swaps (JOB 8), Farkle's saloon palette (a design call, it is deliberate), the "YOUR TEAM" label now wraps to two lines in its half width card at 412 |
