...
function GL(a){var SZ=5,gr=[],ini=[],mv=0,sl=0,pz=0,won=false; // won latch (2026-07-03): board used to stay live after solving — re-solving refired game_win/earn
  ...
  window._LN=function(){won=false;pz++;mv=0;document.getElementById('Lp').textContent=pz;document.getElementById('Lm').textContent='0';sm('');gn();rn()};window._LR=function(){won=false;gr=ini.slice();mv=0;document.getElementById('Lm').textContent='0';sm('Reset');rn()};_LN();}
