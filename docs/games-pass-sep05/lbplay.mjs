...
try{
  if(STAGES.includes("boot")) await bootShots(); else { await pg.goto(`http://127.0.0.1:${port}/index.html?lbtest=1`,{waitUntil:"load"}); await sleep(1200); if((await cur())==="s-how"){ await tap(/^got it$/i); await sleep(500);} await shot("home-resume"); }
